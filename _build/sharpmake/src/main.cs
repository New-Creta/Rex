...
public static class Main
{
  ...
  private static void AutoGenerateEnums()
  {
    ...
    {
      WriteAutoGeneratedArrayToFile(array_gen_settings.ElementType, array_gen_settings.Name, array_gen_settings.Includes, array_gen_settings.ProjectToArrayValues, array_gen_settings.Filepath);
    }
  }
  ...
  private static void WriteAutoGeneratedArrayToFile(string elementType, string name, List<string> includes, Dictionary<string, List<string>> arrayValues, string filename)
  {
    StringBuilder sb = new StringBuilder();
    ...
    foreach (var project_tags in arrayValues)
    {
      sb.AppendLine($"    // {elementType} values for {project_tags.Key}");

      foreach (string val in project_tags.Value)
      {
