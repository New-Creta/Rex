...
TEST_CASE("Quit App Event")
{
  ...
  auto subscription = rex::event_system().subscribe<rex::MouseScroll>([&num_quit_events_fired, &quit_msg](const rex::MouseScroll& evt)
    {
  ...
  rex::event_system().fire_event(rex::MouseScroll(quit_msg));
  REX_CHECK(num_quit_events_fired == 1);

  // Queued event fire
  quit_msg = "Test Quit Event 2";
  rex::event_system().enqueue_event(rex::MouseScroll(quit_msg));
  REX_CHECK(num_quit_events_fired == 1);
