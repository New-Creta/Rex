...
TEST_CASE("Quit App Event")
{
  ...
  auto subscription = rex::event_system().subscribe<rex::WindowDeactivated>([&num_quit_events_fired, &quit_msg](const rex::WindowDeactivated& evt)
    {
  ...
  rex::event_system().fire_event(rex::WindowDeactivated(quit_msg));
  REX_CHECK(num_quit_events_fired == 1);

  // Queued event fire
  quit_msg = "Test Quit Event 2";
  rex::event_system().enqueue_event(rex::WindowDeactivated(quit_msg));
  REX_CHECK(num_quit_events_fired == 1);
