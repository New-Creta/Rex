...
TEST_CASE("TEST - Memory Tracking - Single Memory Tag Allocation")
{
	...
	s32* y = new int(1);
	stats = rex::mem_tracker().current_tracking_stats();
	REX_CHECK(current_global_mem_usage + static_cast<s32>(sizeof(s32)) + debug_allocation_overhead == stats.usage_per_tag[rex::to_int(rex::MemoryTag::Global)].value());
...
TEST_CASE("TEST - Memory Tracking - Multiple Memory Tag Allocation")
{
...

	{
		...
		stats = rex::mem_tracker().current_tracking_stats();
		REX_CHECK(current_global_mem_usage == stats.usage_per_tag[rex::to_int(rex::MemoryTag::Global)].value());
		REX_CHECK(current_engine_mem_usage + static_cast<s32>(sizeof(s32)) + debug_allocation_overhead == stats.usage_per_tag[rex::to_int(rex::MemoryTag::Engine)].value());
