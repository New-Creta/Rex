...
namespace rex
{
    ...
    void trace_log(const LogCategoryBase& category, LogVerbosity verbosity, const T& msg)
    {
        ...
        case LogVerbosity::Fatal:
            trace_fatal_log(category, msg);
            break;
        case LogVerbosity::Error:
            trace_error_log(category, msg);
            break;
        case LogVerbosity::Warning:
            trace_warning_log(category, msg);
            break;
        case LogVerbosity::Log:
            trace_log_log(category, msg);
            break;
        case LogVerbosity::Verbose:
            trace_verbose_log(category, msg);
            break;
        case LogVerbosity::VeryVerbose:
            trace_very_verbose_log(category, msg);
            break;
        default:
            trace_fatal_log(category, "Unknown log category: {0}", category.getCategoryName());
            break;
        }
    }
    ...
    void trace_log(const LogCategoryBase& category, LogVerbosity verbosity, const FormatString& fmt, Args &&... args)
    {
        ...
        {
        case LogVerbosity::Fatal:
            trace_fatal_log(category, fmt, rsl::forward<Args>(args)...);
            break;
        case LogVerbosity::Error:
            trace_error_log(category, fmt, rsl::forward<Args>(args)...);
            break;
        case LogVerbosity::Warning:
            trace_warning_log(category, fmt, rsl::forward<Args>(args)...);
            break;
        case LogVerbosity::Log:
            trace_log_log(category, fmt, rsl::forward<Args>(args)...);
            break;
        case LogVerbosity::Verbose:
            trace_verbose_log(category, fmt, rsl::forward<Args>(args)...);
            break;
        case LogVerbosity::VeryVerbose:
            trace_very_verbose_log(category, fmt, rsl::forward<Args>(args)...);
            break;
        default:
            trace_fatal_log(category, "Unknown log category: {0}", category.getCategoryName());
            break;
