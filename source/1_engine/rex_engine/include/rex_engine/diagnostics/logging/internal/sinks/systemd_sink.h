...
inline rsl::shared_ptr<logger> systemd_logger_mt(
    const rsl::string &logger_name, const rsl::string &ident = "", bool enable_formatting = false)
{
    return Factory::template create<sinks::systemd_sink_mt>(logger_name, ident, enable_formatting);
}

template<typename Factory = rexlog::synchronous_factory>
inline rsl::shared_ptr<logger> systemd_logger_st(
    const rsl::string &logger_name, const rsl::string &ident = "", bool enable_formatting = false)
{
