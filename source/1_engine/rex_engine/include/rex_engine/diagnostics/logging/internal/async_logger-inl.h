...
REXLOG_INLINE rexlog::async_logger::async_logger(
    rsl::string logger_name, sinks_init_list sinks_list, rsl::weak_ptr<details::thread_pool> tp, async_overflow_policy overflow_policy)
    : async_logger(rsl::move(logger_name), sinks_list.begin(), sinks_list.end(), rsl::move(tp), overflow_policy)
{}

REXLOG_INLINE rexlog::async_logger::async_logger(
    rsl::string logger_name, sink_ptr single_sink, rsl::weak_ptr<details::thread_pool> tp, async_overflow_policy overflow_policy)
