...
struct hourly_filename_calculator
{
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        return filename_t(fmt_lib::format(REXLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}{}"), filename_with_extension.filename, now_tm.tm_year + 1900, now_tm.tm_mon + 1,
            now_tm.tm_mday, now_tm.tm_hour, filename_with_extension.ext));
    }
};
...
private:
    ...
    log_clock::time_point next_rotation_tp_()
    {
        ...
        return {rotation_time + rsl::chrono::hours(1)};
    }
    ...
    void delete_old_()
    {
        ...
        if (filenames_q_.full())
        {
            ...
            if (!ok)
            {
                ...
                REXLOG_THROW(rexlog_ex("Failed removing hourly file " + filename_to_str(old_filename), errno));
            }
        }
        filenames_q_.push_back(rsl::move(current_file));
    }
...
inline rsl::shared_ptr<logger> hourly_logger_mt(const rsl::string &logger_name, const filename_t &filename, bool truncate = false,
...
template<typename Factory = rexlog::synchronous_factory>
inline rsl::shared_ptr<logger> hourly_logger_st(const rsl::string &logger_name, const filename_t &filename, bool truncate = false,
    uint16_t max_files = 0, const file_event_handlers &event_handlers = {})
{
