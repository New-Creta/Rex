...
public:
    ...
    void connect(const rsl::string &host, int port)
    {
        ...
        auto port_str = rsl::to_string(port);
        struct addrinfo *addrinfo_result;
        auto rv = ::getaddrinfo(host.c_str(), port_str.c_str(), &hints, &addrinfo_result);
        if (rv != 0)
        {
            throw_rexlog_ex(fmt_lib::format("::getaddrinfo failed: {}", gai_strerror(rv)));
        }
        ...
        if (socket_ == -1)
        {
            throw_rexlog_ex("::connect failed", last_errno);
        }
...
#    error "tcp_sink would raise SIGPIPE since neither SO_NOSIGPIPE nor MSG_NOSIGNAL are available"
    ...
    void send(const char *data, size_t n_bytes)
    {
        ...
        while (bytes_sent < n_bytes)
        {
            ...
            {
                close();
                throw_rexlog_ex("write(2) failed", errno);
            }
