...
struct win32_error : public rexlog_ex
{
    ...
    static rsl::string format(rsl::string const &user_message, DWORD error_code = GetLastError())
    {
    ...
    explicit win32_error(rsl::string const &func_name, DWORD error = GetLastError())
...
public:
    win_eventlog_sink(rsl::string const &source, DWORD event_id = 1000 /* according to mscoree.dll */)
        : source_(source)
        , event_id_(event_id)
    {
        try
