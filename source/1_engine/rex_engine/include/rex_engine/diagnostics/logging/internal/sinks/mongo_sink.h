...
public:
    ...
    mongo_sink(rsl::shared_ptr<mongocxx::instance> instance, const rsl::string &db_name, const rsl::string &collection_name,
        const rsl::string &uri = "mongodb://localhost:27017")
    ...
    void sink_it_(const details::log_msg &msg) override
    {
        ...
        if (client_ != nullptr)
        {
            auto doc = document{} << "timestamp" << bsoncxx::types::b_date(msg.time) << "level" << level::to_string_view(msg.level).data()
                                  << "level_num" << msg.level << "message" << rsl::string(msg.payload.begin(), msg.payload.end())
                                  << "logger_name" << rsl::string(msg.logger_name.begin(), msg.logger_name.end()) << "thread_id"
...
inline rsl::shared_ptr<logger> mongo_logger_mt(const rsl::string &logger_name, const rsl::string &db_name,
    const rsl::string &collection_name, const rsl::string &uri = "mongodb://localhost:27017")
{
    return Factory::template create<sinks::mongo_sink_mt>(logger_name, db_name, collection_name, uri);
}

template<typename Factory = rexlog::synchronous_factory>
inline rsl::shared_ptr<logger> mongo_logger_st(const rsl::string &logger_name, const rsl::string &db_name,
    const rsl::string &collection_name, const rsl::string &uri = "mongodb://localhost:27017")
{
