#pragma once
	...
	namespace rex
	{
		...
		*/
		#define TB_GET_LOG_VERBOSITY(CategoryName)																			\
				CategoryName.GetVerbosity()

		/**
		 * Set the verbosity level of a specific category
		 */
		#define TB_SET_LOG_VERBOSITY(CategoryName, Verbosity)																\
				CategoryName.SetVerbosity(rex::##Verbosity);
		...
		*/
		#define DECLARE_LOG_CATEGORY_EXTERN(CategoryName, DefaultVerbosity)													\
				...
				{																											\
					inline LogCategory##CategoryName() : LogCategory<(int)rex::DefaultVerbosity>(#CategoryName) {}	\
				} CategoryName;
