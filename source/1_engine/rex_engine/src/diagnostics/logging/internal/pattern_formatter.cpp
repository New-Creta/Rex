...
namespace rexlog
{
    ...
    {
        rsl::memset(&m_cached_tm, 0, sizeof(m_cached_tm));
        m_formatters.push_back(rsl::make_unique<details::full_formatter>(details::PaddingInfo{}));
    }
    ...
    void PatternFormatter::handle_flag_impl(char flag, details::PaddingInfo padding)
    {
        ...
        case('+'): // default formatter
            m_formatters.push_back(rsl::make_unique<details::full_formatter>(padding));
            m_need_localtime = true;
            break;

        case 'n': // logger name
            m_formatters.push_back(rsl::make_unique<details::name_formatter<Padder>>(padding));
            break;

        case 'l': // level
            m_formatters.push_back(rsl::make_unique<details::level_formatter<Padder>>(padding));
            break;

        case 'L': // short level
            m_formatters.push_back(rsl::make_unique<details::short_level_formatter<Padder>>(padding));
            break;
        ...
        case('P'): // pid
            m_formatters.push_back(rsl::make_unique<details::pid_formatter<Padder>>(padding));
            break;

        case('^'): // color range start
            m_formatters.push_back(rsl::make_unique<details::color_start_formatter>(padding));
            break;

        case('$'): // color range end
            m_formatters.push_back(rsl::make_unique<details::color_stop_formatter>(padding));
            break;

        case('@'): // source location (filename:filenumber)
            m_formatters.push_back(rsl::make_unique<details::source_location_formatter<Padder>>(padding));
            break;

        case('s'): // short source filename - without directory name
            m_formatters.push_back(rsl::make_unique<details::short_filename_formatter<Padder>>(padding));
            break;

        case('g'): // full source filename
            m_formatters.push_back(rsl::make_unique<details::source_filename_formatter<Padder>>(padding));
            break;

        case('#'): // source line number
            m_formatters.push_back(rsl::make_unique<details::source_linenum_formatter<Padder>>(padding));
            break;

        case('!'): // source funcname
            m_formatters.push_back(rsl::make_unique<details::source_funcname_formatter<Padder>>(padding));
            break;

        case('%'): // % char
            m_formatters.push_back(rsl::make_unique<details::ch_formatter>('%'));
            break;

        case('u'): // elapsed time since last log message in nanos
            m_formatters.push_back(rsl::make_unique<details::elapsed_formatter<Padder, rsl::chrono::nanoseconds>>(padding));
            break;

        case('i'): // elapsed time since last log message in micros
            m_formatters.push_back(rsl::make_unique<details::elapsed_formatter<Padder, rsl::chrono::microseconds>>(padding));
            break;

        case('o'): // elapsed time since last log message in millis
            m_formatters.push_back(rsl::make_unique<details::elapsed_formatter<Padder, rsl::chrono::milliseconds>>(padding));
            break;

        case('O'): // elapsed time since last log message in seconds
            m_formatters.push_back(rsl::make_unique<details::elapsed_formatter<Padder, rsl::chrono::seconds>>(padding));
            break;

        default: // Unknown flag appears as is
            auto unknown_flag = rsl::make_unique<details::aggregate_formatter>();
    ...
    void PatternFormatter::compile_pattern_impl(rsl::string_view pattern)
    {
        ...
        for (auto it = pattern.begin(); it != end; ++it)
        {
            ...
            else // chars not following the % sign should be displayed as is
            {
                if (!user_chars)
                {
                    user_chars = rsl::make_unique<details::aggregate_formatter>();
                }
