...
namespace rexlog
{
  namespace details
  {
        ...
        : m_formatter(new PatternFormatter())
    {
     ...
     void Registry::register_logger(rsl::shared_ptr<Logger> newLogger)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
      register_logger_impl(rsl::move(newLogger));
    }

     void Registry::initialize_logger(rsl::shared_ptr<Logger> newLogger)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     rsl::shared_ptr<Logger> Registry::get(const rex::DebugString& loggerName)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     rsl::shared_ptr<Logger> Registry::default_logger()
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
    ...
    // default Logger is stored in default_logger_ (for faster retrieval) and in the loggers_ map.
     void Registry::set_default_logger(rsl::shared_ptr<Logger> newDefaultLogger)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     void Registry::set_tp(rsl::shared_ptr<ThreadPool> tp)
    {
     ...
     rsl::shared_ptr<ThreadPool> Registry::get_tp()
    {
      const rsl::unique_lock<rsl::recursive_mutex> lock(m_tp_mutex);
      return m_tp;
    }
    ...
    // Set global formatter. Each sink in each Logger will get a clone of this object
     void Registry::set_formatter(rsl::unique_ptr<formatter> formatter)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     void Registry::enable_backtrace(size_t nMessages)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     void Registry::set_error_handler(err_handler handler)
    {
     ...
     void Registry::apply_all(const rsl::function<void(const rsl::shared_ptr<Logger>)>& fun)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
      for(auto& l: m_loggers)
      {
     ...
     void Registry::flush_all()
    {
     ...
     void Registry::drop(const rex::DebugString& loggerName)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     rsl::recursive_mutex& Registry::tp_mutex()
    {
     ...
     void Registry::set_automatic_registration(bool automaticRegistration)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
      m_automatic_registration = automaticRegistration;
    }

     void Registry::set_levels(log_levels levels, const level::LevelEnum* globalLevel)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
     ...
     Registry& Registry::instance()
    {
     ...
     void Registry::apply_logger_env_levels(rsl::shared_ptr<Logger> newLogger)
    {
      const rsl::unique_lock<rsl::mutex> lock(m_logger_map_mutex);
      auto it        = m_log_levels.find(newLogger->name());
      auto new_level = it != m_log_levels.end() ? it->value : m_global_log_level;
     ...
     void Registry::throw_if_exists_impl(const rex::DebugString& loggerName)
    {
     ...
     void Registry::register_logger_impl(rsl::shared_ptr<Logger> newLogger)
    {
