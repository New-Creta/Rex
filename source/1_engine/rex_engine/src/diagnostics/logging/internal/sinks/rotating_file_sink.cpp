...
namespace rexlog
{
  namespace sinks
  {
    ...
    template <typename Mutex>
     RotatingFileSink<Mutex>::RotatingFileSink(filename_t baseFileName, rsl::size_t maxSize, rsl::size_t maxFiles, bool rotateOnOpen, const FileEventHandlers& eventHandlers)
    ...
    template <typename Mutex>
     filename_t RotatingFileSink<Mutex>::calc_filename(const filename_t& filename, rsl::size_t index)
    {
      if(index == 0u)
      {
    ...
    template <typename Mutex>
     filename_t RotatingFileSink<Mutex>::filename()
    {
    ...
    template <typename Mutex>
     void RotatingFileSink<Mutex>::sink_it_impl(const details::LogMsg& msg)
    {
    ...
    template <typename Mutex>
     void RotatingFileSink<Mutex>::flush_impl()
    {
      m_file_helper.flush();
    }
    ...
    // log.3.txt -> delete
    template <typename Mutex>
     void RotatingFileSink<Mutex>::rotate_impl()
    {
    ...
    template <typename Mutex>
     bool RotatingFileSink<Mutex>::rename_file_impl(const filename_t& srcFilename, const filename_t& targetFilename)
    {
