...
namespace rexlog
{
  namespace sinks
  {
    ...
    template <typename ConsoleMutex>
     void ansicolor_sink<ConsoleMutex>::set_pattern(const rex::DebugString& pattern)
    {
      rsl::lock_guard<mutex_t> lock(m_mutex);
    ...
    template <typename ConsoleMutex>
     void ansicolor_sink<ConsoleMutex>::set_formatter(rsl::unique_ptr<rexlog::formatter> sink_formatter)
    {
      rsl::lock_guard<mutex_t> lock(m_mutex);
