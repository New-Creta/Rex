  ...
  namespace gfx
  {
    namespace mesh_factory
    {
      StaticMesh create_grid(s16 cellSize, s32 cellsPerLine, f32 lineWidth)
      {
        ...
        s32 width = cellSize * cellsPerLine;
        s32 depth = cellSize * cellsPerLine;
        ...
        {
          u16 start_vtx = narrow_cast<u16>(vertices.size());
          f32 uv_coordinate = static_cast<f32>(row) / static_cast<f32>(width);
        ...
        {
          u16 start_vtx = narrow_cast<u16>(vertices.size());
          f32 uv_coordinate = static_cast<f32>(column) / static_cast<f32>(width);
        ...
        rsl::unique_ptr<VertexBuffer> vb = gal()->create_vertex_buffer(vertices.size(), sizeof(vertices.front()), vertices.data());
