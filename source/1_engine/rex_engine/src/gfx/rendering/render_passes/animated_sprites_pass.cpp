...
namespace rex
{
	namespace gfx
	{
		AnimatedSpritesPass::AnimatedSpritesPass(const AnimatedSpritesPassDynamicInputs& inputs)
		...
		RenderPassDesc AnimatedSpritesPass::create_desc() const
		{
			...
			desc.pso_desc.shader_pipeline.vs = rex::gfx::shader_lib::instance()->load(rex::path::join(project_shaders, "render_tile_vertex.hlsl"), rex::gfx::ShaderType::Vertex);
			desc.pso_desc.shader_pipeline.ps = rex::gfx::shader_lib::instance()->load(rex::path::join(project_shaders, "render_tile_pixel.hlsl"), rex::gfx::ShaderType::Pixel);
