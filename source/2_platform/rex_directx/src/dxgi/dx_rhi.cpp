...
namespace rex
{
  namespace rhi
  {
    ...
    ResourceSlot compile_shader(const CompileShaderDesc& desc)
    {
      ...
      {
        case ShaderType::VERTEX: return internal::get()->resource_pool.insert(rsl::make_unique<VertexShaderResource>(hash, byte_code)); break;
        case ShaderType::PIXEL: return internal::get()->resource_pool.insert(rsl::make_unique<PixelShaderResource>(hash, byte_code)); break;

        default: REX_ERROR(LogDirectX, "Unsupported Shader Type was given");
      }
...
#pragma region RHI Class
    ...
    bool internal::RenderHardwareInfrastructure::init_descriptor_heap(D3D12_DESCRIPTOR_HEAP_TYPE type, s32 numDescriptors)
    {
      ...
      desc.Flags          = type != D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV ? D3D12_DESCRIPTOR_HEAP_FLAG_NONE : D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
