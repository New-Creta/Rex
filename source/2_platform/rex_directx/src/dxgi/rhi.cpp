...
namespace rex
{
  namespace rhi
  {
    ...
    namespace internal
    {
      ...
      private:
        ...
        bool init_command_objects()
        {
          ...
          if (DX_FAILED(m_device->get()->CreateCommandQueue(&queue_desc, IID_PPV_ARGS(m_command_queue.GetAddressOf()))))
          {
            REX_ERROR(LogRhi, "Failed to create command queue");
            return false;
          }

          rhi::set_debug_name_for(m_command_queue.Get(), "Global Command Queue");

          // Command Allocator
          if (DX_FAILED(m_device->get()->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(m_command_allocator.GetAddressOf()))))
          {
            REX_ERROR(LogRhi, "Failed to create command allocator");
            return false;
          }

          rhi::set_debug_name_for(m_command_allocator.Get(), "Global Command Allocator");

          // Command List
          if (DX_FAILED(m_device->get()->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_command_allocator.Get(), nullptr, IID_PPV_ARGS(m_command_list.GetAddressOf()))))
          {
            REX_ERROR(LogRhi, "Failed to create command list");
            return false;
          }

          rhi::set_debug_name_for(m_command_list.Get(), "Global Command List");

          // Start off in a closed state. This is because the first time we
          // refer to the command list we will Reset it, and it needs to be closed
          // before calling Reset.
          if (DX_FAILED(m_command_list->Close()))
          {
        ...
        {
          if (DX_FAILED(m_device->get()->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_fence))))
          {
            REX_ERROR(LogRhi, "Failed to create DX fence, to synchronize CPU/GPU");
            return false;
          }

          rhi::set_debug_name_for(m_fence.Get(), "Global Fence");
          return true;
        }
    ...
    {
      internal::g_rhi = rsl::make_unique<internal::RenderHarderwareInfrastructure>(userData);
      return internal::g_rhi->init_successful();
    }
