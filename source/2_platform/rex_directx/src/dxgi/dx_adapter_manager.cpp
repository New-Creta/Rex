...
{
  //-------------------------------------------------------------------------
  rsl::function<HRESULT(UINT, rex::wrl::ComPtr<IDXGIAdapter4>*)> get_enumaration_function6(rex::gfx::dxgi::Factory* factory)
  {
  ...
  //-------------------------------------------------------------------------
  rsl::function<HRESULT(UINT, rex::wrl::ComPtr<IDXGIAdapter1>*)> get_enumaration_function1(rex::gfx::dxgi::Factory* factory)
  {
  ...
  //-------------------------------------------------------------------------
  rsl::function<HRESULT(UINT, rex::wrl::ComPtr<IDXGIAdapter>*)> get_enumaration_function(rex::gfx::dxgi::Factory* factory)
  {
    REX_ASSERT_X(factory, "DXGIFactory does not exist");

    return [factory = factory->dx_object()](UINT index, rex::wrl::ComPtr<IDXGIAdapter>* adapter) { return factory->EnumAdapters(index, (*adapter).GetAddressOf()); };
  }

  //-------------------------------------------------------------------------
  template <typename DXGIAdapterInterface>
  rsl::vector<rex::gfx::dxgi::Adapter> get_adapters(const rsl::function<HRESULT(UINT, rex::wrl::ComPtr<DXGIAdapterInterface>*)>& enumarationFnc, uint32 version)
  {
    uint32 i                                       = 0;
    rex::wrl::ComPtr<DXGIAdapterInterface> adapter = nullptr;

    rsl::vector<rex::gfx::dxgi::Adapter> adapters;
