...
{
    template <class T>
    struct hash<rex::not_null<T>>
    {
        std::size_t operator()(const rex::not_null<T>& value) const { return hash<T>{}(value.get()); }
    };
...
{
    template <class T>
    struct hash<rex::strict_not_null<T>>
    {
        std::size_t operator()(const rex::strict_not_null<T>& value) const
        {
