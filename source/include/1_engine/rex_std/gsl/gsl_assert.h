...
//
#if defined(__clang__)
#define GSL_SUPPRESS(x) [[rex::suppress("x")]]
#else
...
namespace details
{
    ...
    inline rex::details::terminate_handler& get_terminate_handler() noexcept
    {
    ...
    {
#if defined(GSL_MSVC_USE_STL_NOEXCEPTION_WORKAROUND)
        (*rex::details::get_terminate_handler())();
#else
        std::terminate();
#endif // defined(GSL_MSVC_USE_STL_NOEXCEPTION_WORKAROUND)
    }
...
#define GSL_CONTRACT_CHECK(type, cond)                                                             \
    (GSL_LIKELY(cond) ? static_cast<void>(0) : rex::details::terminate())
