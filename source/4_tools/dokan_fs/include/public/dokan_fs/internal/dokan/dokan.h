...
#define DOKAN_VERSION_ERROR -7
   ...
   *
   * \param DokanInstance The dokan mount context created by \ref DokanCreateFileSystem .
   * \param WaitHandle Handle to wait handle registration. This handle needs to be closed by calling <a href="https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-unregisterwait">UnregisterWait</a>, <a
   * href="https://learn.microsoft.com/en-us/windows/win32/sync/unregisterwait">UnregisterWaitEx</a> or \ref DokanUnregisterWaitForFileSystemClosed to unregister the callback . \param Callback Callback function to be called from a worker thread when
   * file system is unmounted . \param Context A context parameter to send to callback function . \param dwMilliseconds The time-out interval, in milliseconds. See <a
   * href="https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-registerwaitforsingleobject">RegisterWaitForSingleObject</a>. \return TRUE if successful, FALSE otherwise.
   */
   ...
   */
  BOOL DOKANAPI DokanIsNameInExpression(LPCWSTR Expression, LPCWSTR Name, BOOL IgnoreCase);
