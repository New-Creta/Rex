...
#define DDbgPrint(Format, ...) KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_TRACE_LEVEL, "[dokan2]" Format "\n", __VA_ARGS__));
...
#define DOKAN_PUSH_LOG(RequestContext, Format, ...)                                                                                                                                                                                                      \
  {                                                                                                                                                                                                                                                      \
    ...
    PushDokanLogEntry(RequestContext, "[%02d:%02d:%02d.%03d]" Format, TimeFields.Hour, TimeFields.Minute, TimeFields.Second, TimeFields.Milliseconds, __VA_ARGS__);                                                                                      \
  }
...
#define DOKAN_LOG_FINE_IRP(RequestContext, Format, ...) DOKAN_LOG_INTERNAL(RequestContext, "[%p]: " Format, (RequestContext ? RequestContext->Irp : 0), __VA_ARGS__)
...
// flood
#define DOKAN_DENIED_LOG_EVENT(IrpSp) (IrpSp->MajorFunction == IRP_MJ_FILE_SYSTEM_CONTROL && IrpSp->MinorFunction == IRP_MN_USER_FS_REQUEST && IrpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_EVENT_PROCESS_N_PULL)
...
#define DOKAN_LOG_IOCTL(RequestContext, ControlCode, format, ...)                                                                                                                                                                                        \
  ...
  {                                                                                                                                                                                                                                                      \
    ...
    {                                                                                                                                                                                                                                                    \
      DOKAN_LOG_INTERNAL(RequestContext, "[%p][%s]: " format, RequestContext->Irp, DokanGetIoctlStr(ControlCode), __VA_ARGS__)                                                                                                                           \
    }                                                                                                                                                                                                                                                    \
  } while(0)

// Log the whole Irp informations.
#define DOKAN_LOG_MJ_IRP(RequestContext, Format, ...)                                                                                                                                                                                                    \
  DOKAN_LOG_INTERNAL(RequestContext, "[%p][%s][%s][%s]: " Format, RequestContext->Irp, DokanGetIdTypeStr(RequestContext->DeviceObject->DeviceExtension), DokanGetMajorFunctionStr(RequestContext->IrpSp->MajorFunction),                                 \
                     DokanGetMinorFunctionStr(RequestContext->IrpSp->MajorFunction, RequestContext->IrpSp->MinorFunction), __VA_ARGS__)

// Log the Irp at dispatch time.
#define DOKAN_LOG_BEGIN_MJ(RequestContext) DOKAN_LOG_MJ_IRP(RequestContext, "Begin ProcessId=%lu", RequestContext->ProcessId)
...
#define DOKAN_LOG_END_MJ(RequestContext, Status)                                                                                                                                                                                                         \
  ...
  {                                                                                                                                                                                                                                                      \
    ...
    {                                                                                                                                                                                                                                                    \
      DOKAN_LOG_FINE_IRP(RequestContext, "End - Irp not completed %s", DokanGetNTSTATUSStr(Status));                                                                                                                                                     \
    }                                                                                                                                                                                                                                                    \
    ...
    {                                                                                                                                                                                                                                                    \
      DOKAN_LOG_FINE_IRP(RequestContext, "End - %s Information=%llx", DokanGetNTSTATUSStr(Status), RequestContext->Irp->IoStatus.Information);                                                                                                           \
