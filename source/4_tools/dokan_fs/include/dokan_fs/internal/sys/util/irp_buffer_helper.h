...
#include "dokan_fs/internal/sys/public.h"
...
#define GET_IRP_GENERIC_BUFFER(Irp, Buffer, CompareSize) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, CompareSize, DOKAN_EXIT_NONE, 0, 0)
...
#define MOUNTDEV_NAME_SIZE_COMPARE(MountDevName, BufferLen) (GENERIC_SIZE_COMPARE(MountDevName, BufferLen) || (ULONG)(FIELD_OFFSET(MOUNTDEV_NAME, Name[0]) + (MountDevName)->NameLength) > (BufferLen))

#define DOKAN_NOTIFY_PATH_INTERMEDIATE_SIZE_COMPARE(DokanNotifyPath, BufferLen) (GENERIC_SIZE_COMPARE(DokanNotifyPath, BufferLen) || (ULONG)(FIELD_OFFSET(DOKAN_NOTIFY_PATH_INTERMEDIATE, Buffer[0]) + (DokanNotifyPath)->Length) > (BufferLen))

#define DOKAN_UNICODE_STRING_INTERMEDIATE_SIZE_COMPARE(DokanUnicodeString, BufferLen)                                                                                                                                                                    \
  (GENERIC_SIZE_COMPARE(DokanUnicodeString, BufferLen) || (ULONG)(FIELD_OFFSET(DOKAN_UNICODE_STRING_INTERMEDIATE, Buffer[0]) + (DokanUnicodeString)->Length) > (BufferLen) ||                                                                            \
   (ULONG)(FIELD_OFFSET(DOKAN_UNICODE_STRING_INTERMEDIATE, Buffer[0]) + (DokanUnicodeString)->MaximumLength) > (BufferLen) || (DokanUnicodeString)->Length > (DokanUnicodeString)->MaximumLength)
...
#define GET_IRP_BUFFER(Irp, Buffer) GET_IRP_GENERIC_BUFFER(Irp, Buffer, GENERIC_SIZE_COMPARE)

#define GET_IRP_BUFFER_OR_LEAVE(Irp, Buffer) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, GENERIC_SIZE_COMPARE, DOKAN_EXIT_LEAVE, STATUS_BUFFER_TOO_SMALL, 0)

#define GET_IRP_BUFFER_OR_BREAK(Irp, Buffer) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, GENERIC_SIZE_COMPARE, DOKAN_EXIT_BREAK, STATUS_BUFFER_TOO_SMALL, 0)

#define GET_IRP_BUFFER_OR_RETURN(Irp, Buffer) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, GENERIC_SIZE_COMPARE, DOKAN_EXIT_RETURN, STATUS_BUFFER_TOO_SMALL, 0)

// Variations of Get DeviceIOControl for input buffer with a size check.
// DOKAN_NOTIFY_PATH_INTERMEDIATE
#define GET_IRP_NOTIFY_PATH_INTERMEDIATE_OR_RETURN(Irp, Buffer) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, DOKAN_NOTIFY_PATH_INTERMEDIATE_SIZE_COMPARE, DOKAN_EXIT_RETURN, STATUS_BUFFER_TOO_SMALL, 0)

// DOKAN_UNICODE_STRING_INTERMEDIATE
#define GET_IRP_UNICODE_STRING_INTERMEDIATE_OR_RETURN(Irp, Buffer) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, DOKAN_UNICODE_STRING_INTERMEDIATE_SIZE_COMPARE, DOKAN_EXIT_RETURN, STATUS_BUFFER_TOO_SMALL, 0)

// MOUNTDEV_NAME
#define GET_IRP_MOUNTDEV_NAME_OR_BREAK(Irp, Buffer) GET_IRP_GENERIC_BUFFER_EX(Irp, Buffer, MOUNTDEV_NAME_SIZE_COMPARE, DOKAN_EXIT_BREAK, STATUS_BUFFER_TOO_SMALL, 0)
...
PVOID PrepareOutputWithSize(_Inout_ PIRP Irp, _In_ ULONG Size, _In_ BOOLEAN SetInformationOnFailure);

// Helper that supports the PREPARE_OUTPUT macro without causing an
// assignment-in-conditional at call sites.
inline BOOLEAN PrepareOutputHelper(_Inout_ PIRP Irp, _Out_ VOID** Buffer, _In_ ULONG Size, _In_ BOOLEAN SetInformationOnFailure)
{
...
#define PREPARE_OUTPUT(Irp, Buffer, SetInformationOnFailure) PrepareOutputHelper((Irp), &(Buffer), sizeof(*Buffer), (SetInformationOnFailure))
...
BOOLEAN ExtendOutputBufferBySize(_Inout_ PIRP Irp, _In_ ULONG AdditionalSize, _In_ BOOLEAN UpdateInformationOnFailure);
...
BOOLEAN AppendVarSizeOutputString(_Inout_ PIRP Irp, _Inout_ PVOID Dest, const _In_ UNICODE_STRING* Str, _In_ BOOLEAN UpdateInformationOnFailure, _In_ BOOLEAN FillSpaceWithPartialString);
