...
#include "dokan_fs/internal/sys/util/fcb.h"
...
--*/
{
  ...
  ULONG eventLength;
  PDokanFCB fcb;
  DOKAN_INIT_LOGGER(logger, RequestContext->DeviceObject->DriverObject, IRP_MJ_CLOSE);

  fileObject = RequestContext->IrpSp->FileObject;
  DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p", fileObject);

  if(fileObject == NULL || RequestContext->Vcb == NULL || !fileObject->FsContext2)
  {
  ...
  {
    DokanLogInfo(&logger, L"Closed file with user mode dispatch blocked: \"%wZ\"", &fcb->FileName);
    DokanFreeCCB(RequestContext, ccb);
    DokanFCBUnlock(fcb);
    DokanFreeFCB(RequestContext->Vcb, fcb);
    return STATUS_SUCCESS;
