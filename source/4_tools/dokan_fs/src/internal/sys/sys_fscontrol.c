...
void DokanMaybeLogOplockRequest(__in PDOKAN_LOGGER Logger, __in PDokanFCB Fcb, __in ULONG FsControlCode, __in ULONG OplockCount, __in BOOLEAN AcquiredFcb, __in BOOLEAN AcquiredVcb, __in ULONG RequestedLevel, __in ULONG Flags)
{
  ...
  {
    DokanLogInfo(Logger,
                 L"Oplock request FSCTL_REQUEST_OPLOCK for file \"%wZ\";"
                 L" oplock count %d; acquired FCB %d; acquired VCB %d;"
                 L" level = %I32x; flags = %I32x",
                 &Fcb->FileName, OplockCount, AcquiredFcb, AcquiredVcb, RequestedLevel, Flags);
    return;
  }
  DokanLogInfo(Logger,
               L"Oplock request %s for file \"%wZ\"; oplock count %d;"
               L" acquired FCB %d; acquired VCB %d",
               DokanGetIoctlStr(FsControlCode), &Fcb->FileName, OplockCount, AcquiredFcb, AcquiredVcb);
}

void DokanMaybeLogOplockResult(__in PDOKAN_LOGGER Logger, __in PDokanFCB Fcb, __in ULONG FsControlCode, __in ULONG RequestedLevel, __in ULONG Flags, __in NTSTATUS Status)
{
  if(!DokanOpLockDebugEnabled())
  {
    return;
  ...
  {
    DokanLogInfo(Logger,
                 L"Oplock result for FSCTL_REQUEST_OPLOCK for file \"%wZ\";"
                 L" level = %I32x; flags = %I32x; status = 0x%I32x",
                 &Fcb->FileName, RequestedLevel, Flags, Status);
    return;
  }
  DokanLogInfo(Logger, L"Oplock result for %s for file \"%wZ\"; status = 0x%I32x", DokanGetIoctlStr(FsControlCode), &Fcb->FileName, Status);
}
...
NTSTATUS DokanOplockRequest(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  //
  fsControlCode = RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode;
  ...
  if(fsControlCode == FSCTL_REQUEST_OPLOCK)
  {
    outputBufferLength = RequestContext->IrpSp->Parameters.FileSystemControl.OutputBufferLength;
  ...
  //
  if((DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY)) && ((fsControlCode != FSCTL_REQUEST_OPLOCK) || !FsRtlOplockIsSharedRequest(RequestContext->Irp)))
  {
  ...
  try
  {
    ...
    //
    if((fsControlCode == FSCTL_REQUEST_OPLOCK_LEVEL_1) || (fsControlCode == FSCTL_REQUEST_BATCH_OPLOCK) || (fsControlCode == FSCTL_REQUEST_FILTER_OPLOCK) || (fsControlCode == FSCTL_REQUEST_OPLOCK_LEVEL_2) ||
        ...
        if(FsRtlOplockIsSharedRequest(RequestContext->Irp))
        {
          ...
          if(!DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY))
          {
            ...
            if(DokanFsRtlCheckLockForOplockRequest) // Win8+
              oplockCount = (ULONG)!DokanFsRtlCheckLockForOplockRequest(&fcb->FileLock, &fcb->AdvancedFCBHeader.AllocationSize);
            else
    ...
    else if((fsControlCode == FSCTL_OPLOCK_BREAK_ACKNOWLEDGE) || (fsControlCode == FSCTL_OPBATCH_ACK_CLOSE_PENDING) || (fsControlCode == FSCTL_OPLOCK_BREAK_NOTIFY) || (fsControlCode == FSCTL_OPLOCK_BREAK_ACK_NO_2) ||
    ...
    //  for delete.
    //
    if(((fsControlCode == FSCTL_REQUEST_FILTER_OPLOCK) || (fsControlCode == FSCTL_REQUEST_BATCH_OPLOCK) || ((fsControlCode == FSCTL_REQUEST_OPLOCK) && FlagOn(inputBuffer->RequestedOplockLevel, OPLOCK_LEVEL_CACHE_HANDLE))) &&
    ...
    DokanMaybeLogOplockRequest(&logger, fcb, fsControlCode, oplockCount, acquiredFcb, acquiredVcb, level, flags);

    //
    //  Call the FsRtl routine to grant/acknowledge oplock.
    //
    status = FsRtlOplockFsctrl(DokanGetFcbOplock(fcb), RequestContext->Irp, oplockCount);
    DokanMaybeLogOplockResult(&logger, fcb, fsControlCode, level, flags, status);
    //
...
DokanVolumeUserFsRequest(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  PDokanCCB ccb           = NULL;
  PDokanFCB fcb           = NULL;
  DOKAN_INIT_LOGGER(logger, RequestContext->DeviceObject->DriverObject, IRP_MJ_FILE_SYSTEM_CONTROL);
  ...
  switch(RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode)
  {
    case FSCTL_ACTIVATE_KEEPALIVE:
    {
      ...
      if(fileObject == NULL)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_ACTIVATE_KEEPALIVE with no FileObject.");
      }
      ccb = fileObject->FsContext2;
      if(ccb == NULL || ccb->Identifier.Type != CCB)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_ACTIVATE_KEEPALIVE with no CCB.");
      }

      fcb = ccb->Fcb;
      if(fcb == NULL || fcb->Identifier.Type != FCB)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_ACTIVATE_KEEPALIVE with no FCB.");
      }

      if(!fcb->IsKeepalive)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_ACTIVATE_KEEPALIVE for wrong file: \"%wZ\"", &fcb->FileName);
      }
      ...
      DokanLogInfo(&logger, L"Activating keepalive handle from process %lu.", RequestContext->ProcessId);
      DokanFCBLockRW(fcb);
    ...
    case FSCTL_NOTIFY_PATH:
    {
      ...
      if(fileObject == NULL)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_NOTIFY_PATH with no FileObject.");
      }
      ccb = fileObject->FsContext2;
      if(ccb == NULL || ccb->Identifier.Type != CCB)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_NOTIFY_PATH with no CCB.");
      }
      fcb = ccb->Fcb;
      if(fcb == NULL || fcb->Identifier.Type != FCB)
      {
        return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Received FSCTL_NOTIFY_PATH with no FCB.");
      }
      ...
      DokanFCBLockRO(fcb);
      NTSTATUS status = DokanNotifyReportChange0(RequestContext, fcb, &receivedBuffer, pNotifyPath->CompletionFilter, pNotifyPath->Action);
      DokanFCBUnlock(fcb);
    ...
    case FSCTL_REQUEST_OPLOCK: return DokanOplockRequest(RequestContext);

    case FSCTL_LOCK_VOLUME:
    case FSCTL_UNLOCK_VOLUME:
    case FSCTL_IS_VOLUME_MOUNTED: return STATUS_SUCCESS;

    case FSCTL_GET_REPARSE_POINT: return STATUS_NOT_A_REPARSE_POINT;
  }
  // TODO(someone): Find if there is a way to send FSCTL to Disk type for DokanRedirector
  if(RequestContext->Dcb && RequestContext->Dcb->VolumeDeviceType == FILE_DEVICE_NETWORK_FILE_SYSTEM)
  {
  ...
  DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported FsControlCode %x", RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode);
...
DokanGlobalUserFsRequest(__in PREQUEST_CONTEXT RequestContext)
{
  switch(RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode)
  {
    case FSCTL_EVENT_START: return DokanEventStart(RequestContext);
    ...
    case FSCTL_EVENT_RELEASE: return DokanGlobalEventRelease(RequestContext);

    case FSCTL_EVENT_MOUNTPOINT_LIST: return DokanGetMountPointList(RequestContext);
    ...
    case FSCTL_MOUNTPOINT_CLEANUP: RemoveSessionDevices(RequestContext, GetCurrentSessionId(RequestContext)); return STATUS_SUCCESS;
  }
  DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported FsControlCode %x", RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode);
  return STATUS_INVALID_DEVICE_REQUEST;
}

NTSTATUS PullEvents(__in PREQUEST_CONTEXT RequestContext, __in PIRP_LIST NotifyEvent)
{
  ...
  ULONG currentIoctlBufferBytesRemaining = RequestContext->IrpSp->Parameters.DeviceIoControl.OutputBufferLength;
  PCHAR currentIoctlBuffer               = (PCHAR)RequestContext->Irp->AssociatedIrp.SystemBuffer;
  ...
  while(!IsListEmpty(&NotifyEvent->ListHead))
  {
    ...
    workItem          = CONTAINING_RECORD(workItemListEntry, DRIVER_EVENT_CONTEXT, ListEntry);
  ...
  if(!IsListEmpty(&NotifyEvent->ListHead) && !KeReadStateQueue(&RequestContext->Dcb->NotifyIrpEventQueue))
  {
    KeInsertQueue(&RequestContext->Dcb->NotifyIrpEventQueue, &RequestContext->Dcb->NotifyIrpEventQueueList);
  }
...
NTSTATUS DokanProcessAndPullEvents(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  if(RequestContext->IrpSp->Parameters.DeviceIoControl.OutputBufferLength < sizeof(EVENT_CONTEXT))
  {
  ...
  PEVENT_INFORMATION eventInfo = (PEVENT_INFORMATION)(RequestContext->Irp->AssociatedIrp.SystemBuffer);
  ULONG waitTimeoutMs          = status == STATUS_BUFFER_TOO_SMALL ? 0 : eventInfo->PullEventTimeoutMs;
  ...
  KeRemoveQueueEx(&RequestContext->Dcb->NotifyIrpEventQueue, KernelMode, TRUE, waitTimeoutMs ? &timeout : NULL, &listEntry, 1);
...
DokanDiskUserFsRequest(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  switch(RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode)
  {
    case FSCTL_EVENT_PROCESS_N_PULL: return DokanProcessAndPullEvents(&requestContext);
    case FSCTL_EVENT_RELEASE: return DokanEventRelease(&requestContext, requestContext.Vcb->DeviceObject);
    case FSCTL_EVENT_WRITE: return DokanEventWrite(&requestContext);
    case FSCTL_GET_VOLUME_METRICS: return DokanGetVolumeMetrics(&requestContext);
    case FSCTL_RESET_TIMEOUT: return DokanResetPendingIrpTimeout(&requestContext);
    case FSCTL_GET_ACCESS_TOKEN: return DokanGetAccessToken(&requestContext);
  }
  DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported FsControlCode %x", RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode);
  return STATUS_INVALID_DEVICE_REQUEST;
}
...
{
  DOKAN_LOG_IOCTL(RequestContext, RequestContext->IrpSp->Parameters.FileSystemControl.FsControlCode, "FileObject=%p", RequestContext->IrpSp->FileObject);
...
BOOLEAN MatchDokanDCBType(__in PREQUEST_CONTEXT RequestContext, __in PDokanDCB Dcb, __in PDOKAN_LOGGER Logger, __in BOOLEAN LogFailures)
{
  ...
  if(GetIdentifierType(Dcb) != DCB)
  {
    if(LogFailures)
    {
      DOKAN_LOG_FINE_IRP(RequestContext, "The DCB type is actually %s expected %s.", DokanGetIdTypeStr(Dcb), STR(DCB));
    }
    return FALSE;
  }
  return TRUE;
...
PCHAR CreateSetReparsePointRequest(PREQUEST_CONTEXT RequestContext, PUNICODE_STRING SymbolicLinkName, PULONG Length)
{
  UNREFERENCED_PARAMETER(RequestContext);
...
PCHAR CreateRemoveReparsePointRequest(PREQUEST_CONTEXT RequestContext, PULONG Length)
{
  UNREFERENCED_PARAMETER(RequestContext);
  *Length                          = REPARSE_GUID_DATA_BUFFER_HEADER_SIZE;
  PREPARSE_DATA_BUFFER reparseData = DokanAllocZero(sizeof(REPARSE_DATA_BUFFER));
  if(!reparseData)
  {
...
NTSTATUS SendDirectoryFsctl(PREQUEST_CONTEXT RequestContext, PUNICODE_STRING Path, ULONG Code, PCHAR Input, ULONG Length)
{
  UNREFERENCED_PARAMETER(RequestContext);
  HANDLE handle                = 0;
  ...
  __try
  {
    ...
    directoryStr = ChangePrefix(Path, &g_DosDevicesPrefix, TRUE /*HasPrefix*/, &g_ObjectManagerPrefix);
    if(!directoryStr)
    {
      status = STATUS_INVALID_PARAMETER;
    ...
    status = ZwOpenFile(&handle, FILE_WRITE_ATTRIBUTES, &objectAttributes, &ioStatusBlock, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN_REPARSE_POINT | FILE_OPEN_FOR_BACKUP_INTENT);
    if(!NT_SUCCESS(status))
    {
      DokanLogError(&logger, status, L"SendDirectoryFsctl - ZwOpenFile failed to open\"%wZ\"\n", directoryStr);
      __leave;
    }

    status = ZwFsControlFile(handle, NULL, NULL, NULL, &ioStatusBlock, Code, Input, Length, NULL, 0);
    if(!NT_SUCCESS(status))
    {
      DokanLogError(&logger, status, L"SendDirectoryFsctl - ZwFsControlFile Code %X on \"%wZ\" failed\n", Code, directoryStr);
      __leave;
    }
  }
  __finally
...
NTSTATUS DokanMountVolume(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  PDEVICE_OBJECT deviceObject      = RequestContext->IrpSp->Parameters.MountVolume.DeviceObject;
  dcb                              = deviceObject->DeviceExtension;
  PDEVICE_OBJECT lowerDeviceObject = NULL;
  while(!MatchDokanDCBType(RequestContext, dcb, &logger,
                           /*LogFailures=*/!hasMountedAnyDisk))
  {
    PDEVICE_OBJECT parentDeviceObject = lowerDeviceObject ? lowerDeviceObject : deviceObject;
    ...
    if(!lowerDeviceObject)
    {
      if(!hasMountedAnyDisk)
      {
        ...
        DOKAN_LOG_FINE_IRP(RequestContext, "Not mounting because there is no matching DCB. This is"
    ...
    if(!hasMountedAnyDisk)
    {
      DOKAN_LOG_FINE_IRP(RequestContext, "Processing the lower level DeviceObject, in case"
  ...
  {
    return DokanLogError(&logger, STATUS_UNRECOGNIZED_VOLUME, L"The driver version of the disk does not match.");
  }

  if(IsDeletePending(dcb->DeviceObject))
  {
    return DokanLogError(&logger, STATUS_DEVICE_REMOVED, L"This is a remount try of the device.");
  }

  BOOLEAN isNetworkFileSystem = (dcb->VolumeDeviceType == FILE_DEVICE_NETWORK_FILE_SYSTEM);

  DokanLogInfo(&logger, L"Mounting volume using MountPoint \"%wZ\" device \"%wZ\"", dcb->MountPoint, dcb->DiskDeviceName);
  ...
  if(vcb->Dcb->FcbGarbageCollectionIntervalMs != 0)
  {
    ...
    KeInitializeEvent(&vcb->FcbGarbageListNotEmpty, SynchronizationEvent, FALSE);
    DokanStartFcbGarbageCollector(vcb);
  }

  RtlInitializeGenericTableAvl(&vcb->FcbTable, DokanCompareFcb, DokanAllocateFcbAvl, DokanFreeFcbAvl, vcb);
  ...
  {
    RtlCopyMemory(dokanControl.UNCName, dcb->UNCName->Buffer, dcb->UNCName->Length);
  }
  ...
  else
  {
    ...
    return DokanLogError(&logger, STATUS_DEVICE_REMOVED, L"MountEntry not found.");
  }
  ...
  if(dcb->UseMountManager)
  {
    ...
    if(!NT_SUCCESS(status))
    {
      DokanLogError(&logger, status, L"DokanSendVolumeArrivalNotification failed.");
    }
    if(!isDriveLetter)
    {
...
DokanDispatchFileSystemControl(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  {
    case IRP_MN_MOUNT_VOLUME: return DokanMountVolume(RequestContext);
    case IRP_MN_USER_FS_REQUEST: return DokanUserFsRequest(RequestContext);
  }
  DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported MinorFunction %x", RequestContext->IrpSp->MinorFunction);
  return STATUS_INVALID_DEVICE_REQUEST;
}
