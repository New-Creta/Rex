...
#define DOKAN_LOG_MAX_PACKET_BYTES         (ERROR_LOG_MAXIMUM_SIZE - sizeof(IO_ERROR_LOG_PACKET))
#define DOKAN_LOG_MAX_PACKET_NONNULL_CHARS (DOKAN_LOG_MAX_PACKET_BYTES / sizeof(WCHAR) - 1)

static VOID DokanPrintToSysLog(__in PDRIVER_OBJECT DriverObject, __in UCHAR MajorFunctionCode, __in NTSTATUS MessageId, __in NTSTATUS Status, __in LPCTSTR Format, __in va_list Args)
{
  ...
  __try
  {
    ...
    if(message == NULL)
    {
      DOKAN_LOG_("Failed to allocate message of capacity %d", messageCapacity);
      __leave;
    }
    ...
    if(status == STATUS_BUFFER_OVERFLOW)
    {
      ...
      DOKAN_LOG_("Log message was larger than DOKAN_LOG_MAX_CHAR_COUNT."
                 " Format: %S",
                 Format);
    }
    else if(status != STATUS_SUCCESS)
    {
      DOKAN_LOG_("Failed to generate log message with format: %S; status: %x", Format, status);
      __leave;
    }
    ...
    {
      packetCharCount = (UCHAR)min(messageCharCount - messageCharsWritten, DOKAN_LOG_MAX_PACKET_NONNULL_CHARS);
      packetSize      = sizeof(IO_ERROR_LOG_PACKET) + sizeof(WCHAR) * (packetCharCount + 1);
      ...
      packet->StringOffset      = (USHORT)((char*)&packet->DumpData[0] - (char*)packet);
      packet->ErrorCode         = MessageId;
      packet->FinalStatus       = Status;
      RtlCopyMemory(&packet->DumpData[0], message + messageCharsWritten, sizeof(WCHAR) * packetCharCount);
...
NTSTATUS DokanLogError(__in PDOKAN_LOGGER Logger, __in NTSTATUS Status, __in LPCTSTR Format, ...)
{
  va_list args;
  va_start(args, Format);
  DokanPrintToSysLog(Logger->DriverObject, Logger->MajorFunctionCode, DOKANFS_INFO_MSG, Status, Format, args);
  va_end(args);
  return Status;
}
...
VOID DokanLogInfo(__in PDOKAN_LOGGER Logger, __in LPCTSTR Format, ...)
{
  va_list args;
  va_start(args, Format);
  DokanPrintToSysLog(Logger->DriverObject, Logger->MajorFunctionCode, DOKANFS_INFO_MSG, STATUS_SUCCESS, Format, args);
  va_end(args);
}
...
VOID DokanCaptureBackTrace(__out PDokanBackTrace Trace)
{
  ...
  Trace->ReturnAddresses = (((count > 1) ? ((ULONG64)rawTrace[1] & 0xfffff) : 0) << 40) | (((count > 2) ? ((ULONG64)rawTrace[2] & 0xfffff) : 0) << 20) | ((count > 3) ? ((ULONG64)rawTrace[3] & 0xfffff) : 0);
}
...
PCHAR DokanGetFileInformationClassStr(FILE_INFORMATION_CLASS FileInformationClass)
{
  // List imported from wdm.h
...
VOID PushDokanLogEntry(_In_opt_ PVOID RequestContext, _In_ PCSTR Format, ...)
{
  ...
  // enabled ?
  if(requestContext && requestContext->Vcb && requestContext->Vcb->Dcb && !requestContext->Vcb->Dcb->DispatchDriverLogs)
  {
    return;
  }
  ...
  __try
  {
    ...
    if(g_DokanLogEntryList.NumberOfCachedEntries >= DOKAN_LOG_MAX_ENTRY_CACHED)
    {
    ...
    logEntry = DokanAllocZero(sizeof(DOKAN_LOG_ENTRY) + DOKAN_LOG_MAX_CHAR_COUNT);
    if(!logEntry)
    {
    ...
    va_list args;
    va_start(args, Format);
    NTSTATUS status             = RtlStringCchVPrintfExA(logEntry->Log.Message, DOKAN_LOG_MAX_CHAR_COUNT, &ppszDestEnd, NULL, 0, Format, args);
...
VOID PopDokanLogEntry(_In_opt_ PVOID RequestContext, _In_ PDokanVCB Vcb)
{
  ...
  while(listEntry != &(g_DokanLogEntryList.Log))
  {
    ...
    messageFullSize             = logEntry->Log.MessageLength + FIELD_OFFSET(DOKAN_LOG_MESSAGE, Message[0]);
    PEVENT_CONTEXT eventContext = AllocateEventContextRaw(sizeof(EVENT_CONTEXT) + messageFullSize);
    if(!eventContext)
    {
      return;
    }
    ...
    dokanLogString              = (PDOKAN_LOG_MESSAGE)((PCHAR)(PCHAR)eventContext + sizeof(EVENT_CONTEXT));
    RtlCopyMemory(dokanLogString, &logEntry->Log, messageFullSize);
    if(RequestContext)
    {
      DokanEventNotification(RequestContext, &Vcb->Dcb->NotifyEvent, eventContext);
    }
...
{
  return (!RequestContext && g_DokanDriverLogCacheEnabled) || (RequestContext && g_DokanVcbDriverLogCacheCount);
}
