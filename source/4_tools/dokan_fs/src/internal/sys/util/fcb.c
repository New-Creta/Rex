...
const UNICODE_STRING g_KeepAliveFileName = RTL_CONSTANT_STRING(DOKAN_KEEPALIVE_FILE_NAME);

const UNICODE_STRING g_NotificationFileName = RTL_CONSTANT_STRING(DOKAN_NOTIFICATION_FILE_NAME);

// We must NOT call without VCB lock
BOOLEAN DokanInitializeFcb(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB Fcb)
{
  Fcb->AdvancedFCBHeader.Resource = ExAllocateFromLookasideListEx(&g_DokanEResourceLookasideList);
  if(Fcb->AdvancedFCBHeader.Resource == NULL)
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Failed to allocate FCB ERESOURCE for %p", Fcb);
    return FALSE;
  }
...
PDokanFCB GetOrCreateUninitializedFcb(__in PREQUEST_CONTEXT RequestContext, __in PUNICODE_STRING FileName, __in PBOOLEAN NewElement)
{
  PDokanFCB fcb = NULL;
  ...
  PDokanFCB* fcbInTable = (PDokanFCB*)RtlInsertElementGenericTableAvl(&RequestContext->Vcb->FcbTable, &fcb, sizeof(PDokanFCB), NewElement);
  if(!fcbInTable)
  {
...
PDokanFCB DokanGetFCB(__in PREQUEST_CONTEXT RequestContext, __in PWCHAR FileName, __in ULONG FileNameLength)
{
  ...
  {
    ExFreePool(FileName);
    DOKAN_LOG_FINE_IRP(RequestContext, "Failed to find or allocate FCB for %wZ", &fn);
  ...
  if(newElement)
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "New FCB %p allocated for %wZ", fcb, &fcb->FileName);
    if(!DokanInitializeFcb(RequestContext, fcb))
    {
      BOOLEAN removed = RtlDeleteElementGenericTableAvl(&RequestContext->Vcb->FcbTable, &fcb);
      ASSERT(removed);
      UNREFERENCED_PARAMETER(removed);
      DOKAN_LOG_FINE_IRP(RequestContext, "Failed to init FCB %p for %wZ", fcb, &fcb->FileName);
      ExFreePool(FileName);
      ExFreeToLookasideListEx(&g_DokanFCBLookasideList, fcb);
      DokanVCBUnlock(RequestContext->Vcb);
      return NULL;
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Found existing FCB %p for %wZ", fcb, &fcb->FileName);
...
DokanFreeFCB(__in PDokanVCB Vcb, __in PDokanFCB Fcb)
{
  ...
  {
    DokanCaptureBackTrace(&trace);
    return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Freeing FCB that has wrong identifier type at %I64x:%I64x: %x", trace.Address, trace.ReturnAddresses, GetIdentifierType(Fcb));
  }
  ...
  if(InterlockedDecrement(&Fcb->FileCount) == 0 && !DokanScheduleFcbForGarbageCollection(Vcb, Fcb))
  {
...
VOID DokanDeleteFcb(__in PDokanVCB Vcb, __in PDokanFCB Fcb, __in BOOLEAN DeleteFromTable)
{
  ...
  ExFreeToLookasideListEx(&g_DokanEResourceLookasideList, Fcb->AdvancedFCBHeader.Resource);
...
BOOLEAN DokanScheduleFcbForGarbageCollection(__in PDokanVCB Vcb, __in PDokanFCB Fcb)
{
  ...
  if(Fcb->NextGarbageCollectableFcb.Flink != NULL)
  {
    ...
    DokanLogInfo(&logger, L"Warning: scheduled an FCB for garbage collection when it is"
                          L" already scheduled.");
    return TRUE;
  }
...
VOID DokanCancelFcbGarbageCollection(__in PDokanFCB Fcb, _Inout_ PUNICODE_STRING NewFileName)
{
  if(Fcb->NextGarbageCollectableFcb.Flink != NULL)
  {
...
VOID GarbageCollectFCB(__in PDokanVCB Vcb, __in PDokanFCB Fcb, __in BOOLEAN RemoveFromTable)
{
  RemoveEntryList(&Fcb->NextGarbageCollectableFcb);
  DokanFCBLockRW(Fcb);
...
// return value is 0.
ULONG DeleteFcbGarbageAndGetRemainingCount(__in PDokanVCB Vcb, __in BOOLEAN Force)
{
  ULONG remainingCount  = 0;
  PLIST_ENTRY thisEntry = NULL;
  PLIST_ENTRY nextEntry = NULL;
  PDokanFCB nextFcb     = NULL;
  for(thisEntry = Vcb->FcbGarbageList.Flink; thisEntry != &Vcb->FcbGarbageList; thisEntry = nextEntry)
  {
...
{
  if(Vcb->FcbGarbageCollectorThread == NULL || IsListEmpty(&Vcb->FcbGarbageList))
  {
    return FALSE;
  }
...
NTSTATUS WaitForNewFcbGarbage(__in PDokanVCB Vcb)
{
  ...
  NTSTATUS status = KeWaitForMultipleObjects(2, events, WaitAny, Executive, KernelMode, FALSE, NULL, NULL);
...
VOID FcbGarbageCollectorThread(__in PVOID pVcb)
{
  ...
  DokanLogInfo(&logger, L"Starting FCB garbage collector with %lu ms interval.", Vcb->Dcb->FcbGarbageCollectionIntervalMs);
  for(;;)
  {
...
void DokanStartFcbGarbageCollector(PDokanVCB Vcb)
{
  ...
  status = PsCreateSystemThread(&thread, THREAD_ALL_ACCESS, NULL, NULL, NULL, (PKSTART_ROUTINE)FcbGarbageCollectorThread, Vcb);
  if(!NT_SUCCESS(status))
  {
    // Note: we will revert to shared_ptr-style deletion if the thread is NULL.
    return;
  }
  ObReferenceObjectByHandle(thread, THREAD_ALL_ACCESS, NULL, KernelMode, (PVOID*)&Vcb->FcbGarbageCollectorThread, NULL);

  ZwClose(thread);
}

RTL_GENERIC_COMPARE_RESULTS DokanCompareFcb(__in struct _RTL_AVL_TABLE* Table, __in PVOID FirstStruct, __in PVOID SecondStruct)
{
  ...
  LONG result         = RtlCompareUnicodeString(&firstFcb->FileName, &secondFcb->FileName, !(vcb->Dcb->MountOptions & DOKAN_EVENT_CASE_SENSITIVE));
  DOKAN_LOG_VCB(vcb, "First: %p %wZ Second: %p %wZ - Result: %ld", firstFcb, &firstFcb->FileName, secondFcb, &secondFcb->FileName, result);
  if(result < 0)
  {
...
PVOID DokanAllocateFcbAvl(__in struct _RTL_AVL_TABLE* Table, __in CLONG ByteSize)
{
  PDokanVCB vcb = (PDokanVCB)Table->TableContext;
  if(!vcb->FCBAvlNodeLookasideListInit)
  {
    ...
    {
      DOKAN_LOG_VCB(vcb, "DokanLookasideCreate VCB FCBAvlNodeLookasideList failed.");
      return NULL;
    }
...
VOID DokanRenameFcb(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB Fcb, __in PWCH FileName, __in USHORT FileNameLength)
{
  BOOLEAN removed = RtlDeleteElementGenericTableAvl(&RequestContext->Vcb->FcbTable, &Fcb);
  ASSERT(removed);
  UNREFERENCED_PARAMETER(removed);
