...
NTSTATUS
DokanSendIoContlToMountManager(__in ULONG IoControlCode, __in PVOID InputBuffer, __in ULONG Length, __out PVOID OutputBuffer, __in ULONG OutputLength)
{
  NTSTATUS status;
  ...
  {
    DOKAN_LOG_("IoGetDeviceObjectPointer failed: 0x%x %s", status, DokanGetNTSTATUSStr(status));
    return status;
  }
  ...
  irp = IoBuildDeviceIoControlRequest(IoControlCode, mountDeviceObject, InputBuffer, Length, OutputBuffer, OutputLength, FALSE, &driverEvent, &iosb);
  ...
  {
    DOKAN_LOG_("IoCallDriver IoControlCode=%s pending", DokanGetIoctlStr(IoControlCode));
  ...
  DOKAN_LOG_("IoCallDriver %s IoControlCode=%s", DokanGetNTSTATUSStr(status), DokanGetIoctlStr(IoControlCode));
...
NTSTATUS DokanSendVolumeMountPoint(__in PDokanDCB Dcb, BOOLEAN Create)
{
  ...
  length          = sizeof(MOUNTMGR_VOLUME_MOUNT_POINT) + Dcb->MountPoint->Length + Dcb->PersistentSymbolicLinkName->Length;
  ...
  RtlCopyMemory((PCHAR)volumMountPoint + volumMountPoint->SourceVolumeNameOffset, Dcb->MountPoint->Buffer, volumMountPoint->SourceVolumeNameLength);
  volumMountPoint->TargetVolumeNameOffset = volumMountPoint->SourceVolumeNameOffset + volumMountPoint->SourceVolumeNameLength;
  volumMountPoint->TargetVolumeNameLength = Dcb->PersistentSymbolicLinkName->Length;
  RtlCopyMemory((PCHAR)volumMountPoint + volumMountPoint->TargetVolumeNameOffset, Dcb->PersistentSymbolicLinkName->Buffer, volumMountPoint->TargetVolumeNameLength);

  status = DokanSendIoContlToMountManager(Create ? IOCTL_MOUNTMGR_VOLUME_MOUNT_POINT_CREATED : IOCTL_MOUNTMGR_VOLUME_MOUNT_POINT_DELETED, volumMountPoint, length, NULL, 0);
...
DokanSendVolumeArrivalNotification(PUNICODE_STRING DeviceName)
{
  ...
  status = DokanSendIoContlToMountManager(IOCTL_MOUNTMGR_VOLUME_ARRIVAL_NOTIFICATION, targetName, length, NULL, 0);
...
NTSTATUS
DokanSendVolumeDeletePoints(__in PUNICODE_STRING MountPoint, __in PUNICODE_STRING DeviceName)
{
  NTSTATUS status;
  ...
  if(MountPoint != NULL)
  {
    ...
    nextStringOffset              = point->SymbolicLinkNameOffset + point->SymbolicLinkNameLength;
    RtlCopyMemory((PCHAR)point + point->SymbolicLinkNameOffset, MountPoint->Buffer, MountPoint->Length);
  }
  if(DeviceName != NULL)
  {
    ...
    RtlCopyMemory((PCHAR)point + point->DeviceNameOffset, DeviceName->Buffer, DeviceName->Length);
  }

  status = DokanSendIoContlToMountManager(IOCTL_MOUNTMGR_DELETE_POINTS, point, length, deletedPoints, olength);
...
NTSTATUS
DokanSendVolumeCreatePoint(__in PDRIVER_OBJECT DriverObject, __in PUNICODE_STRING DeviceName, __in PUNICODE_STRING MountPoint)
{
  NTSTATUS status;
  PMOUNTMGR_CREATE_POINT_INPUT point;
  ULONG length;
  DOKAN_INIT_LOGGER(logger, DriverObject, IRP_MJ_FILE_SYSTEM_CONTROL);
  DokanLogInfo(&logger, L"Creating mount point: \"%wZ\" for device name: \"%wZ\".", MountPoint, DeviceName);

  length = sizeof(MOUNTMGR_CREATE_POINT_INPUT) + MountPoint->Length + DeviceName->Length;
  point  = DokanAllocZero(length);
  if(point == NULL)
  {
    return DokanLogError(&logger, STATUS_INSUFFICIENT_RESOURCES, L"Failed to allocate MOUNTMGR_CREATE_POINT_INPUT.");
  }
  ...
  point->SymbolicLinkNameOffset = point->DeviceNameOffset + point->DeviceNameLength;
  point->SymbolicLinkNameLength = MountPoint->Length;
  RtlCopyMemory((PCHAR)point + point->SymbolicLinkNameOffset, MountPoint->Buffer, MountPoint->Length);

  status = DokanSendIoContlToMountManager(IOCTL_MOUNTMGR_CREATE_POINT, point, length, NULL, 0);
...
NTSTATUS DokanQueryAutoMount(PBOOLEAN State)
{
  ...
  status = DokanSendIoContlToMountManager(IOCTL_MOUNTMGR_QUERY_AUTO_MOUNT, NULL, 0, &queryAutoMount, sizeof(queryAutoMount));
  if(NT_SUCCESS(status))
  {
...
NTSTATUS DokanSendAutoMount(BOOLEAN State)
{
  ...
  status = DokanSendIoContlToMountManager(IOCTL_MOUNTMGR_SET_AUTO_MOUNT, &setAutoMount, sizeof(setAutoMount), NULL, 0);
  return status;
}