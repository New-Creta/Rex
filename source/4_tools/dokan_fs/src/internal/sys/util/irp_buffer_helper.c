...
ULONG GetProvidedInputSize(_In_ PIRP Irp)
{
  ...
  {
    case IRP_MJ_DEVICE_CONTROL: return irpSp->Parameters.DeviceIoControl.InputBufferLength;
    case IRP_MJ_FILE_SYSTEM_CONTROL: return irpSp->Parameters.FileSystemControl.InputBufferLength;
    default: return 0;
  }
}
...
ULONG GetProvidedOutputSize(_In_ PIRP Irp)
{
  ...
  switch(irpSp->MajorFunction)
  {
    case IRP_MJ_DEVICE_CONTROL: return irpSp->Parameters.DeviceIoControl.OutputBufferLength;
    case IRP_MJ_DIRECTORY_CONTROL: return irpSp->Parameters.QueryDirectory.Length;
    case IRP_MJ_FILE_SYSTEM_CONTROL: return irpSp->Parameters.FileSystemControl.OutputBufferLength;
    case IRP_MJ_QUERY_INFORMATION: return irpSp->Parameters.QueryFile.Length;
    case IRP_MJ_QUERY_SECURITY: return irpSp->Parameters.QuerySecurity.Length;
    case IRP_MJ_QUERY_VOLUME_INFORMATION: return irpSp->Parameters.QueryVolume.Length;
    default: return 0;
  }
}
...
PVOID GetInputBuffer(_In_ PIRP Irp)
{
  ...
  if(irpSp->MajorFunction == IRP_MJ_DEVICE_CONTROL && (irpSp->Parameters.DeviceIoControl.IoControlCode & methodMask) == METHOD_NEITHER)
  {
    buffer = irpSp->Parameters.DeviceIoControl.Type3InputBuffer;
  }
  if(irpSp->MajorFunction == IRP_MJ_FILE_SYSTEM_CONTROL && (irpSp->Parameters.FileSystemControl.FsControlCode & methodMask) == METHOD_NEITHER)
  {
    buffer = irpSp->Parameters.FileSystemControl.Type3InputBuffer;
  }

  // If using a Type3InputBuffer, we need to probe it.
  if(Irp->RequestorMode != KernelMode && buffer != NULL && buffer != Irp->AssociatedIrp.SystemBuffer)
  {
    __try
    {
...
BOOLEAN AppendVarSizeOutputString(_Inout_ PIRP Irp, _Inout_ PVOID Dest, const _In_ UNICODE_STRING* Str, _In_ BOOLEAN UpdateInformationOnFailure, _In_ BOOLEAN FillSpaceWithPartialString)
{
  ...
  if(remainingSize < copySize)
  {
    ...
    if(!ExtendOutputBufferBySize(Irp, additionalSize, UpdateInformationOnFailure))
    {
