...
PUNICODE_STRING DokanAllocDuplicateString(const __in PUNICODE_STRING Src)
{
...
BOOLEAN DokanDuplicateUnicodeString(__out PUNICODE_STRING Dest, const __in PUNICODE_STRING Src)
{
  if(Dest->Buffer)
  {
...
BOOLEAN StartsWith(const __in UNICODE_STRING* Str, const __in UNICODE_STRING* Prefix)
{
...
BOOLEAN StartsWithDosDevicesPrefix(const __in UNICODE_STRING* Str)
{
  return StartsWith(Str, &g_DosDevicesPrefix);
}

BOOLEAN StartsWithVolumeGuidPrefix(const __in UNICODE_STRING* Str)
{
  return StartsWith(Str, &g_VolumeGuidPrefix);
}

PUNICODE_STRING ChangePrefix(const UNICODE_STRING* Str, const UNICODE_STRING* Prefix, BOOLEAN HasPrefix, const UNICODE_STRING* NewPrefix)
{
  ...
  UNICODE_STRING strAfterPrefix = DokanWrapUnicodeString((PWCHAR)((PCHAR)Str->Buffer + prefixLength), Str->Length - prefixLength);
  RtlUnicodeStringCat(newStr, &strAfterPrefix);
  return newStr;
}

BOOLEAN IsMountPointDriveLetter(const __in UNICODE_STRING* MountPoint)
{
  ...
  BOOLEAN nonTerminatedDriveLetterLength  = MountPoint->Length == driveLetterLength;
  BOOLEAN nullTerminatedDriveLetterLength = MountPoint->Length == driveLetterLength + sizeof(WCHAR) && MountPoint->Buffer[colonIndex + 1] == L'\0';
  // Note: the size range is for an optional null char.
  return StartsWithDosDevicesPrefix(MountPoint) && (nonTerminatedDriveLetterLength || nullTerminatedDriveLetterLength) && MountPoint->Buffer[colonIndex] == L':';
}

ULONG DokanSearchWcharinUnicodeStringWithUlong(__in PUNICODE_STRING inputPUnicodeString, __in WCHAR targetWchar, __in ULONG offsetPosition, __in int isIgnoreTargetWchar)
{
...
LONG DokanSearchUnicodeStringChar(__in PUNICODE_STRING UnicodeString, __in WCHAR Char)
{
  return DokanSearchStringChar(UnicodeString->Buffer, UnicodeString->Length, Char);
}

LONG DokanSearchStringChar(__in PWCHAR String, __in ULONG Length, __in WCHAR Char)
{
