...
--*/
{
  ...
  __try
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p MdlAddress=%p UserBuffer=%p Length=%ld ByteOffset=%I64u", fileObject, RequestContext->Irp->MdlAddress, RequestContext->Irp->UserBuffer, RequestContext->IrpSp->Parameters.Read.Length,
    ...
    if(fileObject == NULL && RequestContext->Irp->MdlAddress != NULL)
    {
      ...
      currentAddress = MmGetSystemAddressForMdlNormalSafe(RequestContext->Irp->MdlAddress);
      ...
      RequestContext->Irp->IoStatus.Information = RequestContext->IrpSp->Parameters.Read.Length;
      status                                    = STATUS_SUCCESS;
      __leave;
    }
    ...
    if(RequestContext->IrpSp->Parameters.Read.ByteOffset.LowPart == FILE_USE_FILE_POINTER_POSITION && RequestContext->IrpSp->Parameters.Read.ByteOffset.HighPart == -1)
    {
    ...
    {
      status = DokanAllocateMdl(RequestContext, RequestContext->IrpSp->Parameters.Read.Length);
    ...
    {
      CcFlushCache(&fcb->SectionObjectPointers, &RequestContext->IrpSp->Parameters.Read.ByteOffset, RequestContext->IrpSp->Parameters.Read.Length, NULL);
    }
    ...
    eventContext->Operation.Read.BufferLength = RequestContext->IrpSp->Parameters.Read.Length;
...
VOID DokanCompleteRead(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ...
  DOKAN_LOG_FINE_IRP(RequestContext, "BufferLen %lu, Event.BufferLen %lu", bufferLen, EventInfo->BufferLength);
  ...
  else
  {
    ...
    if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status) && EventInfo->BufferLength > 0 && (fileObject->Flags & FO_SYNCHRONOUS_IO) && !(RequestContext->Irp->Flags & IRP_PAGING_IO))
    {
      // update current byte offset only when synchronous IO and not pagind IO
      fileObject->CurrentByteOffset.QuadPart = EventInfo->Operation.Read.CurrentByteOffset.QuadPart;
