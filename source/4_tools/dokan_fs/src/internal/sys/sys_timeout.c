...
ReleaseTimeoutPendingIrp(__in PDokanDCB Dcb)
{
  ...
  for(thisEntry = listHead->Flink; thisEntry != listHead; thisEntry = nextEntry)
  {
    ...
    if(irpEntry->AsyncStatus == STATUS_SUCCESS && tickCount.QuadPart < irpEntry->TickCount.QuadPart)
    {
      continue;
    }
    ...
    if(irpSp->MajorFunction == IRP_MJ_CREATE)
    {
      ...
      if(fileObject != NULL)
      {
        ...
        if(ccb != NULL)
        {
          ...
          OplockDebugRecordFlag(fcb, canceled ? DOKAN_OPLOCK_DEBUG_CANCELED_CREATE : DOKAN_OPLOCK_DEBUG_TIMED_OUT_CREATE);
        }
      }
      DokanCancelCreateIrp(&irpEntry->RequestContext, canceled ? STATUS_CANCELLED : STATUS_INSUFFICIENT_RESOURCES);
    }
    else
    {
  ...
  if(shouldUnmount)
  {
    ...
    // in Explorer.
    DokanLogInfo(&logger, L"Unmounting due to operation timeout before keepalive handle was"
                          L" activated.");
...
--*/
{
  ...
  while(waitObj)
  {
    status = KeWaitForMultipleObjects(3, pollevents, WaitAny, Executive, KernelMode, FALSE, NULL, NULL);
...
--*/
{
  ...
  status = PsCreateSystemThread(&thread, THREAD_ALL_ACCESS, NULL, NULL, NULL, (PKSTART_ROUTINE)DokanTimeoutThread, Dcb);
  ...
  ObReferenceObjectByHandle(thread, THREAD_ALL_ACCESS, NULL, KernelMode, (PVOID*)&Dcb->TimeoutThread, NULL);
