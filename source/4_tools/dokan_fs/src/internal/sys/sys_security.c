...
DokanDispatchQuerySecurity(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  securityInfo = &RequestContext->IrpSp->Parameters.QuerySecurity.SecurityInformation;
  ...
  RtlCopyMemory(eventContext->Operation.Security.FileName, fcb->FileName.Buffer, fcb->FileName.Length);
  DokanFCBUnlock(fcb);
...
VOID DokanCompleteQuerySecurity(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ...
  if(EventInfo->Status == STATUS_SUCCESS && EventInfo->BufferLength <= bufferLength && buffer != NULL)
  {
    if(!RtlValidRelativeSecurityDescriptor(EventInfo->Buffer, EventInfo->BufferLength, RequestContext->IrpSp->Parameters.QuerySecurity.SecurityInformation))
    {
  ...
  else if(EventInfo->Status == STATUS_BUFFER_OVERFLOW || (EventInfo->Status == STATUS_SUCCESS && bufferLength < EventInfo->BufferLength))
  {
    RequestContext->Irp->IoStatus.Information = EventInfo->BufferLength;
...
DokanDispatchSetSecurity(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  securityInfo = &RequestContext->IrpSp->Parameters.SetSecurity.SecurityInformation;
  ...
  securityDescriptor = RequestContext->IrpSp->Parameters.SetSecurity.SecurityDescriptor;
  ...
  // align BufferOffset.
  eventLength = sizeof(EVENT_CONTEXT) + securityDescLength + fcb->FileName.Length + 3;

  if(EVENT_CONTEXT_MAX_SIZE < eventLength)
  {
    // TODO: Handle this case like DispatchWrite.
    DOKAN_LOG_FINE_IRP(RequestContext, "SecurityDescriptor is too big: %d (limit %d)", eventLength, EVENT_CONTEXT_MAX_SIZE);
    return STATUS_INSUFFICIENT_RESOURCES;
  }
  ...
  eventContext->Operation.SetSecurity.BufferOffset = (FIELD_OFFSET(EVENT_CONTEXT, Operation.SetSecurity.FileName[0]) + fcb->FileName.Length + sizeof(WCHAR) + 3) & ~0x03;
  RtlCopyMemory((PCHAR)eventContext + eventContext->Operation.SetSecurity.BufferOffset, securityDescriptor, securityDescLength);
...
VOID DokanCompleteSetSecurity(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ...
  {
    DokanFCBLockRO(fcb);
    DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_SECURITY, FILE_ACTION_MODIFIED);
    DokanFCBUnlock(fcb);
  }
