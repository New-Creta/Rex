...
--*/
{
  ...
  ULONG eventLength;
  DOKAN_INIT_LOGGER(logger, RequestContext->DeviceObject->DriverObject, IRP_MJ_CLEANUP);
  ...
  if(fileObject == NULL || RequestContext->Vcb == NULL || !DokanCheckCCB(RequestContext, fileObject->FsContext2))
  {
  ...
  if(fcb->IsKeepalive)
  {
    ...
    if(shouldUnmount)
    {
      ...
      {
        DokanLogInfo(&logger, L"Ignoring keepalive close because unmount is already in"
                              L" progress.");
      }
      else
      {
...
VOID DokanCompleteCleanup(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ...
  {
    DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_LAST_WRITE, FILE_ACTION_MODIFIED);
  }
  ...
  if(DokanFCBFlagsIsSet(fcb, DOKAN_DELETE_ON_CLOSE))
  {
    ...
    {
      DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_DIR_NAME, FILE_ACTION_REMOVED);
    }
    else
    {
      DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_REMOVED);
    }
  }
  DokanFCBUnlock(fcb);
  //
  ...
  //
  (VOID) FsRtlFastUnlockAll(&fcb->FileLock, fileObject, IoGetRequestorProcess(RequestContext->Irp), NULL);

  if(DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY))
  {
    FsRtlNotifyCleanup(RequestContext->Vcb->NotifySync, &RequestContext->Vcb->DirNotifyList, ccb);
  }
