...
DokanCommonLockControl(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  if(!DokanFsRtlAreThereWaitingFileLocks ||
     ...
     ((IRP_MN_LOCK == RequestContext->IrpSp->MinorFunction) && ((ULONGLONG)RequestContext->IrpSp->Parameters.LockControl.ByteOffset.QuadPart < (ULONGLONG)fcb->AdvancedFCBHeader.AllocationSize.QuadPart)) ||
     ((IRP_MN_LOCK != RequestContext->IrpSp->MinorFunction) && DokanFsRtlAreThereWaitingFileLocks(&fcb->FileLock)))
  {
    //
    ...
    status = DokanCheckOplock(fcb, RequestContext->Irp, NULL /* EventContext */, NULL /*DokanOplockComplete*/, NULL);
  }
...
DokanDispatchLock(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  __try
  {
    ...
    // parameters of Lock
    eventContext->Operation.Lock.ByteOffset = RequestContext->IrpSp->Parameters.LockControl.ByteOffset;
    if(RequestContext->IrpSp->Parameters.LockControl.Length != NULL)
    {
      eventContext->Operation.Lock.Length.QuadPart = RequestContext->IrpSp->Parameters.LockControl.Length->QuadPart;
    }
    else
    {
      DOKAN_LOG_FINE_IRP(RequestContext, "LockControl.Length = NULL");
    }
    eventContext->Operation.Lock.Key = RequestContext->IrpSp->Parameters.LockControl.Key;
