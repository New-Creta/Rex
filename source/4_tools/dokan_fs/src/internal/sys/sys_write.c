...
DokanDispatchWrite(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  __try
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p MdlAddress=%p UserBuffer=%p Length=%ld ByteOffset=%I64u", fileObject, RequestContext->Irp->MdlAddress, RequestContext->Irp->UserBuffer, RequestContext->IrpSp->Parameters.Write.Length,
    ...
    DOKAN_INIT_LOGGER(logger, RequestContext->DeviceObject->DriverObject, IRP_MJ_WRITE);
    ...
    if(RequestContext->IrpSp->Parameters.Write.ByteOffset.LowPart == FILE_WRITE_TO_END_OF_FILE && RequestContext->IrpSp->Parameters.Write.ByteOffset.HighPart == -1)
    {
      writeToEoF = TRUE;
    }
    ...
    {
      CcFlushCache(&fcb->SectionObjectPointers, writeToEoF ? NULL : &RequestContext->IrpSp->Parameters.Write.ByteOffset, RequestContext->IrpSp->Parameters.Write.Length, NULL);

      DokanPagingIoLockRW(fcb);
      DokanPagingIoUnlock(fcb);

      CcPurgeCacheSection(&fcb->SectionObjectPointers, writeToEoF ? NULL : &RequestContext->IrpSp->Parameters.Write.ByteOffset, RequestContext->IrpSp->Parameters.Write.Length, FALSE);
    }
    ...
    safeEventLength.QuadPart = sizeof(EVENT_CONTEXT) + RequestContext->IrpSp->Parameters.Write.Length + fcb->FileName.Length;
    if(safeEventLength.HighPart != 0 || safeEventLength.QuadPart < sizeof(EVENT_CONTEXT) + fcb->FileName.Length)
    {
      DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Write with unsupported total size: %I64u", safeEventLength.QuadPart);
      status = STATUS_INVALID_PARAMETER;
      __leave;
    }
    ...
    // more bigger memory.
    RequestContext->Irp->Tail.Overlay.DriverContext[DRIVER_CONTEXT_EVENT] = eventContext;
    ...
    eventContext->Operation.Write.ByteOffset = RequestContext->IrpSp->Parameters.Write.ByteOffset;
    ...
    if(isSynchronousIo && ((RequestContext->IrpSp->Parameters.Write.ByteOffset.LowPart == FILE_USE_FILE_POINTER_POSITION) && (RequestContext->IrpSp->Parameters.Write.ByteOffset.HighPart == -1)))
    {
      // NOTE:
    ...
    eventContext->Operation.Write.BufferLength = RequestContext->IrpSp->Parameters.Write.Length;

    // the offset from the beginning of structure
    // the contents to write will be copyed to this offset
    eventContext->Operation.Write.BufferOffset = FIELD_OFFSET(EVENT_CONTEXT, Operation.Write.FileName[0]) + fcb->FileName.Length + sizeof(WCHAR); // adds last null char

    // copies the content to write to EventContext
    RtlCopyMemory((PCHAR)eventContext + eventContext->Operation.Write.BufferOffset, buffer, RequestContext->IrpSp->Parameters.Write.Length);
    ...
    else
    {
      ...
      ULONG eventWriteContextLength    = max(sizeof(EVENT_CONTEXT), eventContext->Operation.Write.BufferOffset);
      PEVENT_CONTEXT eventWriteContext = AllocateEventContext(RequestContext, eventWriteContextLength, ccb);
      ...
      RtlCopyMemory(eventWriteContext, eventContext, eventContext->Operation.Write.BufferOffset);
      ...
      if(!FlagOn(RequestContext->Irp->Flags, IRP_PAGING_IO))
      {
        status = FsRtlCheckOplock(DokanGetFcbOplock(fcb), RequestContext->Irp, eventWriteContext, DokanOplockComplete, DokanPrePostIrp);
...
VOID DokanCompleteWrite(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ...
  if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status))
  {
    ...
    if(fcb->AdvancedFCBHeader.FileSize.QuadPart < EventInfo->Operation.Write.CurrentByteOffset.QuadPart)
    {
      if(!isPagingIo)
      {
        DokanFCBLockRO(fcb);
      ...
      DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_SIZE, FILE_ACTION_MODIFIED);
      ...
      InterlockedExchange64(&fcb->AdvancedFCBHeader.FileSize.QuadPart, EventInfo->Operation.Write.CurrentByteOffset.QuadPart);
    }
    ...
    if(EventInfo->BufferLength != 0 && fileObject->Flags & FO_SYNCHRONOUS_IO && !isPagingIo)
    {
      // update current byte offset only when synchronous IO and not paging IO
      fileObject->CurrentByteOffset.QuadPart = EventInfo->Operation.Write.CurrentByteOffset.QuadPart;
      DOKAN_LOG_FINE_IRP(RequestContext, "Updated CurrentByteOffset %I64u", fileObject->CurrentByteOffset.QuadPart);
    }
  }
}
