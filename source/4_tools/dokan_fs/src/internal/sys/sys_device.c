...
#include "dokan_fs/internal/sys/util/irp_buffer_helper.h"
...
GlobalDeviceControl(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported IoControlCode %x", RequestContext->IrpSp->Parameters.DeviceIoControl.IoControlCode);
  return status;
}

VOID DokanPopulateDiskGeometry(__out PDISK_GEOMETRY DiskGeometry)
{
  DiskGeometry->Cylinders.QuadPart = DOKAN_DEFAULT_DISK_SIZE / DOKAN_DEFAULT_SECTOR_SIZE / 32 / 2;
...
NTSTATUS
DiskDeviceControl(__in PREQUEST_CONTEXT RequestContext, __in PDEVICE_OBJECT DeviceObject)
{
  PDokanDCB dcb;
  PDokanVCB vcb;
  NTSTATUS status = STATUS_INVALID_DEVICE_REQUEST;
  DOKAN_INIT_LOGGER(logger, RequestContext->DeviceObject->DriverObject, IRP_MJ_DEVICE_CONTROL);
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Device object is pending for delete valid anymore");
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Volume is unmounted so ignore dcb requests");
    return STATUS_NO_SUCH_DEVICE;
  }

  DOKAN_LOG_FINE_IRP(RequestContext, "Device name \"%wZ\"", dcb->DiskDeviceName);
  ...
  switch(RequestContext->IrpSp->Parameters.DeviceIoControl.IoControlCode)
  {
    ...
    case IOCTL_DISK_IS_WRITABLE: status = IS_DEVICE_READ_ONLY(DeviceObject) ? STATUS_MEDIA_WRITE_PROTECTED : STATUS_SUCCESS; break;

    case IOCTL_DISK_MEDIA_REMOVAL: status = STATUS_SUCCESS; break;

    case IOCTL_STORAGE_MEDIA_REMOVAL: status = STATUS_SUCCESS; break;
    ...
    case IOCTL_DISK_CHECK_VERIFY: status = STATUS_SUCCESS; break;

    case IOCTL_STORAGE_CHECK_VERIFY2: status = STATUS_SUCCESS; break;
    ...
    case IOCTL_STORAGE_QUERY_PROPERTY:
    {
      ...
      if(query->QueryType == PropertyExistsQuery)
      {
        ...
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "PropertyExistsQuery StorageDeviceUniqueIdProperty");
        ...
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "PropertyExistsQuery StorageDeviceWriteCacheProperty");
          status = STATUS_NOT_SUPPORTED;
        }
        else
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "PropertyExistsQuery Unknown %d", query->PropertyId);
          status = STATUS_NOT_SUPPORTED;
        }
      }
      else if(query->QueryType == PropertyStandardQuery)
      {
        ...
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "PropertyStandardQuery StorageDeviceProperty");
        ...
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "PropertyStandardQuery StorageAdapterProperty");
          status = STATUS_NOT_SUPPORTED;
        }
        else
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "PropertyStandardQuery Unknown %d", query->PropertyId);
          status = STATUS_ACCESS_DENIED;
        }
      }
      else
      {
        DOKAN_LOG_FINE_IRP(RequestContext, "Unknown query type %d", query->QueryType);
        status = STATUS_ACCESS_DENIED;
      }
      break;
    }
    ...
    case IOCTL_MOUNTDEV_QUERY_DEVICE_NAME:
    {
      ...
      if(!PrepareOutputHelper(RequestContext->Irp, &mountdevName, FIELD_OFFSET(MOUNTDEV_NAME, Name),
      ...
      if(AppendVarSizeOutputString(RequestContext->Irp, &mountdevName->Name, dcb->DiskDeviceName,
    ...
    case IOCTL_MOUNTDEV_QUERY_UNIQUE_ID:
    {
      ...
      if(!PrepareOutputHelper(RequestContext->Irp, &uniqueId, FIELD_OFFSET(MOUNTDEV_UNIQUE_ID, UniqueId),
                              /*SetInformationOnFailure=*/TRUE))
      {
        status = STATUS_BUFFER_TOO_SMALL;
        break;
      ...
      if(AppendVarSizeOutputString(RequestContext->Irp, &uniqueId->UniqueId, dcb->DiskDeviceName,
                                   /*UpdateInformationOnFailure=*/FALSE,
                                   /*FillSpaceWithPartialString=*/FALSE))
      {
        status = STATUS_SUCCESS;
    ...
    case IOCTL_MOUNTDEV_QUERY_SUGGESTED_LINK_NAME:
    {
      ...
      if(!PrepareOutputHelper(RequestContext->Irp, &linkName, FIELD_OFFSET(MOUNTDEV_SUGGESTED_LINK_NAME, Name),
                              /*SetInformationOnFailure=*/TRUE))
      {
        status = STATUS_BUFFER_TOO_SMALL;
        break;
      ...
      {
        DokanLogInfo(&logger, L"Not suggesting a link name because auto-assignment was "
                              L"requested.");
      ...
      {
        DokanLogInfo(&logger, L"Not suggesting link name because DCB has no mount point set.");
        status = STATUS_NOT_FOUND;
        break;
      }
      ...
      if(!AppendVarSizeOutputString(RequestContext->Irp, &linkName->Name, dcb->MountPoint,
                                    /*UpdateInformationOnFailure=*/FALSE,
                                    /*FillSpaceWithPartialString=*/FALSE))
      {
        DokanLogInfo(&logger, L"Could not fit the suggested name in the output buffer.");
        status = STATUS_BUFFER_OVERFLOW;
        break;
      }
    ...
    case IOCTL_MOUNTDEV_LINK_CREATED:
    {
      ...
      UNICODE_STRING mountdevNameString = DokanWrapUnicodeString(mountdevName->Name, mountdevName->NameLength);
      status                            = STATUS_SUCCESS;
      ...
      if(!dcb->PersistentSymbolicLinkName && StartsWithVolumeGuidPrefix(&mountdevNameString))
      {
        dcb->PersistentSymbolicLinkName = DokanAllocDuplicateString(&mountdevNameString);
        break;
      }
      ...
      if(dcb->MountPoint && RtlEqualUnicodeString(dcb->MountPoint, &mountdevNameString,
      ...
      // later.
      PMOUNT_ENTRY mountEntry = FindMountEntryByName(dcb->Global, dcb->DiskDeviceName, dcb->UNCName, /*LockGlobal=*/TRUE);
      if(mountEntry != NULL)
      {
        RtlStringCchCopyUnicodeString(mountEntry->MountControl.MountPoint, MAXIMUM_FILENAME_LENGTH, dcb->MountPoint);
      }
      else
      {
    ...
    case IOCTL_MOUNTDEV_LINK_DELETED:
    {
      ...
      {
        DokanLogInfo(&logger, L"Mount manager is disabled for this device or dokan initiated"
                              L" deletion; ignoring.");
        break;
      }
      if(!dcb->MountPoint || dcb->MountPoint->Length == 0)
      {
        DokanLogInfo(&logger, L"Deleting the device even though it never had the mount "
                              L"point set.");
    ...
    case IOCTL_REDIR_QUERY_PATH:
    {
      ...
      if(dcb->UNCName != NULL && dcb->UNCName->Length > 0)
      {
        ...
        if(RequestContext->IrpSp->Parameters.DeviceIoControl.IoControlCode == IOCTL_REDIR_QUERY_PATH)
        {
          PQUERY_PATH_REQUEST pathReq;

          GET_IRP_BUFFER_OR_BREAK(RequestContext->Irp, pathReq);

          DOKAN_LOG_FINE_IRP(RequestContext, "PathNameLength = %d", pathReq->PathNameLength);
          DOKAN_LOG_FINE_IRP(RequestContext, "SecurityContext = %p", pathReq->SecurityContext);
          DOKAN_LOG_FINE_IRP(RequestContext, "FilePathName = %.*ls", (unsigned int)(pathReq->PathNameLength / sizeof(WCHAR)), pathReq->FilePathName);
          ...
          {
            prefixOk = (_wcsnicmp(pathReq->FilePathName, dcb->UNCName->Buffer, dcb->UNCName->Length / sizeof(WCHAR)) == 0);
          }
        }
        else
        {
          ...
          DOKAN_LOG_FINE_IRP(RequestContext, "pSecurityContext = %p", pathReqEx->pSecurityContext);
          DOKAN_LOG_FINE_IRP(RequestContext, "EaLength = %d", pathReqEx->EaLength);
          DOKAN_LOG_FINE_IRP(RequestContext, "pEaBuffer = %p", pathReqEx->pEaBuffer);
          DOKAN_LOG_FINE_IRP(RequestContext, "PathNameLength = %d", pathReqEx->PathName.Length);
          DOKAN_LOG_FINE_IRP(RequestContext, "FilePathName = %*ls", (unsigned int)(pathReqEx->PathName.Length / sizeof(WCHAR)), pathReqEx->PathName.Buffer);
          ...
          {
            prefixOk = (_wcsnicmp(pathReqEx->PathName.Buffer, dcb->UNCName->Buffer, dcb->UNCName->Length / sizeof(WCHAR)) == 0);
          }
        }
    ...
    default: DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported IoControlCode %x", RequestContext->IrpSp->Parameters.DeviceIoControl.IoControlCode); break;
  }
...
NTSTATUS
DiskDeviceControlWithLock(__in PREQUEST_CONTEXT RequestContext, __in PDEVICE_OBJECT DeviceObject)
{
  PDokanDCB dcb;
  NTSTATUS status = STATUS_INVALID_DEVICE_REQUEST;
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "IoAcquireRemoveLock failed with %#x %s", status, DokanGetNTSTATUSStr(status));
...
VolumeDeviceControl(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  ULONG baseCode = DEVICE_TYPE_FROM_CTL_CODE(RequestContext->IrpSp->Parameters.DeviceIoControl.IoControlCode);
  switch(baseCode)
  {
    case IOCTL_STORAGE_BASE:
    case IOCTL_DISK_BASE:
    case FILE_DEVICE_NETWORK_FILE_SYSTEM: status = DiskDeviceControlWithLock(RequestContext, RequestContext->Dcb->DeviceObject); break;
    ...
    case MOUNTDEVCONTROLTYPE:
      ...
      status = DiskDeviceControlWithLock(RequestContext, RequestContext->Dcb->DeviceObject);
      break;
...
DokanDispatchDeviceControl(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  DOKAN_LOG_IOCTL(RequestContext, RequestContext->IrpSp->Parameters.DeviceIoControl.IoControlCode, "DeviceControl");

  if(RequestContext->DeviceObject->DriverObject == NULL || RequestContext->DeviceObject->ReferenceCount < 0)
  {
  ...
  {
    status = DiskDeviceControlWithLock(RequestContext, RequestContext->DeviceObject);
  }
