...
VOID SetCommonEventContext(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_CONTEXT EventContext, __in_opt PDokanCCB Ccb)
{
...
AllocateEventContextRaw(__in ULONG EventContextLength)
{
  ...
  if(EventContextLength < sizeof(EVENT_CONTEXT) || EventContextLength > MAXULONG - sizeof(DRIVER_EVENT_CONTEXT))
  {
...
{
  PDRIVER_EVENT_CONTEXT driverEventContext = CONTAINING_RECORD(EventContext, DRIVER_EVENT_CONTEXT, EventContext);
...
VOID ReleaseNotifyEvent(__in PIRP_LIST NotifyEvent)
{
  ...
  while(!IsListEmpty(&NotifyEvent->ListHead))
  {
    ...
    driverEventContext = CONTAINING_RECORD(listHead, DRIVER_EVENT_CONTEXT, ListEntry);
    ExFreePool(driverEventContext);
  }
...
VOID NotificationThread(__in PVOID pDcb)
{
  ...
  do
  {
    status = KeWaitForMultipleObjects(2, events, WaitAny, Executive, KernelMode, FALSE, NULL, waitBlock);
...
DokanStartEventNotificationThread(__in PDokanDCB Dcb)
{
  ...
  status = PsCreateSystemThread(&thread, THREAD_ALL_ACCESS, NULL, NULL, NULL, (PKSTART_ROUTINE)NotificationThread, Dcb);
  ...
  ObReferenceObjectByHandle(thread, THREAD_ALL_ACCESS, NULL, KernelMode, (PVOID*)&Dcb->EventNotificationThread, NULL);
...
VOID DokanStopEventNotificationThread(__in PDokanDCB Dcb)
{
  ...
  if(KeSetEvent(&Dcb->ReleaseEvent, 0, FALSE) > 0 && Dcb->EventNotificationThread)
  {
...
VOID DokanStopFcbGarbageCollectorThread(__in PDokanVCB Vcb)
{
  ...
  {
    KeWaitForSingleObject(Vcb->FcbGarbageCollectorThread, Executive, KernelMode, FALSE, NULL);
    ObDereferenceObject(Vcb->FcbGarbageCollectorThread);
    Vcb->FcbGarbageCollectorThread = NULL;
  }
}

NTSTATUS DokanEventRelease(__in_opt PREQUEST_CONTEXT RequestContext, __in PDEVICE_OBJECT DeviceObject)
{
  PDokanDCB dcb;
  PDokanVCB vcb;
  NTSTATUS status = STATUS_SUCCESS;
  DOKAN_INIT_LOGGER(logger, DeviceObject == NULL ? NULL : DeviceObject->DriverObject, 0);
  ...
  {
    return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"VCB being released has wrong identifier type.");
  }
  dcb = vcb->Dcb;
  ...
  DokanLogInfo(&logger, L"Starting unmount for device \"%wZ\"", dcb->DiskDeviceName);
...
ULONG GetCurrentSessionId(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "IoGetRequestorSessionId failed %s", DokanGetNTSTATUSStr(status));
    return (ULONG)-1;
  }
...
NTSTATUS DokanGlobalEventRelease(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  {
    if(szMountPoint->Length > sizeof(dokanControl.MountPoint) - 12 * sizeof(WCHAR))
    {
  ...
  if(mountEntry == NULL)
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "Cannot found device associated to mount point %ws", dokanControl.MountPoint);
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Device is still not mounted, so an unmount not possible at this "
                                       "point");
    return STATUS_DEVICE_BUSY;
  }

  return DokanEventRelease(RequestContext, mountEntry->MountControl.VolumeDeviceObject);
}
