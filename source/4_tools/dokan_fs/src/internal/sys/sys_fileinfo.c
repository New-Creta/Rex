...
NTSTATUS FillNameInformation(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB Fcb, __in PFILE_NAME_INFORMATION NameInfo)
{
  PUNICODE_STRING fileName = &Fcb->FileName;
  PCHAR dest               = (PCHAR)&NameInfo->FileName;
  NameInfo->FileNameLength = fileName->Length;

  BOOLEAN isNetworkDevice = (RequestContext->Dcb->VolumeDeviceType == FILE_DEVICE_NETWORK_FILE_SYSTEM);
  if(isNetworkDevice)
  {
    PUNICODE_STRING devicePath = RequestContext->Dcb->UNCName->Length ? RequestContext->Dcb->UNCName : RequestContext->Dcb->DiskDeviceName;
...
DokanDispatchQueryInformation(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  __try
  {
    ...
    infoClass  = RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass;

    DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p InfoClass=%s ", fileObject, DokanGetFileInformationClassStr(infoClass));

    if(fileObject == NULL || !RequestContext->Vcb || !DokanCheckCCB(RequestContext, fileObject->FsContext2))
    {
    ...
    switch(infoClass)
    {
      case FileAllInformation:
      {
        ...
        if(!PrepareOutputHelper(RequestContext->Irp, &allInfo, FIELD_OFFSET(FILE_ALL_INFORMATION, NameInformation.FileName),
      ...
      case FileNameInformation:
      {
        ...
        if(!PrepareOutputHelper(RequestContext->Irp, &nameInfo, FIELD_OFFSET(FILE_NAME_INFORMATION, FileName),
                                /*SetInformationOnFailure=*/FALSE))
        {
          status = STATUS_BUFFER_TOO_SMALL;
          __leave;
      ...
      case FileNetworkPhysicalNameInformation:
      {
        ...
        if(!PrepareOutputHelper(RequestContext->Irp, &netInfo, FIELD_OFFSET(FILE_NETWORK_PHYSICAL_NAME_INFORMATION, FileName),
                                /*SetInformationOnFailure=*/FALSE))
        {
        ...
        if(!AppendVarSizeOutputString(RequestContext->Irp, &netInfo->FileName, &fcb->FileName,
                                      /*UpdateInformationOnFailure=*/FALSE,
      ...
      default: DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported FileInfoClass %x", infoClass);
    }
    ...
    eventContext->Operation.File.BufferLength = RequestContext->IrpSp->Parameters.QueryFile.Length;

    // copy file name to EventContext from FCB
    eventContext->Operation.File.FileNameLength = fcb->FileName.Length;
    RtlCopyMemory(eventContext->Operation.File.FileName, fcb->FileName.Buffer, fcb->FileName.Length);
...
VOID DokanCompleteQueryInformation(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ULONG bufferLen = 0;
  PVOID buffer    = NULL;
  PDokanCCB ccb;

  DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p", RequestContext->IrpSp->FileObject);
  ...
  else
  {
    ...
    if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status) && (RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass == FileAllInformation || RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass == FileStandardInformation ||
                                                            RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass == FileNetworkOpenInformation))
    {
      FSRTL_ADVANCED_FCB_HEADER* header = RequestContext->IrpSp->FileObject->FsContext;
      LONGLONG allocationSize           = 0;
      LONGLONG fileSize                 = 0;

      ASSERT(header != NULL);

      if(RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass == FileAllInformation)
      {
        ...
        allInfo->PositionInformation.CurrentByteOffset = RequestContext->IrpSp->FileObject->CurrentByteOffset;

        DokanFCBLockRO(ccb->Fcb);
        RequestContext->Irp->IoStatus.Status = FillNameInformation(RequestContext, ccb->Fcb, &allInfo->NameInformation);
        DokanFCBUnlock(ccb->Fcb);
      }
      else if(RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass == FileStandardInformation)
      {
      ...
      else if(RequestContext->IrpSp->Parameters.QueryFile.FileInformationClass == FileNetworkOpenInformation)
      {
      ...
      DOKAN_LOG_FINE_IRP(RequestContext, "AllocationSize: %llu, EndOfFile: %llu", allocationSize, fileSize);
    }
  }
}

VOID FlushFcb(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB Fcb, __in_opt PFILE_OBJECT FileObject)
{
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "MmFlushImageSection FCB=%p FileCount=%lu.", Fcb, Fcb->FileCount);
    MmFlushImageSection(&Fcb->SectionObjectPointers, MmFlushForWrite);
    DOKAN_LOG_FINE_IRP(RequestContext, "MmFlushImageSection done FCB=%p FileCount=%lu.", Fcb, Fcb->FileCount);
  }

  if(Fcb->SectionObjectPointers.DataSectionObject != NULL)
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "CcFlushCache FCB=%p FileCount=%lu.", Fcb, Fcb->FileCount);
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "CcFlushCache done FCB=%p FileCount=%lu.", Fcb, Fcb->FileCount);
  }
}

VOID FlushIfDescendant(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB FcbRelatedTo, __in PDokanFCB Fcb)
{
  ...
  DOKAN_LOG_FINE_IRP(RequestContext, "Check \"%wZ\" if is related to \"%wZ\"", &Fcb->FileName, &FcbRelatedTo->FileName);

  if(!StartsWith(&Fcb->FileName, &FcbRelatedTo->FileName))
  {
    return;
  }
  DOKAN_LOG_FINE_IRP(RequestContext, "Flush \"%wZ\" if it is possible.", &Fcb->FileName);
  FlushFcb(RequestContext, Fcb, NULL);
}

VOID FlushAllCachedFcb(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB FcbRelatedTo, __in_opt PFILE_OBJECT FileObject)
{
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "FlushAllCachedFcb file passed in. Flush only: FCB=%p FileObject=%p.", FcbRelatedTo, FileObject);
  ...
  for(PDokanFCB* fcbInTable = (PDokanFCB*)RtlEnumerateGenericTableAvl(&RequestContext->Vcb->FcbTable, /*Restart=*/TRUE); fcbInTable != NULL; fcbInTable = (PDokanFCB*)RtlEnumerateGenericTableAvl(&RequestContext->Vcb->FcbTable, /*Restart=*/FALSE))
  {
...
ULONG PopulateRenameEventInformations(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB Fcb, __in_opt PEVENT_CONTEXT EventContext, __in_opt PDokanFCB RootDirectoryFcb)
{
  ULONG fileNameLength          = 0;
  PFILE_OBJECT targetFileObject = RequestContext->IrpSp->Parameters.SetFile.FileObject;
  ...
  PFILE_RENAME_INFORMATION renameInfo     = (PFILE_RENAME_INFORMATION)RequestContext->Irp->AssociatedIrp.SystemBuffer;
  PDOKAN_RENAME_INFORMATION renameContext = NULL;

  if(EventContext)
  {
    renameContext                  = (PDOKAN_RENAME_INFORMATION)((PCHAR)EventContext + EventContext->Operation.SetFile.BufferOffset);
  ...
  while(renameInfoFileNameLength > sizeof(WCHAR) && renameInfo->FileName[renameInfoFileNameLength / sizeof(WCHAR) - 1] == L'\\')
  ...
  if(targetFileObject == NULL)
  {
    ...
    BOOLEAN isAlternateRename   = renameInfo->FileNameLength > sizeof(WCHAR) && renameInfo->FileName[0] == L':';
    WCHAR targetWchar           = isAlternateRename ? L':' : L'\\';
    BOOLEAN isIgnoreTargetWchar = !isAlternateRename;
    fcbFileNamePos              = DokanSearchWcharinUnicodeStringWithUlong(&Fcb->FileName, targetWchar, fcbFileNamePos, isIgnoreTargetWchar);
    ...
    if(renameContext)
    {
      RtlCopyMemory(renameContext->FileName, Fcb->FileName.Buffer, fcbFileNamePos * sizeof(WCHAR));
      RtlCopyMemory(renameContext->FileName + fcbFileNamePos, renameInfo->FileName, renameInfoFileNameLength);
    }
  }
  ...
  else
  {
    ...
    BOOLEAN addSeparator         = rootFileName->Buffer[rootFileName->Length / sizeof(WCHAR) - 1] != L'\\';
    fileNameLength               = rootFileName->Length + (addSeparator ? sizeof(WCHAR) : 0);
    if(renameContext)
    {
      RtlCopyMemory(renameContext->FileName, rootFileName->Buffer, rootFileName->Length);
      if(addSeparator)
      {
        renameContext->FileName[rootFileName->Length / sizeof(WCHAR)] = L'\\';
      }
      RtlCopyMemory((PCHAR)renameContext->FileName + fileNameLength, renameInfo->FileName, renameInfoFileNameLength);
    }
  ...
  if(renameContext)
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "Rename: \"%wZ\" => \"%ls\", Fcb=%p FileCount = %u", Fcb->FileName, renameContext->FileName, Fcb, (ULONG)Fcb->FileCount);
  }
...
DokanDispatchSetInformation(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  {
    fileObject = RequestContext->IrpSp->FileObject;
    DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p InfoClass=%s", fileObject, DokanGetFileInformationClassStr(RequestContext->IrpSp->Parameters.SetFile.FileInformationClass));
    ...
    switch(RequestContext->IrpSp->Parameters.SetFile.FileInformationClass)
    {
      case FileAllocationInformation:
      {
        ...
        {
          LARGE_INTEGER AllocationSize = ((PFILE_ALLOCATION_INFORMATION)buffer)->AllocationSize;
          if(AllocationSize.QuadPart < fcb->AdvancedFCBHeader.AllocationSize.QuadPart && !MmCanFileBeTruncated(fileObject->SectionObjectPointer, &AllocationSize))
          {
            status = STATUS_USER_MAPPED_FILE;
            __leave;
          }
          DOKAN_LOG_FINE_IRP(RequestContext, "AllocationSize %lld", ((PFILE_ALLOCATION_INFORMATION)buffer)->AllocationSize.QuadPart);
        }
      }
      break;
      case FileEndOfFileInformation:
      {
        if((fileObject->SectionObjectPointer != NULL) && (fileObject->SectionObjectPointer->DataSectionObject != NULL))
        {
          PFILE_END_OF_FILE_INFORMATION pInfoEoF = (PFILE_END_OF_FILE_INFORMATION)buffer;

          if(pInfoEoF->EndOfFile.QuadPart < fcb->AdvancedFCBHeader.FileSize.QuadPart && !MmCanFileBeTruncated(fileObject->SectionObjectPointer, &pInfoEoF->EndOfFile))
          {
            status = STATUS_USER_MAPPED_FILE;
            __leave;
          }
        ...
        DOKAN_LOG_FINE_IRP(RequestContext, "EndOfFile %lld", ((PFILE_END_OF_FILE_INFORMATION)buffer)->EndOfFile.QuadPart);
      }
      break;
      case FilePositionInformation:
      {
        ...
        posInfo = (PFILE_POSITION_INFORMATION)RequestContext->Irp->AssociatedIrp.SystemBuffer;
        ASSERT(posInfo != NULL);

        DOKAN_LOG_FINE_IRP(RequestContext, "FilePositionInformation %lld", posInfo->CurrentByteOffset.QuadPart);
      ...
      case FileRenameInformationEx:
        ...
        if(targetFileObject != NULL)
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "FileObject Specified so perform flush %p \"%wZ\"", targetFileObject, &(targetFileObject->FileName));
        ...
        break;
      default: DOKAN_LOG_FINE_IRP(RequestContext, "Unsupported FileInfoClass %x", RequestContext->IrpSp->Parameters.SetFile.FileInformationClass);
    }
    ...
    eventLength = sizeof(EVENT_CONTEXT) + fcb->FileName.Length;
    if(RequestContext->IrpSp->Parameters.SetFile.Length > MAXULONG - eventLength)
    {
    ...
    if(isRename)
    {
      ...
      {
        // Relative rename
        status = ObReferenceObjectByHandle(renameInfo->RootDirectory, STANDARD_RIGHTS_READ, *IoFileObjectType, KernelMode, (PVOID*)&rootDirObject, NULL);
        if(!NT_SUCCESS(status))
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "Failed to get RootDirectory object - %s", DokanGetNTSTATUSStr(status));
          __leave;
        }
        ...
        if(DokanCheckCCB(RequestContext, rootDirectoryCcb))
        {
          ...
          DOKAN_LOG_FINE_IRP(RequestContext, "RootDirectory FCB %p \"%wZ\"", rootDirectoryFcb, rootDirectoryFcb->FileName);
          ...
          // invalid.
          if(renameInfo->FileNameLength >= sizeof(WCHAR) && renameInfo->FileName[0] == L':')
          {
            status = STATUS_INVALID_PARAMETER;
            __leave;
          }
      ...
      ULONG renameFileNameLength = PopulateRenameEventInformations(RequestContext, fcb, NULL, rootDirectoryFcb);
      if(renameFileNameLength > MAXULONG - eventLength)
      {
        DOKAN_LOG_FINE_IRP(RequestContext, "Invalid destination FileName length %lu", renameFileNameLength);
        status = STATUS_INSUFFICIENT_RESOURCES;
        __leave;
      }
    ...
    eventContext->Operation.SetFile.FileInformationClass = RequestContext->IrpSp->Parameters.SetFile.FileInformationClass;

    // the size of FileInformation
    eventContext->Operation.SetFile.BufferLength = RequestContext->IrpSp->Parameters.SetFile.Length;

    // the offset from begining of structure to fill FileInfo
    eventContext->Operation.SetFile.BufferOffset = FIELD_OFFSET(EVENT_CONTEXT, Operation.SetFile.FileName[0]) + fcb->FileName.Length + sizeof(WCHAR); // the last null char

    if(isRename)
    {
      PopulateRenameEventInformations(RequestContext, fcb, eventContext, rootDirectoryFcb);
    }
    else
    {
      // copy FileInformation
      RtlCopyMemory((PCHAR)eventContext + eventContext->Operation.SetFile.BufferOffset, RequestContext->Irp->AssociatedIrp.SystemBuffer, RequestContext->IrpSp->Parameters.SetFile.Length);
    }

    // copy the file name
    eventContext->Operation.SetFile.FileNameLength = fcb->FileName.Length;
    RtlCopyMemory(eventContext->Operation.SetFile.FileName, fcb->FileName.Buffer, fcb->FileName.Length);

    // FsRtlCheckOpLock is called with non-NULL completion routine - not blocking.
    status = DokanCheckOplock(fcb, RequestContext->Irp, eventContext, DokanOplockComplete, DokanPrePostIrp);
    //
...
BOOLEAN IsInSameDirectory(PUNICODE_STRING fileName1, PUNICODE_STRING fileName2)
{
...
VOID DokanCompleteSetInformation(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  PDokanCCB ccb;
  PDokanFCB fcb = NULL;
  ...
  __try
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p infoClass=%s", RequestContext->IrpSp->FileObject, DokanGetFileInformationClassStr(infoClass));
    ...
    if(!(RequestContext->Irp->Flags & IRP_PAGING_IO))
    {
      ...
      if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status) && infoClass == FileRenameInformation)
      {
    ...
    switch(infoClass)
    {
      ...
      {
        // Process rename
        oldFileName = DokanWrapUnicodeString(fcb->FileName.Buffer, fcb->FileName.Length);
        // Copy new file name
        ...
        RtlCopyMemory(buffer, EventInfo->Buffer, EventInfo->BufferLength);
        DokanRenameFcb(RequestContext, fcb, buffer, (USHORT)EventInfo->BufferLength);
        DOKAN_LOG_FINE_IRP(RequestContext, "Fcb=%p renamed \"%wZ\"", fcb, &fcb->FileName);
        break;
      }
    }
    ...
    switch(infoClass)
    {
      case FileAllocationInformation: DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_SIZE, FILE_ACTION_MODIFIED); break;
      case FileBasicInformation: DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_LAST_ACCESS | FILE_NOTIFY_CHANGE_CREATION, FILE_ACTION_MODIFIED); break;
      case FileEndOfFileInformation: DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_SIZE, FILE_ACTION_MODIFIED); break;
      case FileRenameInformation:
      case FileRenameInformationEx:
      {
        if(IsInSameDirectory(&oldFileName, &fcb->FileName))
        {
          DokanNotifyReportChange0(RequestContext, fcb, &oldFileName, DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY) ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_RENAMED_OLD_NAME);
          DokanNotifyReportChange(RequestContext, fcb, DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY) ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_RENAMED_NEW_NAME);
        }
        else
        {
          DokanNotifyReportChange0(RequestContext, fcb, &oldFileName, DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY) ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_REMOVED);
          DokanNotifyReportChange(RequestContext, fcb, DokanFCBFlagsIsSet(fcb, DOKAN_FILE_DIRECTORY) ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_ADDED);
        }
        // free old file name
        ExFreePool(oldFileName.Buffer);
      }
      break;
      case FileValidDataLengthInformation: DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_SIZE, FILE_ACTION_MODIFIED); break;
    }
  }
  __finally
  {
