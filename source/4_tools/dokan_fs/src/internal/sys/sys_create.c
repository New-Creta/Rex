...
static const UNICODE_STRING systemVolumeInformationFileName = RTL_CONSTANT_STRING(L"\\System Volume Information");
...
VOID DokanMaybeBackOutAtomicOplockRequest(__in PDokanCCB Ccb, __in PIRP Irp)
{
  ...
  {
    FsRtlCheckOplockEx(DokanGetFcbOplock(Ccb->Fcb), Irp, OPLOCK_FLAG_BACK_OUT_ATOMIC_OPLOCK, NULL, NULL, NULL);
...
DokanFreeCCB(__in PREQUEST_CONTEXT RequestContext, __in PDokanCCB ccb)
{
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "WARNING. &ccb->NextCCB is empty. This should never happen, "
...
NTSTATUS DokanGetParentDir(const __in WCHAR* fileName, __out WCHAR** parentDir, __out ULONG* parentDirLength)
{
 ...
 */
void FixFileNameForReparseMountPoint(const __in UNICODE_STRING* MountPoint, __in PIRP Irp, __in PIO_STACK_LOCATION IrpSp)
{
  ...
  if(!NT_SUCCESS(FsRtlFindExtraCreateParameter(ecpList, &iopSymlinkECPGuid, (void**)&ecpContext, 0)))
  {
    return;
  }
  if(FsRtlIsEcpFromUserMode(ecpContext) || !ecpContext->FlagsMountPoint.MountPoint.MountPoint)
  {
    return;
  }
  ...
  PWSTR unparsedNameInFileObject            = (PWSTR)RtlOffsetToPointer(FileName->Buffer, fileNameLength - unparsedNameLength);
  UNICODE_STRING unparsedNameInFileObjectUS = DokanWrapUnicodeString(unparsedNameInFileObject, unparsedNameLength);

  PWSTR unparsedNameInEcp            = (PWSTR)RtlOffsetToPointer(ecpContext->Name.Buffer, ecpNameLength - unparsedNameLength);
  UNICODE_STRING unparsedNameInEcpUS = DokanWrapUnicodeString(unparsedNameInEcp, unparsedNameLength);
...
{
  return (FileName.Length > 0 && (RtlEqualUnicodeString(&FileName, &g_KeepAliveFileName, FALSE) || RtlEqualUnicodeString(&FileName, &g_NotificationFileName, FALSE)));
}

NTSTATUS
DokanCheckShareAccess(_In_ PREQUEST_CONTEXT RequestContext, _In_ PFILE_OBJECT FileObject, _In_ PDokanFCB FcbOrDcb, _In_ ACCESS_MASK DesiredAccess, _In_ ULONG ShareAccess)
...

{
  ...
  if((FcbOrDcb->Identifier.Type == FCB) && !FlagOn(ShareAccess, FILE_SHARE_DELETE) && DokanFCBFlagsIsSet(FcbOrDcb, DOKAN_DELETE_ON_CLOSE))
  ...
  //
  if((FcbOrDcb->Identifier.Type == FCB) && !FlagOn(ShareAccess, FILE_SHARE_WRITE) && FlagOn(DesiredAccess, FILE_EXECUTE | FILE_READ_DATA | FILE_WRITE_DATA | FILE_APPEND_DATA | DELETE | MAXIMUM_ALLOWED) &&
  ...
  status = IoCheckShareAccess(DesiredAccess, ShareAccess, FileObject, &FcbOrDcb->ShareAccess, FALSE);
...
VOID DokanRetryCreateAfterOplockBreak(__in PVOID Context, __in PIRP Irp)
{
  ...
  NTSTATUS status = DokanBuildRequestContext((PDEVICE_OBJECT)Context, Irp,
                                             /*IsTopLevelIrp=*/FALSE, &requestContext);
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("Failed to build request context for IRP=%p Status=%s", Irp, DokanGetNTSTATUSStr(status));
    return;
  }
...
--*/
{
  ...
  DOKAN_INIT_LOGGER(logger, RequestContext->DeviceObject->DriverObject, IRP_MJ_CREATE);
  ...
  __try
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext,
                       ...
                       fileObject, relatedFileObject, &fileObject->FileName, RequestContext->IrpSp->Flags, RequestContext->IrpSp->Parameters.Create.SecurityContext->DesiredAccess, RequestContext->IrpSp->Parameters.Create.Options,
                       RequestContext->IrpSp->Parameters.Create.FileAttributes, RequestContext->IrpSp->Parameters.Create.ShareAccess);

    disposition = (RequestContext->IrpSp->Parameters.Create.Options >> 24) & 0x000000ff;
    ...
    FixFileNameForReparseMountPoint(RequestContext->Dcb->MountPoint, RequestContext->Irp, RequestContext->IrpSp);

    BOOLEAN isNetworkFileSystem = (RequestContext->Dcb->VolumeDeviceType == FILE_DEVICE_NETWORK_FILE_SYSTEM);
    ...
    if(!RequestContext->Vcb->HasEventWait)
    {
      ...
      else
      {
        ...
        DOKAN_LOG_FINE_IRP(RequestContext, "Here we only go in if some antivirus software tries to "
                                           "create files before startup is finished.");
        if(fileObject->FileName.Length > 0)
        {
          DOKAN_LOG_FINE_IRP(RequestContext, "Verify if the system tries to access System Volume");
          if(StartsWith(&fileObject->FileName, &systemVolumeInformationFileName))
          {
            DOKAN_LOG_FINE_IRP(RequestContext, "It's an access to System Volume, so don't return "
                                               "SUCCESS. We don't have one.");
            status = STATUS_NO_SUCH_FILE;
            __leave;
          }
        }
        DokanLogInfo(&logger, L"Handle created before IOCTL_EVENT_WAIT for file \"%wZ\"", &fileObject->FileName);
        status = STATUS_SUCCESS;
        __leave;
      }
    }
    ...
    if((fileObject->FileName.Length > sizeof(WCHAR)) && (fileObject->FileName.Buffer[1] == L'\\') && (fileObject->FileName.Buffer[0] == L'\\'))
    {
    ...
    if(relatedFileObject != NULL && relatedFileObject->FsContext2)
    {
      ...
      if(relatedCcb->Fcb)
      {
        ...
        if(relatedFcb->FileName.Length > 0 && relatedFcb->FileName.Buffer != NULL)
        {
          ...
          relatedFileName->Buffer = DokanAlloc(relatedFcb->FileName.MaximumLength);
    ...
    if(relatedFileName == NULL && fileObject->FileName.Length == 0)
    {
      ...
      if(RequestContext->IrpSp->Parameters.Create.Options & FILE_DIRECTORY_FILE)
      {
    ...
    if(fileObject->FileName.Length > sizeof(WCHAR) && fileObject->FileName.Buffer[fileObject->FileName.Length / sizeof(WCHAR) - 1] == L'\\')
    {
      fileObject->FileName.Length -= sizeof(WCHAR);
    }
    ...
    if(relatedFileName != NULL)
    {
      ...
      if(fileObject->FileName.Length > 0 && fileObject->FileName.Buffer[0] == '\\')
      {
        DOKAN_LOG_FINE_IRP(RequestContext, "When RelatedFileObject is specified, the file name should "
                                           "be relative path");
        status = STATUS_INVALID_PARAMETER;
        __leave;
      }
      if(relatedFileName->Length > 0 && fileObject->FileName.Length > 0 && relatedFileName->Buffer[relatedFileName->Length / sizeof(WCHAR) - 1] != '\\' && fileObject->FileName.Buffer[0] != ':')
      {
      ...
      if(relatedFileName->Length / sizeof(WCHAR) == 1 && fileObject->FileName.Length > 0 && relatedFileName->Buffer[0] == '\\' && fileObject->FileName.Buffer[0] == ':')
      {
    ...
    if(!RequestContext->Dcb->UseAltStream && DokanSearchUnicodeStringChar(&fileObject->FileName, L':') != -1)
    {
    ...
    if(relatedFileName != NULL && !alternateDataStreamOfRootDir)
    {
      ...
      RtlCopyMemory((PCHAR)fileName + relatedFileName->Length + (needBackSlashAfterRelatedFile ? sizeof(WCHAR) : 0), fileObject->FileName.Buffer, fileObject->FileName.Length);
    ...
    else
    {
      ...
      // object
      RtlCopyMemory(fileName, fileObject->FileName.Buffer, fileObject->FileName.Length);
    }

    // Remove possible UNCName prefix
    if(RequestContext->Dcb->UNCName != NULL)
    {
      UNICODE_STRING fileNameUS = DokanWrapUnicodeString(fileName, fileNameLength);
                                ...
                                /*CaseInSensitive=*/TRUE))
      {
        ...
        {
          RtlMoveMemory(fileName, (PCHAR)fileName + RequestContext->Dcb->UNCName->Length, fileNameLength);
    ...
    if(IS_DEVICE_READ_ONLY(RequestContext->DeviceObject) &&
       ((disposition == FILE_SUPERSEDE) || (disposition == FILE_CREATE) || (disposition == FILE_OVERWRITE) || (disposition == FILE_OVERWRITE_IF) || (RequestContext->IrpSp->Parameters.Create.Options & FILE_DELETE_ON_CLOSE)))
    {
    ...
    if(fileObject->FsContext2 != NULL)
    {
      ...
      if(GetIdentifierType(ccb) == CCB && (DokanCCBFlagsIsSet(ccb, DOKAN_RETRY_CREATE)))
      {
    ...
    if(allocateCcb)
    {
      ...
      {
        DokanLogInfo(&logger, L"Opened file with user mode dispatch blocked: \"%wZ\"", &fileObject->FileName);
      }
    ...
    if(FlagOn(RequestContext->IrpSp->Parameters.Create.Options, FILE_DIRECTORY_FILE) && FlagOn(RequestContext->IrpSp->Parameters.Create.FileAttributes, FILE_ATTRIBUTE_TEMPORARY) && (FILE_CREATE == disposition || FILE_OPEN_IF == disposition))
    {
      status = STATUS_INVALID_PARAMETER;
      __leave;
    }
    ...
    if(RequestContext->IrpSp->Parameters.Create.Options & FILE_OPEN_FOR_BACKUP_INTENT)
    {
    ...
    if(fcb->IsKeepalive)
    {
      DokanLogInfo(&logger, L"Opened keepalive file from process %d.", RequestContext->ProcessId);
    }
    if(fcb->BlockUserModeDispatch)
    {
      RequestContext->Irp->IoStatus.Information = FILE_OPENED;
    ...
    {
      if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->SecurityDescriptor)
      {
        // (CreateOptions & FILE_DIRECTORY_FILE) == FILE_DIRECTORY_FILE
        if(SeAssignSecurity(NULL, // we don't keep track of parents, this will have to be
                                  // handled in user mode
                            RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->SecurityDescriptor, &newFileSecurityDescriptor,
                            (RequestContext->IrpSp->Parameters.Create.Options & FILE_DIRECTORY_FILE) || (RequestContext->IrpSp->Flags & SL_OPEN_TARGET_DIRECTORY),
                            &RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->SubjectSecurityContext, IoGetFileObjectGenericMapping(), PagedPool) == STATUS_SUCCESS)
        {
      ...
      if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length > 0)
      {
        // add 1 WCHAR for NULL
        alignedObjectNameSize = PointerAlignSize(sizeof(DOKAN_UNICODE_STRING_INTERMEDIATE) + RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length + sizeof(WCHAR));
      }
      // else alignedObjectNameSize =
      // PointerAlignSize(sizeof(DOKAN_UNICODE_STRING_INTERMEDIATE)) SEE
      // DECLARATION

      if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Length > 0)
      {
        // add 1 WCHAR for NULL
        alignedObjectTypeNameSize = PointerAlignSize(sizeof(DOKAN_UNICODE_STRING_INTERMEDIATE) + RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Length + sizeof(WCHAR));
      }
    ...
    if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState)
    {
      ...
      eventContext->Operation.Create.SecurityContext.AccessState.SecurityEvaluated       = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->SecurityEvaluated;
      eventContext->Operation.Create.SecurityContext.AccessState.GenerateAudit           = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->GenerateAudit;
      eventContext->Operation.Create.SecurityContext.AccessState.GenerateOnClose         = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->GenerateOnClose;
      eventContext->Operation.Create.SecurityContext.AccessState.AuditPrivileges         = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->AuditPrivileges;
      eventContext->Operation.Create.SecurityContext.AccessState.Flags                   = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->Flags;
      eventContext->Operation.Create.SecurityContext.AccessState.RemainingDesiredAccess  = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->RemainingDesiredAccess;
      eventContext->Operation.Create.SecurityContext.AccessState.PreviouslyGrantedAccess = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->PreviouslyGrantedAccess;
      eventContext->Operation.Create.SecurityContext.AccessState.OriginalDesiredAccess   = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->OriginalDesiredAccess;
    ...
    OplockDebugRecordCreateRequest(fcb, RequestContext->IrpSp->Parameters.Create.SecurityContext->DesiredAccess, RequestContext->IrpSp->Parameters.Create.ShareAccess);

    // Other SecurityContext attributes
    eventContext->Operation.Create.SecurityContext.DesiredAccess = RequestContext->IrpSp->Parameters.Create.SecurityContext->DesiredAccess;

    // Other Create attributes
    eventContext->Operation.Create.FileAttributes = RequestContext->IrpSp->Parameters.Create.FileAttributes;
    eventContext->Operation.Create.CreateOptions  = RequestContext->IrpSp->Parameters.Create.Options;
    if(IS_DEVICE_READ_ONLY(RequestContext->DeviceObject) && // do not reorder eval as
    ...
    eventContext->Operation.Create.ShareAccess    = RequestContext->IrpSp->Parameters.Create.ShareAccess;
    ...
    if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState)
    {
      ...
      intermediateUnicodeStr->Length        = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length;
      intermediateUnicodeStr->MaximumLength = (USHORT)alignedObjectNameSize;

      if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length > 0)
      {
        RtlCopyMemory(intermediateUnicodeStr->Buffer, RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Buffer, RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectName.Length);
      ...
      intermediateUnicodeStr->Length        = RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Length;
      intermediateUnicodeStr->MaximumLength = (USHORT)alignedObjectTypeNameSize;

      if(RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Length > 0)
      {
        RtlCopyMemory(intermediateUnicodeStr->Buffer, RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Buffer, RequestContext->IrpSp->Parameters.Create.SecurityContext->AccessState->ObjectTypeName.Length);
    ...
    RtlCopyMemory(((char*)&eventContext->Operation.Create + eventContext->Operation.Create.FileNameOffset), parentDir ? fileName : fcb->FileName.Buffer, parentDir ? fileNameLength : fcb->FileName.Length);
    *(PWCHAR)((char*)&eventContext->Operation.Create + eventContext->Operation.Create.FileNameOffset + (parentDir ? fileNameLength : fcb->FileName.Length)) = 0;
    ...
    openRequiringOplock = BooleanFlagOn(RequestContext->IrpSp->Parameters.Create.Options, FILE_OPEN_REQUIRING_OPLOCK);
    if(FlagOn(RequestContext->IrpSp->Parameters.Create.Options, FILE_COMPLETE_IF_OPLOCKED))
    {
    ...
    if(fcb->FileCount > 1)
    {
      ...
      status = DokanCheckShareAccess(RequestContext, fileObject, fcb, eventContext->Operation.Create.SecurityContext.DesiredAccess, eventContext->Operation.Create.ShareAccess);

      if(!NT_SUCCESS(status))
      {
        DOKAN_LOG_FINE_IRP(RequestContext, "DokanCheckShareAccess failed with 0x%x %s", status, DokanGetNTSTATUSStr(status));
        ...
        //
        if((status == STATUS_SHARING_VIOLATION) && !FlagOn(RequestContext->IrpSp->Parameters.Create.Options, FILE_COMPLETE_IF_OPLOCKED))
        {
          POPLOCK oplock = DokanGetFcbOplock(fcb);
          // This may enter a wait state!

          OplockDebugRecordFlag(fcb, DOKAN_OPLOCK_DEBUG_EXPLICIT_BREAK_IN_CREATE);
          OplockDebugRecordProcess(fcb);

          OplockBreakStatus = FsRtlOplockBreakH(oplock, RequestContext->Irp, 0, RequestContext->DeviceObject, DokanRetryCreateAfterOplockBreak, DokanPrePostIrp);
          ...
          {
            DOKAN_LOG_FINE_IRP(RequestContext, "FsRtlOplockBreakH returned 0x%x %s", OplockBreakStatus, DokanGetNTSTATUSStr(OplockBreakStatus));
          ...
          {
            status = DokanCheckShareAccess(RequestContext, fileObject, fcb, eventContext->Operation.Create.SecurityContext.DesiredAccess, eventContext->Operation.Create.ShareAccess);
            DOKAN_LOG_FINE_IRP(RequestContext, "Checked share access again, status = 0x%08x", status);
        ...
        else
        {
          if(status == STATUS_SHARING_VIOLATION && FlagOn(RequestContext->IrpSp->Parameters.Create.Options, FILE_COMPLETE_IF_OPLOCKED))
          {
            DOKAN_LOG_FINE_IRP(RequestContext, "Failing a create with FILE_COMPLETE_IF_OPLOCKED because "
                                               "of sharing violation");
          }

          DOKAN_LOG_FINE_IRP(RequestContext, "Create: sharing/oplock failed, status = 0x%08x", status);
          __leave;
        }
      }
    ...
    {
      IoSetShareAccess(eventContext->Operation.Create.SecurityContext.DesiredAccess, eventContext->Operation.Create.ShareAccess, fileObject, &fcb->ShareAccess);
    }
    ...
    if(fcb->FileCount > 1)
    {
      status = FsRtlCheckOplock(DokanGetFcbOplock(fcb), RequestContext->Irp, RequestContext->DeviceObject, DokanRetryCreateAfterOplockBreak, DokanPrePostIrp);
    ...
    status = FsRtlCheckOplockEx(DokanGetFcbOplock(fcb), RequestContext->Irp, OPLOCK_FLAG_OPLOCK_KEY_CHECK_ONLY, NULL, NULL, NULL);

    if(!NT_SUCCESS(status))
    {
      DOKAN_LOG_FINE_IRP(RequestContext, "FsRtlCheckOplockEx return status = 0x%08x", status);
      __leave;
    }
    ...
    if(openRequiringOplock)
    {
      ...
      if((status == STATUS_SUCCESS))
      {
        status = FsRtlOplockFsctrl(DokanGetFcbOplock(fcb), RequestContext->Irp, fcb->FileCount);
      }
...
VOID DokanCompleteCreate(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  PDokanCCB ccb = NULL;
  PDokanFCB fcb = NULL;
  ...
  RequestContext->Irp->IoStatus.Information = EventInfo->Operation.Create.Information;
  DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p CreateInformation=%s", RequestContext->IrpSp->FileObject, DokanGetCreateInformationStr(RequestContext->Irp->IoStatus.Information));
  ...
  if((RequestContext->Irp->IoStatus.Information == FILE_DOES_NOT_EXIST) && (IS_DEVICE_READ_ONLY(RequestContext->IrpSp->DeviceObject)))
  {
    DWORD disposition = (RequestContext->IrpSp->Parameters.Create.Options >> 24) & 0x000000ff;
  ...
  if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status) && (RequestContext->IrpSp->Parameters.Create.Options & FILE_DIRECTORY_FILE || EventInfo->Operation.Create.Flags & DOKAN_FILE_DIRECTORY))
  {
    if(RequestContext->IrpSp->Parameters.Create.Options & FILE_DIRECTORY_FILE)
    {
  ...
  if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status) && RequestContext->IrpSp->Parameters.Create.Options & FILE_DELETE_ON_CLOSE)
  {
    DokanFCBFlagsSetBit(fcb, DOKAN_DELETE_ON_CLOSE);
    DokanCCBFlagsSetBit(ccb, DOKAN_DELETE_ON_CLOSE);
    DOKAN_LOG_FINE_IRP(RequestContext, "FILE_DELETE_ON_CLOSE is set so remember for delete in cleanup");
  }
  ...
  if(NT_SUCCESS(RequestContext->Irp->IoStatus.Status))
  {
    if(RequestContext->Irp->IoStatus.Information == FILE_CREATED)
    {
      ...
      {
        DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_DIR_NAME, FILE_ACTION_ADDED);
      }
      else
      {
        DokanNotifyReportChange(RequestContext, fcb, FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_ADDED);
      }
    }
    ccb->AtomicOplockRequestPending = FALSE;
  }
