...
DokanGetAccessToken(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  __try
  {
    ...
    for(thisEntry = listHead->Flink; thisEntry != listHead; thisEntry = nextEntry)
    {
      ...
      {
        accessState = irpEntry->RequestContext.IrpSp->Parameters.Create.SecurityContext->AccessState;
      }
      break;
    }
    ...
    {
      DOKAN_LOG_FINE_IRP(RequestContext, "ObOpenObjectByPointer failed: 0x%x %s", status, DokanGetNTSTATUSStr(status));
      __leave;
    }
