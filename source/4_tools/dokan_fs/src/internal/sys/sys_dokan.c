...
#include "dokan_fs/internal/sys/util/str.h"
...
BOOLEAN
DokanFastIoCheckIfPossible(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in ULONG Length, __in BOOLEAN Wait, __in ULONG LockKey, __in BOOLEAN CheckForReadOperation, __out PIO_STATUS_BLOCK IoStatus, __in PDEVICE_OBJECT DeviceObject)
{
...
BOOLEAN
DokanFastIoRead(__in PFILE_OBJECT FileObject, __in PLARGE_INTEGER FileOffset, __in ULONG Length, __in BOOLEAN Wait, __in ULONG LockKey, __in PVOID Buffer, __out PIO_STATUS_BLOCK IoStatus, __in PDEVICE_OBJECT DeviceObject)
{
  UNREFERENCED_PARAMETER(FileObject);
  UNREFERENCED_PARAMETER(FileOffset);
  UNREFERENCED_PARAMETER(Length);
...
NTSTATUS DokanAcquireForCcFlush(__in PFILE_OBJECT FileObject, __in PDEVICE_OBJECT DeviceObject)
{
...
NTSTATUS DokanReleaseForCcFlush(__in PFILE_OBJECT FileObject, __in PDEVICE_OBJECT DeviceObject)
{
...
NTSTATUS
DokanFilterCallbackAcquireForCreateSection(__in PFS_FILTER_CALLBACK_DATA CallbackData, __out PVOID* CompletionContext)
{
  PDokanCCB ccb;
  ...
  if (ccb == NULL || CallbackData->Parameters.AcquireForSectionSynchronization.SyncType != SyncTypeCreateSection)
  {
...
DokanLookasideCreate(__in LOOKASIDE_LIST_EX* pCache, __in size_t cbElement)
{
  ...
  {
    DOKAN_LOG_("ExInitializeLookasideListEx failed, Status (0x%x) %s", Status, DokanGetNTSTATUSStr(Status));
    return FALSE;
  }
...
VOID InitMultiVersionResources()
{
  ...
  if (RtlIsNtDdiVersionAvailable(NTDDI_WIN8))
  {
    ...
    DokanFsRtlCheckLockForOplockRequest = (DokanPtr_FsRtlCheckLockForOplockRequest*)MmGetSystemRoutineAddress(&SystemRoutineName);
    RtlInitUnicodeString(&SystemRoutineName, L"FsRtlAreThereWaitingFileLocks");
    DokanFsRtlAreThereWaitingFileLocks = (DokanPtr_FsRtlAreThereWaitingFileLocks*)MmGetSystemRoutineAddress(&SystemRoutineName);
  }
}
...

{
...
#ifdef DEBUG_
  DOKAN_LOG_("ver.%x, %s %s", DOKAN_DRIVER_VERSION, __DATE__, __TIME__);
#endif
  ...
  ExInitializeNPagedLookasideList(&DokanIrpEntryLookasideList, NULL, NULL, 0, sizeof(IRP_ENTRY), TAG, 0);
  ...
  if (!NT_SUCCESS(status))
  {
    ...
    DOKAN_LOG_("  FsRtlRegisterFileSystemFilterCallbacks returned 0x%x %s", status, DokanGetNTSTATUSStr(status));
    return status;
  }
  ...
  if (!DokanLookasideCreate(&g_DokanEResourceLookasideList, sizeof(ERESOURCE)))
  {
  ...
  DOKAN_LOG_("%s FixFileNameForReparseMountPoint=%d", DokanGetNTSTATUSStr(status), g_FixFileNameForReparseMountPoint);
...
{
  return FsRtlCheckOplock(DokanGetFcbOplock(Fcb), Irp, Context, CompletionRoutine, PostIrpRoutine);
}
...
NTSTATUS DokanNotifyReportChange0(__in PREQUEST_CONTEXT RequestContext, __in PDokanFCB Fcb, __in PUNICODE_STRING FileName, __in ULONG FilterMatch, __in ULONG Action)
{
  ...
  if (DokanSearchUnicodeStringChar(FileName, L':') != -1)
  { // FileStream

    // Convert file action to stream action
    switch (Action)
    {
    case FILE_ACTION_ADDED: Action = FILE_ACTION_ADDED_STREAM; break;
    case FILE_ACTION_REMOVED: Action = FILE_ACTION_REMOVED_STREAM; break;
    case FILE_ACTION_MODIFIED: Action = FILE_ACTION_MODIFIED_STREAM; break;
    default: break;
    }
  ...
  __try
  {
    FsRtlNotifyFullReportChange(Fcb->Vcb->NotifySync, &Fcb->Vcb->DirNotifyList, (PSTRING)FileName, nameOffset,
      NULL, // StreamName
  ...
  __except (GetExceptionCode() == STATUS_ACCESS_VIOLATION ? EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH)
  {
    ...
    try
    {
      ...
      // happens.
      return DokanLogError(&logger, STATUS_OBJECT_NAME_INVALID, L"Access violation in file change notification for \"%wZ\".", FileName);
    }
    __except (GetExceptionCode() == STATUS_ACCESS_VIOLATION ? EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH)
    {
      // This is not a case we think ever happens, but we may as well not
      // crash if it does.
      return DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Access violation on the file name passed in a notification.");
    }
  }
  DOKAN_LOG_FINE_IRP(RequestContext, "FCB=%p NameOffset=%x FilterMatch=%x Action=%x Success", Fcb, nameOffset, FilterMatch, Action);
  return STATUS_SUCCESS;
}
...
{
  ASSERT(Fcb != NULL);
  return DokanNotifyReportChange0(RequestContext, Fcb, &Fcb->FileName, FilterMatch, Action);
}
...
DokanCheckCCB(__in PREQUEST_CONTEXT RequestContext, __in_opt PDokanCCB Ccb)
{
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "MountId is different: Ccb=%d Dcb=%d", Ccb->MountId, RequestContext->Dcb->MountId);
    return FALSE;
  }
...
DokanAllocateMdl(__in PREQUEST_CONTEXT RequestContext, __in ULONG Length)
{
  ...
  {
    RequestContext->Irp->MdlAddress = IoAllocateMdl(RequestContext->Irp->UserBuffer, Length, FALSE, FALSE, RequestContext->Irp);
    ...
    __try
    {
      MmProbeAndLockPages(RequestContext->Irp->MdlAddress, RequestContext->Irp->RequestorMode, IoWriteAccess);
    }
...
VOID DokanLockWarn(const __in ERESOURCE* Resource, const __in DokanResourceDebugInfo* DebugInfo, __in PDOKAN_LOGGER Logger, __in const char* Site, const __in UNICODE_STRING* ObjectName, __in const void* ObjectPointer)
{
  ...
  if (DebugInfo->ExclusiveOwnerThread != NULL)
  {
    DokanLogInfo(Logger,
      ...
      ObjectName, ObjectPointer, Resource, KeGetCurrentThread(), Site, DebugInfo->ExclusiveOwnerThread, DebugInfo->ExclusiveLockSite);
...
VOID DokanLockNotifyResolved(const __in ERESOURCE* Resource, __in PDOKAN_LOGGER Logger)
{
  DokanLogInfo(Logger, L"Blocking on ERESOURCE %I64x has resolved on thread %I64x", Resource, KeGetCurrentThread());
  DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_TRACE_LEVEL, "Blocking on ERESOURCE %I64x has resolved on thread %I64x\n", Resource, KeGetCurrentThread());
}

VOID DokanResourceLockWithDebugInfo(__in BOOLEAN Writable, __in PERESOURCE Resource, __in PDokanResourceDebugInfo DebugInfo, __in PDOKAN_LOGGER Logger, __in const char* Site, const __in UNICODE_STRING* ObjectName, __in const void* ObjectPointer)
{
  // The wait is in 100ns units. Negative means "from now" as opposed to an
  // absolute wake up time.
  LARGE_INTEGER wait = RtlConvertLongToLargeInteger(-DOKAN_RESOURCE_LOCK_DEBUG_INTERVAL_MSEC * 10);
  ...
  for (;;)
  {
    ...
    else if ((systemTime.QuadPart - lastWarnTime.QuadPart) / 10 >= DOKAN_RESOURCE_LOCK_WARNING_MSEC)
    {
      DokanLockWarn(Resource, DebugInfo, Logger, Site, ObjectName, ObjectPointer);
      warned = TRUE;
      lastWarnTime = systemTime;
    }
...
ULONG GetOplockControlDebugInfoBit(ULONG FsControlCode)
{
  ...
  {
  case FSCTL_REQUEST_OPLOCK_LEVEL_1: return 1;
  case FSCTL_REQUEST_OPLOCK_LEVEL_2: return 2;
  case FSCTL_REQUEST_BATCH_OPLOCK: return 4;
  case FSCTL_OPLOCK_BREAK_ACKNOWLEDGE: return 8;
  case FSCTL_OPBATCH_ACK_CLOSE_PENDING: return 16;
  case FSCTL_OPLOCK_BREAK_NOTIFY: return 32;
  case FSCTL_OPLOCK_BREAK_ACK_NO_2: return 64;
  case FSCTL_REQUEST_FILTER_OPLOCK: return 128;
  case FSCTL_REQUEST_OPLOCK: return 256;
...
{
  InterlockedOr((PLONG)&Fcb->OplockDebugInfo.MajorFunctionMask, (1 << MajorFunction));
}
...
{
  InterlockedOr64((PLONG64)&Fcb->OplockDebugInfo.OplockProcessMask, (LONG64)PsGetCurrentProcess());
}
...
void OplockDebugRecordRequest(__in PDokanFCB Fcb, __in ULONG FsControlMinorFunction, __in ULONG OplockLevel)
{
  ...
  InterlockedOr((PLONG)&Fcb->OplockDebugInfo.OplockFsctlMask, GetOplockControlDebugInfoBit(FsControlMinorFunction));
...
void OplockDebugRecordCreateRequest(__in PDokanFCB Fcb, __in ACCESS_MASK AccessMask, __in ULONG ShareAccess)
{
  ...
  InterlockedOr((PLONG)&Fcb->OplockDebugInfo.ShareAccessMask, (LONG)ShareAccess);
}
...
VOID RunAsSystem(_In_ PKSTART_ROUTINE StartRoutine, PVOID StartContext)
{
  ...
  NTSTATUS status = PsCreateSystemThread(&handle, THREAD_ALL_ACCESS, &objectAttribs, NULL, NULL, StartRoutine, StartContext);
  if (!NT_SUCCESS(status))
  {
    DOKAN_LOG_("PsCreateSystemThread failed: 0x%X %s", status, DokanGetNTSTATUSStr(status));
  }
  else
  {
