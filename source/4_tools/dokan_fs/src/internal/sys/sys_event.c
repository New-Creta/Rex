...
VOID DokanCreateIrpCancelRoutine(_Inout_ PDEVICE_OBJECT DeviceObject, _Inout_ _IRQL_uses_cancel_ PIRP Irp)
{
  ...
  PIRP_ENTRY irpEntry = Irp->Tail.Overlay.DriverContext[DRIVER_CONTEXT_IRP_ENTRY];
  if(irpEntry != NULL)
  {
...
VOID DokanIrpCancelRoutine(_Inout_ PDEVICE_OBJECT DeviceObject, _Inout_ _IRQL_uses_cancel_ PIRP Irp)
{
  KIRQL oldIrql;
  PIRP_ENTRY irpEntry;
  ...
  status = DokanBuildRequestContext(DeviceObject, Irp, /*IsTopLevelIrp=*/FALSE, &requestContext);
...
--*/
{
  ...
  REQUEST_CONTEXT requestContext;
  NTSTATUS status = DokanBuildRequestContext(irpSp->DeviceObject, Irp, /*IsTopLevelIrp=*/FALSE, &requestContext);
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("Failed to build request context for IRP=%p Status=%s", Irp, DokanGetNTSTATUSStr(status));
    return;
  ...
  DOKAN_LOG_FINE_IRP((&requestContext), "Oplock break completed %s", DokanGetNTSTATUSStr(Irp->IoStatus.Status));
...
VOID DokanPrePostIrp(IN PVOID Context, IN PIRP Irp)
{
  ...
  PIO_STACK_LOCATION irpSp = IoGetCurrentIrpStackLocation(Irp);
  REQUEST_CONTEXT requestContext;
  DokanBuildRequestContext(irpSp->DeviceObject, Irp, /*IsTopLevelIrp=*/FALSE, &requestContext);
  DOKAN_LOG_FINE_IRP((&requestContext), "Mark Irp pending");
  IoMarkIrpPending(Irp);
}

NTSTATUS
RegisterPendingIrpMain(__in PREQUEST_CONTEXT RequestContext, __in_opt PEVENT_CONTEXT EventContext, __in PIRP_LIST IrpList, __in ULONG CheckMount, __in NTSTATUS CurrentStatus)
{
  PIRP_ENTRY irpEntry;
  KIRQL oldIrql;

  if(RequestContext->Vcb && CheckMount && IsUnmountPendingVcb(RequestContext->Vcb))
  {
  ...
  if(EventContext)
  {
    EventContext->SerialNumber = InterlockedIncrement((LONG*)&RequestContext->Dcb->SerialNumber);
  ...
  RequestContext->Irp->Tail.Overlay.DriverContext[DRIVER_CONTEXT_IRP_ENTRY] = irpEntry;
...
NTSTATUS
DokanRegisterPendingIrp(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_CONTEXT EventContext)
{
  NTSTATUS status;
  ...
  // that match it.
  if(RequestContext->IrpSp->MajorFunction != IRP_MJ_WRITE && EventContext->Length > EVENT_CONTEXT_MAX_SIZE)
  {
    InterlockedIncrement64((LONG64*)&RequestContext->Vcb->VolumeMetrics.LargeIRPRegistrationCanceled);
  ...
  {
    status = RegisterPendingIrpMain(RequestContext, EventContext, &RequestContext->Dcb->PendingIrp,
                                    /*CheckMount=*/TRUE,
...
VOID DokanRegisterPendingRetryIrp(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  RegisterPendingIrpMain(RequestContext, /*EventContext=*/NULL, &RequestContext->Dcb->PendingRetryIrp,
                         /*CheckMount=*/TRUE,
                         /*CurrentStatus=*/STATUS_SUCCESS);
}

VOID DokanRegisterAsyncCreateFailure(__in PREQUEST_CONTEXT RequestContext, __in NTSTATUS Status)
{
  if(!RequestContext->Vcb)
  {
    return;
  }
  RegisterPendingIrpMain(RequestContext, /*EventContext=*/NULL, &RequestContext->Dcb->PendingIrp,
                         /*CheckMount=*/TRUE, /*CurrentStatus=*/Status);
  KeSetEvent(&RequestContext->Dcb->ForceTimeoutEvent, 0, FALSE);
}

void DokanDispatchCompletion(__in PDEVICE_OBJECT DeviceObject, __in PIRP_ENTRY irpEntry, __in PEVENT_INFORMATION eventInfo)
{
  ...
  irpEntry->RequestContext.Irp->Tail.Overlay.DriverContext[DRIVER_CONTEXT_IRP_ENTRY] = NULL;
  ...
  {
    case IRP_MJ_DIRECTORY_CONTROL: DokanCompleteDirectoryControl(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_READ: DokanCompleteRead(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_WRITE: DokanCompleteWrite(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_QUERY_INFORMATION: DokanCompleteQueryInformation(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_QUERY_VOLUME_INFORMATION: DokanCompleteQueryVolumeInformation(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_CREATE: DokanCompleteCreate(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_CLEANUP: DokanCompleteCleanup(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_LOCK_CONTROL: DokanCompleteLock(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_SET_INFORMATION: DokanCompleteSetInformation(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_FLUSH_BUFFERS: DokanCompleteFlush(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_QUERY_SECURITY: DokanCompleteQuerySecurity(&irpEntry->RequestContext, eventInfo); break;
    case IRP_MJ_SET_SECURITY: DokanCompleteSetSecurity(&irpEntry->RequestContext, eventInfo); break;
  }

  DOKAN_LOG_END_MJ((&irpEntry->RequestContext), irpEntry->RequestContext.Irp->IoStatus.Status);
}
...
GetEventInfoSize(__in ULONG MajorFunction, __in PEVENT_INFORMATION EventInfo)
{
  ...
  return max((ULONG)sizeof(EVENT_INFORMATION), FIELD_OFFSET(EVENT_INFORMATION, Buffer[0]) + (ULONG)EventInfo->BufferLength);
}
...
DokanCompleteIrp(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  for(thisEntry = listHead->Flink; thisEntry != listHead; thisEntry = nextEntry)
  {
    ...
    if(eventInfo->SerialNumber < lastSerialNumber)
    {
      ...
      result           = DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Reply batch not sorted by serial number.");
    ...
    offset += GetEventInfoSize(irpEntry->RequestContext.IrpSp->MajorFunction, eventInfo);
    // Everything through offset - 1 must be readable by the completion function
    // that receives the EVENT_INFORMATION object.
    if(offset > bufferLength)
    {
      result           = DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Full EVENT_INFORMATION size too large for passed-in buffer.");
      badUsageByCaller = TRUE;
      break;
    }
    ...
    if(!RequestContext->Dcb->AllowIpcBatching)
    {
      if(offset < bufferLength)
      {
        result           = DokanLogError(&logger, STATUS_INVALID_PARAMETER, L"Unexpected batch reply with batching flagged off.");
        badUsageByCaller = TRUE;
      }
      break;
    }
  ...
  while(!IsListEmpty(&completeList))
  {
    ...
    {
      eventInfo     = (PEVENT_INFORMATION)(buffer + offset);
      eventInfoSize = GetEventInfoSize(irpEntry->RequestContext.IrpSp->MajorFunction, eventInfo);
  ...
  if(badUsageByCaller)
  {
    ...
    DokanLogInfo(&logger, L"Unmounting to avoid hanging requests due to incorrect usage.");
...
// ExFreePool.
char* GetDeviceOwner(__in PDOKAN_LOGGER Logger, const __in WCHAR* DeviceNameForLog, __in PDEVICE_OBJECT DeviceObject, _Inout_ PULONG OwnerSize)
{
  ...
  __try
  {
    status = ObOpenObjectByPointer(DeviceObject, OBJ_KERNEL_HANDLE, NULL, READ_CONTROL, 0, KernelMode, &handle);
    if(!NT_SUCCESS(status))
    {
      DokanLogInfo(Logger, L"Failed to open device: %s, status: 0x%x", DeviceNameForLog, status);
      __leave;
    }
    status = ZwQuerySecurityObject(handle, OWNER_SECURITY_INFORMATION, NULL, 0, OwnerSize);
    ...
    status = ZwQuerySecurityObject(handle, OWNER_SECURITY_INFORMATION, result, *OwnerSize, OwnerSize);
    if(!NT_SUCCESS(status))
    {
      DokanLogInfo(Logger,
                   L"Failed to query for owner of device: %s,"
                   L" status: 0x%x",
                   DeviceNameForLog, status);
      ExFreePool(result);
      result = NULL;
...
BOOLEAN HasSameOwner(__in PDOKAN_LOGGER Logger, __in PDEVICE_OBJECT NewDevice, __in PDEVICE_OBJECT OldDevice)
{
  ...
  __try
  {
    ...
    if(oldOwnerSize != newOwnerSize || RtlCompareMemory(newOwner, oldOwner, oldOwnerSize) != oldOwnerSize)
    {
...
// TRUE.
BOOLEAN MaybeUnmountOldDrive(__in PREQUEST_CONTEXT RequestContext, __in PDOKAN_LOGGER Logger, __in PDOKAN_GLOBAL DokanGlobal, __in PDOKAN_CONTROL OldControl, __in PDOKAN_CONTROL NewControl)
{
  DokanLogInfo(Logger,
               L"Mount point exists and"
               L" DOKAN_EVENT_REPLACE_DOKAN_DRIVE_IF_EXISTS is set: %s",
               NewControl->MountPoint);
  if(!HasSameOwner(Logger, NewControl->DiskDeviceObject, OldControl->DiskDeviceObject))
  {
    DokanLogInfo(Logger, L"Not replacing existing drive with different owner.");
    return FALSE;
  }
  DokanLogInfo(Logger, L"Unmounting the existing drive.");
  DokanUnmount(RequestContext, OldControl->Dcb);
  PMOUNT_ENTRY entryAfterUnmount = FindMountEntry(DokanGlobal, NewControl, FALSE);
  if(entryAfterUnmount != NULL)
  {
    DokanLogInfo(Logger, L"Warning: old mount entry was not removed by unmount attempt.");
    return FALSE;
  }
...
VOID RemoveSessionDevices(__in PREQUEST_CONTEXT RequestContext, __in ULONG sessionId)
{
  if(sessionId == -1)
  {
    return;
  ...
  if(!ExAcquireResourceExclusiveLite(&RequestContext->DokanGlobal->Resource, TRUE))
  {
    DOKAN_LOG("Not able to acquire dokanGlobal->Resource \n");
  }
  PDEVICE_ENTRY foundEntry;
  while((foundEntry = FindDeviceForDeleteBySessionId(RequestContext->DokanGlobal, sessionId)) != NULL)
  {
...
DokanEventStart(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  outBufferLen = RequestContext->IrpSp->Parameters.DeviceIoControl.OutputBufferLength;
  if(outBufferLen != sizeof(EVENT_DRIVER_INFO) || !eventStart)
  {
    return DokanLogError(&logger, STATUS_INSUFFICIENT_RESOURCES, L"Buffer IN/OUT received do not match the expected size.");
  }
  ...
  if(eventStart == NULL || baseGuidString == NULL)
  {
    ...
    return DokanLogError(&logger, STATUS_INSUFFICIENT_RESOURCES, L"Failed to allocate buffers in event start.");
  }

  RtlCopyMemory(eventStart, RequestContext->Irp->AssociatedIrp.SystemBuffer, sizeof(EVENT_START));
  ...
  if(DokanSearchStringChar(eventStart->MountPoint, sizeof(eventStart->MountPoint), '\0') == -1 || DokanSearchStringChar(eventStart->UNCName, sizeof(eventStart->UNCName), '\0') == -1)
  {
  ...
  if(eventStart->DeviceType == DOKAN_NETWORK_FILE_SYSTEM && DokanSearchStringChar(eventStart->UNCName, sizeof(eventStart->UNCName), '\0') == -1)
  {
  ...
  {
    case DOKAN_DISK_FILE_SYSTEM: deviceType = FILE_DEVICE_DISK_FILE_SYSTEM; break;
    case DOKAN_NETWORK_FILE_SYSTEM:
      deviceType = FILE_DEVICE_NETWORK_FILE_SYSTEM;
      deviceCharacteristics |= FILE_REMOTE_DEVICE;
      break;
    default: DOKAN_LOG_FINE_IRP(RequestContext, "Unknown device type: %d", eventStart->DeviceType); deviceType = FILE_DEVICE_DISK_FILE_SYSTEM;
  }
  ...
  PMOUNT_ENTRY foundEntry = FindMountEntry(RequestContext->DokanGlobal, &dokanControl, FALSE);
  ...
  baseGuid.Data2 = (USHORT)(RequestContext->DokanGlobal->MountId & 0xFFFF) ^ baseGuid.Data2;
  baseGuid.Data3 = (USHORT)(RequestContext->DokanGlobal->MountId >> 16) ^ baseGuid.Data3;
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Using volume security descriptor of length %d", eventStart->VolumeSecurityDescriptorLength);
    deviceCharacteristics |= FILE_DEVICE_SECURE_OPEN;
    volumeSecurityDescriptor = eventStart->VolumeSecurityDescriptor;
  }

  status = DokanCreateDiskDevice(RequestContext->DeviceObject->DriverObject, RequestContext->DokanGlobal->MountId, eventStart->MountPoint, eventStart->UNCName, volumeSecurityDescriptor, sessionId, baseGuidString, RequestContext->DokanGlobal,
                                 deviceType, deviceCharacteristics, mountGlobally, useMountManager, &dokanControl);
  ...
  dcb->DispatchDriverLogs = (eventStart->Flags & DOKAN_EVENT_DISPATCH_DRIVER_LOGS) != 0;
  dcb->AllowIpcBatching   = (eventStart->Flags & DOKAN_EVENT_ALLOW_IPC_BATCHING) != 0;
  ...
  if(foundEntry != NULL)
  {
    if(MaybeUnmountOldDrive(RequestContext, &logger, RequestContext->DokanGlobal, &foundEntry->MountControl, &dokanControl))
    {
  ...
  else if(eventStart->Flags & DOKAN_EVENT_DRIVE_LETTER_IN_USE)
  {
    ...
    DokanLogInfo(&logger, L"Forcing auto-assignment because the drive letter is in use by"
                          L" another driver.");
    dcb->ForceDriveLetterAutoAssignment = TRUE;
  }
  ...
  PMOUNT_ENTRY mountEntry = InsertMountEntry(RequestContext->DokanGlobal, &dokanControl, FALSE);
  if(mountEntry != NULL)
  {
  ...
  else
  {
    ...
    return DokanLogError(&logger, STATUS_INSUFFICIENT_RESOURCES, L"Failed to allocate new mount entry.");
  }
  ...
  RtlStringCchCopyW(driverInfo->DeviceName, sizeof(driverInfo->DeviceName) / sizeof(WCHAR), &(dcb->SymbolicLinkName->Buffer[deviceNamePos]));
  ...
  dcb->FcbGarbageCollectionIntervalMs = eventStart->FcbGarbageCollectionIntervalMs;
  // Sanitize the garbage collection parameter.
  if(dcb->FcbGarbageCollectionIntervalMs > 0)
  {
    if(dcb->FcbGarbageCollectionIntervalMs < MIN_FCB_GARBAGE_COLLECTION_INTERVAL)
    {
      DokanLogInfo(&logger,
  ...
  DokanLogInfo(&logger, L"Event start using mount ID: %d; device name: %s.", dcb->MountId, driverInfo->DeviceName);
  ...
  if(useMountManager)
  {
    ...
    if(isMountPointDriveLetter)
    {
      if(!dcb->MountPointDetermined)
      {
        ...
        DokanLogError(&logger, 0, L"Warning: mount point creation is being forced.");
        ...
        if(!dcb->MountPointDetermined)
        {
          ...
          DokanLogError(&logger, 0, L"Mount point was still not assigned after forcing.");
          driverInfo->Status = DOKAN_START_FAILED;
          driverInfo->Flags |= DOKAN_DRIVER_INFO_NO_MOUNT_POINT_ASSIGNED;
        }
      }
      if(RtlCompareMemory(mountEntry->MountControl.MountPoint, L"\\DosDevices\\", 24) == 24)
      {
        driverInfo->ActualDriveLetter = mountEntry->MountControl.MountPoint[12];
        DokanLogInfo(&logger, L"Returning actual mount point %c", driverInfo->ActualDriveLetter);
      }
      else
      {
        DokanLogInfo(&logger, L"Warning: actual mount point %s does not have expected prefix.", mountEntry->MountControl.MountPoint);
      }
    }
    else if(!isMountPointDriveLetter && dcb->PersistentSymbolicLinkName)
    {
      ...
      PCHAR setReparseInput       = CreateSetReparsePointRequest(RequestContext, dcb->PersistentSymbolicLinkName, &setReparseInputlength);
      if(setReparseInput)
      {
        status = SendDirectoryFsctl(RequestContext, dcb->MountPoint, FSCTL_SET_REPARSE_POINT, setReparseInput, setReparseInputlength);
        ...
        {
          DokanLogError(&logger, status, L"Failed to set reparse point on MountPoint \"%wZ\"", dcb->MountPoint);
          driverInfo->Status = DOKAN_START_FAILED;
          driverInfo->Flags |= DOKAN_DRIVER_INFO_NO_MOUNT_POINT_ASSIGNED | DOKAN_DRIVER_INFO_SET_REPARSE_POINT_FAILED;
        }
      }
    }
  }
  ...
  {
    DokanEventRelease(RequestContext, mountEntry->MountControl.VolumeDeviceObject);
    driverInfo->DeviceNumber = 0;
    driverInfo->MountId      = 0;
  }

  DokanLogInfo(&logger, L"Finished event start with status %d and flags: %I32x", driverInfo->Status, driverInfo->Flags);
  DOKAN_LOG_FINE_IRP(RequestContext, "Finished event start with status %d and flags: %I32x", driverInfo->Status, driverInfo->Flags);
...
DokanEventWrite(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  for(thisEntry = listHead->Flink; thisEntry != listHead; thisEntry = nextEntry)
  {
    ...
    if(eventIrpSp->Parameters.DeviceIoControl.OutputBufferLength < eventContext->Length)
    {
  ...
  DOKAN_LOG_FINE_IRP(RequestContext, "EventWrite : Cannot found corresponding IRP. User should "
