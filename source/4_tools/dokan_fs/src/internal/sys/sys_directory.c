...
DokanDispatchDirectoryControl(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  {
    case IRP_MN_QUERY_DIRECTORY: return DokanQueryDirectory(RequestContext);
    case IRP_MN_NOTIFY_CHANGE_DIRECTORY: return DokanNotifyChangeDirectory(RequestContext);
  }
...
DokanQueryDirectory(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Index specified %d", RequestContext->IrpSp->Parameters.QueryDirectory.FileIndex);
  }
  ...
  {
    DOKAN_LOG_FINE_IRP(RequestContext, "Pattern=\"%wZ\"", RequestContext->IrpSp->Parameters.QueryDirectory.FileName);
  }

  DOKAN_LOG_FINE_IRP(RequestContext, "FileObject=%p FileInformationClass=%s", fileObject, DokanGetFileInformationClassStr(RequestContext->IrpSp->Parameters.QueryDirectory.FileInformationClass));
  ...
  {
    status = DokanAllocateMdl(RequestContext, RequestContext->IrpSp->Parameters.QueryDirectory.Length);
  ...
  initial = (BOOLEAN)(ccb->SearchPattern == NULL && !(DokanCCBFlagsIsSet(ccb, DOKAN_DIR_MATCH_ALL)));
  ...
  if(initial)
  {
    ...
    if(RequestContext->IrpSp->Parameters.QueryDirectory.FileName)
    {
      ...
      ccb->SearchPatternLength = RequestContext->IrpSp->Parameters.QueryDirectory.FileName->Length;
      ccb->SearchPattern       = DokanAllocZero(ccb->SearchPatternLength + sizeof(WCHAR));
      ...
      RtlCopyMemory(ccb->SearchPattern, RequestContext->IrpSp->Parameters.QueryDirectory.FileName->Buffer, ccb->SearchPatternLength);
    }
    else
    {
  ...
  eventContext->Operation.Directory.FileInformationClass = RequestContext->IrpSp->Parameters.QueryDirectory.FileInformationClass;
  eventContext->Operation.Directory.BufferLength         = RequestContext->IrpSp->Parameters.QueryDirectory.Length; // length of buffer
  ...
  RtlCopyMemory(eventContext->Operation.Directory.DirectoryName, fcb->FileName.Buffer, fcb->FileName.Length);
  DokanFCBUnlock(fcb);
  ...
  if(ccb->SearchPatternLength && ccb->SearchPattern)
  {
    ...
    searchBuffer = (PVOID)((SIZE_T)&eventContext->Operation.Directory.SearchPatternBase[0] + (SIZE_T)eventContext->Operation.Directory.SearchPatternOffset);

    RtlCopyMemory(searchBuffer, ccb->SearchPattern, ccb->SearchPatternLength);

    DOKAN_LOG_FINE_IRP(RequestContext, "ccb->SearchPattern %ws", ccb->SearchPattern);
  }
...
DokanNotifyChangeDirectory(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  FsRtlNotifyFullChangeDirectory(RequestContext->Vcb->NotifySync, &RequestContext->Vcb->DirNotifyList, ccb, (PSTRING)&fcb->FileName, (RequestContext->IrpSp->Flags & SL_WATCH_TREE) ? TRUE : FALSE, FALSE,
                                 RequestContext->IrpSp->Parameters.NotifyDirectory.CompletionFilter, RequestContext->Irp, NULL, NULL);
  DokanFCBUnlock(fcb);
...
VOID DokanCompleteDirectoryControl(__in PREQUEST_CONTEXT RequestContext, __in PEVENT_INFORMATION EventInfo)
{
  ...
  else
  {
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "EventInfo->Directory.Index = %lu", EventInfo->Operation.Directory.Index);
    DOKAN_LOG_FINE_IRP(RequestContext, "EventInfo->BufferLength = % lu", EventInfo->BufferLength);
    DOKAN_LOG_FINE_IRP(RequestContext, "EventInfo->Status = % x(% lu)", EventInfo->Status, EventInfo->Status);
