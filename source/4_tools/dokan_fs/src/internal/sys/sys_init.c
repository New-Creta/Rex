...
PDEVICE_ENTRY
InsertDeviceToDelete(PDOKAN_GLOBAL dokanGlobal, PDEVICE_OBJECT DiskDeviceObject, PDEVICE_OBJECT VolumeDeviceObject, BOOLEAN lockGlobal, ULONG SessionId, PUNICODE_STRING MountPoint)
{
...
InsertDcbToDelete(PDokanDCB Dcb, PDEVICE_OBJECT VolumeDeviceObject, BOOLEAN lockGlobal)
{
  ...
  return InsertDeviceToDelete(Dcb->Global, Dcb->DeviceObject, VolumeDeviceObject, lockGlobal, Dcb->SessionId, Dcb->MountPoint);
}
...
VOID DeleteDeviceDelayed(PDOKAN_GLOBAL dokanGlobal)
{
  ...
  for(entry = listHead->Flink, nextEntry = entry->Flink; entry != listHead; entry = nextEntry, nextEntry = entry->Flink, totalCount++)
  {
    deviceEntry = CONTAINING_RECORD(entry, DEVICE_ENTRY, ListEntry);
    if(deviceEntry)
    {
      DOKAN_LOG_("There is a device for delayed delete. Counter %lu", deviceEntry->Counter);
      ...
      if(deviceEntry->Counter > 3)
      {
        ...
        {
          DOKAN_LOG_("Counter reached the limit. ReferenceCount %lu", deviceEntry->VolumeDeviceObject->ReferenceCount);
          if(deviceEntry->VolumeDeviceObject->ReferenceCount == 0)
          {
            if(canDeleteDiskDevice)
            {
              ...
              {
                DOKAN_LOG_("Delete of Symbolic failed Name: %wZ %s", dcb->SymbolicLinkName, DokanGetNTSTATUSStr(status));
              }

              FreeDcbNames(dcb);

              DOKAN_LOG_("Delete the volume device. ReferenceCount %lu", deviceEntry->VolumeDeviceObject->ReferenceCount);
              IoDeleteDevice(deviceEntry->VolumeDeviceObject);
              deviceEntry->VolumeDeviceObject = NULL;
            }
            else
            {
              DOKAN_LOG_("Disk device has still some references. "
                         "ReferenceCount %lu",
          ...
          else
          {
            ...
            DOKAN_LOG_("Counter reached the limit. Can't delete the volume "
        ...
        {
          DOKAN_LOG_("Delete the disk device. ReferenceCount %lu", deviceEntry->DiskDeviceObject->ReferenceCount);
...
--*/
{
  ...
  while(waitObj)
  {
    status = KeWaitForMultipleObjects(2, pollevents, WaitAny, Executive, KernelMode, FALSE, NULL, NULL);
...
--*/
{
  ...
  status = PsCreateSystemThread(&thread, THREAD_ALL_ACCESS, NULL, NULL, NULL, (PKSTART_ROUTINE)DokanDeleteDeviceThread, dokanGlobal);
  ...
  ObReferenceObjectByHandle(thread, THREAD_ALL_ACCESS, NULL, KernelMode, (PVOID*)&dokanGlobal->DeviceDeleteThread, NULL);
...
BOOLEAN IsDeletePending(__in PDEVICE_OBJECT DeviceObject)
{
  ...
  if(DeviceObject->DeviceObjectExtension && DeviceObject->DeviceObjectExtension->ExtensionFlags & (DOE_UNLOAD_PENDING | DOE_DELETE_PENDING | DOE_REMOVE_PENDING | DOE_REMOVE_PROCESSED))
  {
...
FindMountEntry(__in PDOKAN_GLOBAL dokanGlobal, __in PDOKAN_CONTROL DokanControl, __in BOOLEAN lockGlobal)
{
  ...
  DokanLogInfo(&logger, L"Finding mount entry; lockGlobal = %d; mount point = %s.", lockGlobal, DokanControl->MountPoint);
  ...
  for(listEntry = dokanGlobal->MountPointList.Flink; listEntry != &dokanGlobal->MountPointList; listEntry = listEntry->Flink)
  {
    ...
    if(useMountPoint)
    {
      ...
      if((wcscmp(DokanControl->MountPoint, mountEntry->MountControl.MountPoint) == 0) && (isSessionIdMatch || mountEntry->MountControl.SessionId == (ULONG)-1))
      {
      ...
      {
        DokanLogInfo(&logger, L"Skipping entry with non-matching mount point: %s", mountEntry->MountControl.MountPoint);
      }
    }
    else
    {
      ...
      {
        DokanLogInfo(&logger, L"Found entry with matching device name: %s", mountEntry->MountControl.DeviceName);
        found = TRUE;
        break;
      }
    }
  ...
  if(found)
  {
    DokanLogInfo(&logger, L"Mount entry found: %s -> %s", mountEntry->MountControl.MountPoint, mountEntry->MountControl.DeviceName);
    return mountEntry;
  }
  else
  {
...
PMOUNT_ENTRY FindMountEntryByName(__in PDOKAN_GLOBAL DokanGlobal, __in PUNICODE_STRING DiskDeviceName, __in PUNICODE_STRING UNCName, __in BOOLEAN LockGlobal)
{
  ...
  RtlCopyMemory(dokanControl->DeviceName, DiskDeviceName->Buffer, DiskDeviceName->Length);
...
DokanGetMountPointList(__in PREQUEST_CONTEXT RequestContext)
{
  ...
  __try
  {
    ...
    dokanMountPointInfo = (PDOKAN_MOUNT_POINT_INFO)RequestContext->Irp->AssociatedIrp.SystemBuffer;
    for(listEntry = RequestContext->DokanGlobal->MountPointList.Flink; listEntry != &RequestContext->DokanGlobal->MountPointList; listEntry = listEntry->Flink, ++i)
    {
      if(!ExtendOutputBufferBySize(RequestContext->Irp, sizeof(DOKAN_MOUNT_POINT_INFO),
      ...
      dokanMountPointInfo[i].MountOptions = mountEntry->MountControl.MountOptions;
      RtlCopyMemory(&dokanMountPointInfo[i].MountPoint, &mountEntry->MountControl.MountPoint, sizeof(mountEntry->MountControl.MountPoint));
      RtlCopyMemory(&dokanMountPointInfo[i].UNCName, &mountEntry->MountControl.UNCName, sizeof(mountEntry->MountControl.UNCName));
      RtlCopyMemory(&dokanMountPointInfo[i].DeviceName, &mountEntry->MountControl.DeviceName, sizeof(mountEntry->MountControl.DeviceName));
    }
...
DokanCreateGlobalDiskDevice(__in PDRIVER_OBJECT DriverObject, __out PDOKAN_GLOBAL* DokanGlobal)
{
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("IoCreateDevice returned 0x%x %s", status, DokanGetNTSTATUSStr(status));
    return status;
  }
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("IoCreateDevice Disk FileSystem failed: 0x%x %s", status, DokanGetNTSTATUSStr(status));
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("IoCreateDevice Cd FileSystem failed: 0x%x %s", status, DokanGetNTSTATUSStr(status));
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("IoCreateSymbolicLink returned 0x%x %s", status, DokanGetNTSTATUSStr(status));
    IoDeleteDevice(fsDiskDeviceObject);
    IoDeleteDevice(fsCdDeviceObject);
    IoDeleteDevice(deviceObject);
    return status;
  }
  DOKAN_LOG_("SymbolicLink: \"%wZ\" -> \"%wZ\" created", &deviceName, &symbolicLinkName);
...
VOID DokanRegisterUncProvider(__in PVOID pDcb)
{
  ...
  {
    status = FsRtlRegisterUncProvider(&(Dcb->MupHandle), Dcb->DiskDeviceName, FALSE);
...
VOID DokanCreateMountPointSysProc(__in PVOID pDcb)
{
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("IoCreateSymbolicLink for mount point \"%wZ\" failed: 0x%X %s", Dcb->MountPoint, status, DokanGetNTSTATUSStr(status));
  }
}
...
VOID DokanCreateMountPoint(__in PDokanDCB Dcb)
{
  ...
  if(Dcb->MountPoint != NULL && Dcb->MountPoint->Length > 0)
  {
    ...
    {
      DokanSendVolumeCreatePoint(Dcb->DriverObject, Dcb->DiskDeviceName, Dcb->MountPoint);
    }
    else
    {
...
BOOLEAN DeleteMountPointSymbolicLink(__in PUNICODE_STRING MountPoint)
{
  ...
  if(!NT_SUCCESS(status))
  {
    DOKAN_LOG_("Delete Mount Point failed Symbolic Name: %wZ %s", MountPoint, DokanGetNTSTATUSStr(status));
    return FALSE;
  }
  return TRUE;
}
...
VOID DokanDeleteMountPoint(__in_opt PREQUEST_CONTEXT RequestContext, __in PDokanDCB Dcb)
{
  ...
  if(Dcb->MountPoint != NULL && Dcb->MountPoint->Length > 0)
  {
    if(Dcb->UseMountManager)
    {
      ...
      {
        DokanLogInfo(&logger, L"Issuing a clean mount manager delete for device \"%wZ\"", Dcb->DiskDeviceName);
      ...
      else if(Dcb->PersistentSymbolicLinkName)
      {
        ...
        PCHAR removeReparseInput       = CreateRemoveReparsePointRequest(RequestContext, &removeReparseInputlength);
        if(removeReparseInput)
        {
          SendDirectoryFsctl(RequestContext, Dcb->MountPoint, FSCTL_DELETE_REPARSE_POINT, removeReparseInput, removeReparseInputlength);
...
NTSTATUS
DokanSetVolumeSecurity(__in PDEVICE_OBJECT DeviceObject, __in PSECURITY_DESCRIPTOR SecurityDescriptor)
{
  ...
  __try
  {
    status = ObOpenObjectByPointer(DeviceObject, OBJ_KERNEL_HANDLE, NULL, WRITE_DAC, 0, KernelMode, &deviceHandle);
    if(!NT_SUCCESS(status))
    {
      DOKAN_LOG_("Failed to open volume handle: 0x%x %s", status, DokanGetNTSTATUSStr(status));
      __leave;
    }
    ...
    if(!NT_SUCCESS(status))
    {
      DOKAN_LOG_("ZeSetSecurityObject failed: 0x%x %s", status, DokanGetNTSTATUSStr(status));
      __leave;
    }
  }
  __finally
                      ...
                      __in PDOKAN_GLOBAL DokanGlobal, __in DEVICE_TYPE DeviceType, __in ULONG DeviceCharacteristics, __in BOOLEAN MountGlobally, __in BOOLEAN UseMountManager, __out PDOKAN_CONTROL DokanControl)
{
  ...
  __try
  {
    DokanLogInfo(&logger, L"Creating disk device; mount point = %s; mount ID = %ul", MountPoint, MountId);
    diskDeviceNameBuf   = DokanAllocZero(MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
    symbolicLinkNameBuf = DokanAllocZero(MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
    mountPointBuf       = DokanAllocZero(MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
    if(diskDeviceNameBuf == NULL || symbolicLinkNameBuf == NULL || mountPointBuf == NULL)
    {
      status = DokanLogError(&logger, STATUS_INSUFFICIENT_RESOURCES, L"Could not allocate buffers while creating disk device.");
      __leave;
    }
    ...
    if(!NT_SUCCESS(status))
    {
      DokanLogError(&logger, status, (isNetworkFileSystem ? L"IoCreateDevice(FILE_DEVICE_UNKNOWN) failed." : L"IoCreateDeviceSecure(FILE_DEVICE_DISK) failed."));
      __leave;
    }

    if(VolumeSecurityDescriptor != NULL)
    {
      status = DokanSetVolumeSecurity(diskDeviceObject, VolumeSecurityDescriptor);

      if(!NT_SUCCESS(status))
      {
        DokanLogError(&logger, status, L"Failed to set volume security descriptor.");
        __leave;
      }
    ...
    if(dcb->DiskDeviceName == NULL || dcb->SymbolicLinkName == NULL || dcb->MountPoint == NULL || (dcb->UNCName == NULL && UNCName != NULL))
    {
    ...
    DokanLogInfo(&logger,
                 ...
                 dcb->DiskDeviceName, dcb->SymbolicLinkName, dcb->MountPoint, DeviceType);
    ...
    DokanLogInfo(&logger, L"SymbolicLink: \"%wZ\" -> \"%wZ\" created", dcb->SymbolicLinkName, dcb->DiskDeviceName);
    ...
    RtlStringCchCopyW(DokanControl->DeviceName, sizeof(DokanControl->DeviceName) / sizeof(WCHAR), diskDeviceNameBuf);
    RtlStringCchCopyW(DokanControl->MountPoint, sizeof(DokanControl->MountPoint) / sizeof(WCHAR), mountPointBuf);
    if(UNCName != NULL)
    {
      RtlStringCchCopyW(DokanControl->UNCName, sizeof(DokanControl->UNCName) / sizeof(WCHAR), UNCName);
    }
...
VOID DokanDeleteDeviceObject(__in_opt PREQUEST_CONTEXT RequestContext, __in PDokanDCB Dcb)
{
  ...
  {
    DokanLogInfo(&logger, L"Changing interface state to false for mounted device \"%wZ\"", &Dcb->MountedDeviceInterfaceName);
  ...
  {
    DokanLogInfo(&logger, L"Changing interface state to false for disk device \"%wZ\"", &Dcb->DiskDeviceInterfaceName);
