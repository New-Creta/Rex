...
#include "dokan_fs/internal/dokan/dokan_vector.h"
...
int InitializePool()
{
  (void)InitializeCriticalSectionAndSpinCount(&g_IoBatchBufferCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_IoEventBufferCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_EventResultCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_16KEventResultCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_32KEventResultCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_64KEventResultCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_128KEventResultCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_FileInfoCriticalSection, 0x80000400);
  (void)InitializeCriticalSectionAndSpinCount(&g_DirectoryListCriticalSection, 0x80000400);

  if(g_ThreadPool)
  {
    DbgPrint("Dokan Error: Thread pool has already been created.");
  ...
  if(!g_ThreadPool)
  {
    DbgPrint("Dokan Error: Failed to create thread pool.");
    return DOKAN_DRIVER_INSTALL_ERROR;
  }

  g_IoBatchBufferPool   = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_BATCH_POOL_SIZE);
  g_IoEventBufferPool   = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EVENT_POOL_SIZE);
  g_EventResultPool     = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EVENT_POOL_SIZE);
  g_16KEventResultPool  = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EXTRA_EVENT_POOL_SIZE);
  g_32KEventResultPool  = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EXTRA_EVENT_POOL_SIZE);
  g_64KEventResultPool  = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EXTRA_EVENT_POOL_SIZE);
  g_128KEventResultPool = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EXTRA_EVENT_POOL_SIZE);
  g_FileInfoPool        = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_IO_EVENT_POOL_SIZE);
  g_DirectoryListPool   = DokanVector_AllocWithCapacity(sizeof(PVOID), DOKAN_DIRECTORY_LIST_POOL_SIZE);
...
VOID CleanupPool()
{
  ...
  //////////////////// IO batch buffer object pool ////////////////////
  {
    EnterCriticalSection(&g_IoBatchBufferCriticalSection);
    {
      ...
      {
        FreeIoBatchBuffer(*(PDOKAN_IO_BATCH*)DokanVector_GetItem(g_IoBatchBufferPool, i));
      }
  ...
  //////////////////// IO event buffer object pool ////////////////////
  {
    EnterCriticalSection(&g_IoEventBufferCriticalSection);
    {
      ...
      {
        FreeIoEventBuffer(*(PDOKAN_IO_EVENT*)DokanVector_GetItem(g_IoEventBufferPool, i));
      }
  ...
  //////////////////// Event result object pool ////////////////////
  {
    EnterCriticalSection(&g_EventResultCriticalSection);
    {
      ...
      {
        FreeEventResult(*(PEVENT_INFORMATION*)DokanVector_GetItem(g_EventResultPool, i));
      }
...

  {
    EnterCriticalSection(&g_16KEventResultCriticalSection);
    {
      ...
      {
        FreeEventResult(*(PEVENT_INFORMATION*)DokanVector_GetItem(g_16KEventResultPool, i));
      }
...

  {
    EnterCriticalSection(&g_32KEventResultCriticalSection);
    {
      ...
      {
        FreeEventResult(*(PEVENT_INFORMATION*)DokanVector_GetItem(g_32KEventResultPool, i));
      }
...

  {
    EnterCriticalSection(&g_64KEventResultCriticalSection);
    {
      ...
      {
        FreeEventResult(*(PEVENT_INFORMATION*)DokanVector_GetItem(g_64KEventResultPool, i));
      }
...

  {
    EnterCriticalSection(&g_128KEventResultCriticalSection);
    {
      ...
      {
        FreeEventResult(*(PEVENT_INFORMATION*)DokanVector_GetItem(g_128KEventResultPool, i));
      }
  ...
  //////////////////// File info object pool ////////////////////
  {
    EnterCriticalSection(&g_FileInfoCriticalSection);
    {
      ...
      {
        FreeFileOpenInfo(*(PDOKAN_OPEN_INFO*)DokanVector_GetItem(g_FileInfoPool, i));
      }
...
PDOKAN_IO_BATCH PopIoBatchBuffer()
{
  ...
  EnterCriticalSection(&g_IoBatchBufferCriticalSection);
  {
    ...
    {
      ioBatch = *(PDOKAN_IO_BATCH*)DokanVector_GetLastItem(g_IoBatchBufferPool);
...
{
  assert(IoBatch);
  LONG currentEventContextBatchCount = InterlockedDecrement(&IoBatch->EventContextBatchCount);
...
PDOKAN_IO_EVENT PopIoEventBuffer()
{
  ...
  EnterCriticalSection(&g_IoEventBufferCriticalSection);
  {
    ...
    {
      ioEvent = *(PDOKAN_IO_EVENT*)DokanVector_GetLastItem(g_IoEventBufferPool);
...
PEVENT_INFORMATION PopEventResult()
{
  ...
  EnterCriticalSection(&g_EventResultCriticalSection);
  {
    ...
    {
      eventResult = *(PEVENT_INFORMATION*)DokanVector_GetLastItem(g_EventResultPool);
...
PEVENT_INFORMATION Pop16KEventResult()
{
  ...
  EnterCriticalSection(&g_16KEventResultCriticalSection);
  {
    ...
    {
      eventResult = *(PEVENT_INFORMATION*)DokanVector_GetLastItem(g_16KEventResultPool);
...
VOID Push16KEventResult(PEVENT_INFORMATION EventResult)
{
  ...
  EnterCriticalSection(&g_16KEventResultCriticalSection);
  {
    if(DokanVector_GetCount(g_16KEventResultPool) < DOKAN_IO_EXTRA_EVENT_POOL_SIZE)
    {
...
PEVENT_INFORMATION Pop32KEventResult()
{
  ...
  EnterCriticalSection(&g_32KEventResultCriticalSection);
  {
    ...
    {
      eventResult = *(PEVENT_INFORMATION*)DokanVector_GetLastItem(g_32KEventResultPool);
...
VOID Push32KEventResult(PEVENT_INFORMATION EventResult)
{
  ...
  EnterCriticalSection(&g_32KEventResultCriticalSection);
  {
    if(DokanVector_GetCount(g_32KEventResultPool) < DOKAN_IO_EXTRA_EVENT_POOL_SIZE)
    {
...
PEVENT_INFORMATION Pop64KEventResult()
{
  ...
  EnterCriticalSection(&g_64KEventResultCriticalSection);
  {
    ...
    {
      eventResult = *(PEVENT_INFORMATION*)DokanVector_GetLastItem(g_64KEventResultPool);
...
VOID Push64KEventResult(PEVENT_INFORMATION EventResult)
{
  ...
  EnterCriticalSection(&g_64KEventResultCriticalSection);
  {
    if(DokanVector_GetCount(g_64KEventResultPool) < DOKAN_IO_EXTRA_EVENT_POOL_SIZE)
    {
...
PEVENT_INFORMATION Pop128KEventResult()
{
  ...
  EnterCriticalSection(&g_128KEventResultCriticalSection);
  {
    ...
    {
      eventResult = *(PEVENT_INFORMATION*)DokanVector_GetLastItem(g_128KEventResultPool);
...
VOID Push128KEventResult(PEVENT_INFORMATION EventResult)
{
  ...
  EnterCriticalSection(&g_128KEventResultCriticalSection);
  {
    if(DokanVector_GetCount(g_128KEventResultPool) < DOKAN_IO_EXTRA_EVENT_POOL_SIZE)
    {
...
PDOKAN_OPEN_INFO PopFileOpenInfo()
{
  ...
  if(!fileInfo)
  {
    ...
    if(!fileInfo)
    {
      DbgPrint("Dokan Error: Failed to allocate DOKAN_OPEN_INFO.");
      return NULL;
    }
...
PDOKAN_VECTOR PopDirectoryList()
{
  ...
  EnterCriticalSection(&g_DirectoryListCriticalSection);
  {
    ...
    {
      directoryList = *(PDOKAN_VECTOR*)DokanVector_GetLastItem(g_DirectoryListPool);
