...
static BOOL DokanServiceCheck(LPCWSTR ServiceName)
{
  ...
  serviceHandle = OpenServiceW(controlHandle, ServiceName, SERVICE_START | SERVICE_STOP | SERVICE_QUERY_STATUS);

  if(serviceHandle == NULL)
  {
    DbgPrint("DokanServiceCheck: Failed to open Service ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), GetLastError());
...
static BOOL DokanServiceControl(LPCWSTR ServiceName, ULONG Type)
{
  ...
  if(controlHandle == NULL)
  {
    DbgPrint("DokanServiceControl: Failed to open Service Control Manager. error = {}",
                  GetLastError());
    return FALSE;
  }

  serviceHandle = OpenServiceW(controlHandle, ServiceName, SERVICE_START | SERVICE_STOP | SERVICE_QUERY_STATUS | DELETE);

  if(serviceHandle == NULL)
  {
    DbgPrint("DokanServiceControl: Failed to open Service ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), GetLastError());
    CloseServiceHandle(controlHandle);
    return FALSE;
  }
  ...
  if(QueryServiceStatus(serviceHandle, &ss) != 0)
  {
    if(Type == DOKAN_SERVICE_DELETE)
    {
      ...
      {
        DbgPrint("DokanServiceControl: Service ({}) deleted", rsl::to_string(rsl::wstring_view(ServiceName)));
        result = TRUE;
      }
      else
      {
        DbgPrint("DokanServiceControl: Failed to delete service ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), GetLastError());
        result = FALSE;
      }
    }
    else if(ss.dwCurrentState == SERVICE_STOPPED && Type == DOKAN_SERVICE_START)
    {
      ...
      {
        DbgPrint("DokanServiceControl: Service ({}) started", rsl::to_string(rsl::wstring_view(ServiceName)));
        result = TRUE;
      }
      else
      {
        DbgPrint("DokanServiceControl: Failed to start service ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), GetLastError());
        result = FALSE;
      }
    }
    else if(ss.dwCurrentState == SERVICE_RUNNING && Type == DOKAN_SERVICE_STOP)
    {
      ...
      {
        DbgPrint("DokanServiceControl: Service ({}) stopped", rsl::to_string(rsl::wstring_view(ServiceName)));
        result = TRUE;
      }
      else
      {
        DbgPrint("DokanServiceControl: Failed to stop service ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), GetLastError());
        result = FALSE;
      }
    }
  }
  ...
  {
    DbgPrint("DokanServiceControl: QueryServiceStatus Failed ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), GetLastError());
    result = FALSE;
  }
...
BOOL DOKANAPI DokanServiceInstall(LPCWSTR ServiceName, DWORD ServiceType, LPCWSTR ServiceFullPath)
{
  SC_HANDLE controlHandle;
  SC_HANDLE serviceHandle;
  ...
  if(controlHandle == NULL)
  {
    DbgPrint("DokanServiceInstall: Failed to open Service Control "
                  "Manager. error = {}",
                  GetLastError());
    return FALSE;
  }

  serviceHandle = CreateServiceW(controlHandle, ServiceName, ServiceName, 0, ServiceType, SERVICE_AUTO_START, SERVICE_ERROR_IGNORE, ServiceFullPath, NULL, NULL, NULL, NULL, NULL);
  ...
  if(serviceHandle == NULL)
  {
    ...
    {
      DbgPrint("DokanServiceInstall: Service ({}) is already installed", rsl::to_string(rsl::wstring_view(ServiceName)));
    }
    else
    {
      DbgPrint("DokanServiceInstall: Failed to install service ({}). error = {}", rsl::to_string(rsl::wstring_view(ServiceName)), error);
    }
    CloseServiceHandle(controlHandle);
    return FALSE;
  }
  ...
  DbgPrint("DokanServiceInstall: Service ({}) installed", rsl::to_string(rsl::wstring_view(ServiceName)));

  if(DokanServiceControl(ServiceName, DOKAN_SERVICE_START))
  {
    DbgPrint("DokanServiceInstall: Service ({}) started", rsl::to_string(rsl::wstring_view(ServiceName)));
    return TRUE;
  }
  else
  {
    DbgPrint("DokanServiceInstall: Service ({}) start failed", rsl::to_string(rsl::wstring_view(ServiceName)));
    return FALSE;
  }
}
...
#define DOKAN_NP_PATH        L"%SystemRoot%\\System32\\dokannp" DOKAN_MAJOR_API_VERSION L".dll"
...
BOOL DOKANAPI DokanNetworkProviderInstall()
{
  ...
  if(RegCreateKeyExW(HKEY_LOCAL_MACHINE, DOKAN_NP_SERVICE_KEY L"\\NetworkProvider", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &key, &position) != ERROR_SUCCESS)
  {
    return FALSE;
  }

  RegSetValueExW(key, L"DeviceName", 0, REG_SZ, (BYTE*)DOKAN_NP_DEVICE_NAME, (DWORD)(wcslen(DOKAN_NP_DEVICE_NAME) + 1) * sizeof(WCHAR));

  RegSetValueExW(key, L"Name", 0, REG_SZ, (BYTE*)DOKAN_NP_NAME, (DWORD)(wcslen(DOKAN_NP_NAME) + 1) * sizeof(WCHAR));

  RegSetValueExW(key, L"ProviderPath", 0, REG_EXPAND_SZ, (BYTE*)DOKAN_NP_PATH, (DWORD)(wcslen(DOKAN_NP_PATH) + 1) * sizeof(WCHAR));

  RegCloseKey(key);

  if(RegOpenKeyExW(HKEY_LOCAL_MACHINE, DOKAN_NP_ORDER_KEY, 0, KEY_ALL_ACCESS, &key) != ERROR_SUCCESS)
  {
    return FALSE;
  }

  if(RegQueryValueExW(key, L"ProviderOrder", 0, &type, (BYTE*)&buffer, &buffer_size) != ERROR_SUCCESS)
  {
    return FALSE;
  }
  ...
  if(wcsstr(buffer, commanp) == NULL)
  {
    ...
    if(RegSetValueExW(key, L"ProviderOrder", 0, REG_SZ, (BYTE*)&buffer, (DWORD)(wcslen(buffer) + 1) * sizeof(WCHAR)) != ERROR_SUCCESS)
    {
      return FALSE;
    }
  }
...
BOOL DOKANAPI DokanNetworkProviderUninstall()
{
  ...
  RegOpenKeyExW(HKEY_LOCAL_MACHINE, DOKAN_NP_SERVICE_KEY, 0, KEY_ALL_ACCESS, &key);
  RegDeleteKeyW(key, L"NetworkProvider");

  RegCloseKey(key);

  RegOpenKeyExW(HKEY_LOCAL_MACHINE, DOKAN_NP_ORDER_KEY, 0, KEY_ALL_ACCESS, &key);

  RegQueryValueExW(key, L"ProviderOrder", 0, &type, (BYTE*)&buffer, &buffer_size);
  ...
  wcscat_s(buffer2, sizeof(buffer2) / sizeof(WCHAR), dokan_pos + wcslen(commanp));
  RegSetValueExW(key, L"ProviderOrder", 0, REG_SZ, (BYTE*)&buffer2, (DWORD)(wcslen(buffer2) + 1) * sizeof(WCHAR));
...
BOOL CreateMountPoint(LPCWSTR MountPoint, LPCWSTR DeviceName)
{
  ...
  handle = CreateFileW(MountPoint, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_BACKUP_SEMANTICS, NULL);
  ...
  if(handle == INVALID_HANDLE_VALUE)
  {
    ...
    DbgPrint("Use {} as mount point failed: ({}) {}", rsl::to_string(rsl::wstring_view(MountPoint)), GetLastError(), rsl::to_string(rsl::wstring_view(errorMsg)));
    return FALSE;
  }
  ...
  result = DeviceIoControl(handle, FSCTL_SET_REPARSE_POINT, reparseData, bufferLength, NULL, 0, &resultLength, NULL);
...
BOOL DeleteMountPoint(LPCWSTR MountPoint)
{
  ...
  if(handle == INVALID_HANDLE_VALUE)
  {
    DbgPrint("CreateFile failed: {} ({})", rsl::to_string(rsl::wstring_view(MountPoint)), GetLastError());
    return FALSE;
  }

  reparseData.ReparseTag = IO_REPARSE_TAG_MOUNT_POINT;

  result = DeviceIoControl(handle, FSCTL_DELETE_REPARSE_POINT, &reparseData, REPARSE_GUID_DATA_BUFFER_HEADER_SIZE, NULL, 0, &resultLength, NULL);

  CloseHandle(handle);

  if(result)
  {
    DbgPrint("DeleteMountPoint {} success", rsl::to_string(rsl::wstring_view(MountPoint)));
  }
  else
  {
    ...
    FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), errorMsg, 256, NULL);
    DbgPrint("DeleteMountPoint {} failed: ({}) {}", rsl::to_string(rsl::wstring_view(MountPoint)), GetLastError(), rsl::to_string(rsl::wstring_view(errorMsg)));
  }
  return result;
}
...
VOID DokanBroadcastLink(WCHAR cLetter, BOOL bRemoved)
{
  ...
  if(BroadcastSystemMessage(BSF_NOHANG | BSF_FORCEIFHUNG, &receipients, WM_DEVICECHANGE, device_event, (LPARAM)&params) <= 0)
  {
    DbgPrint("DokanBroadcastLink: BroadcastSystemMessage failed - {}", GetLastError());
  }
...
{
  WCHAR unmountPoint[MAX_PATH];
  GenerateUnmountPoint(DokanInstance->MountPoint, unmountPoint, ARRAYSIZE(unmountPoint));
  if(!IsMountPointDriveLetter(DokanInstance->MountPoint))
  {
    size_t length = wcslen(unmountPoint);
    if(!(DokanInstance->DokanOptions->Options & DOKAN_OPTION_MOUNT_MANAGER) && length + 1 < MAX_PATH)
    {
      ...
      DeleteVolumeMountPointW(unmountPoint);
    }
  }
  else
  {
