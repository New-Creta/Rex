...
DWORD SendWriteRequest(PDOKAN_IO_EVENT IoEvent, ULONG WriteEventContextLength, PDOKAN_IO_BATCH* WriteIoBatch)
{
  ...
  {
    *WriteIoBatch = (PDOKAN_IO_BATCH)malloc((SIZE_T)FIELD_OFFSET(DOKAN_IO_BATCH, EventContext) + WriteEventContextLength);
    if(!*WriteIoBatch)
    {
      DbgPrint("Dokan Error: Failed to allocate IO event buffer.");
...
VOID DispatchWrite(PDOKAN_IO_EVENT IoEvent)
{
  ...
  DbgPrint("###WriteFile file handle = {}, eventID = {}, event Info = {}", (void*)IoEvent->DokanOpenInfo, IoEvent->DokanOpenInfo != NULL ? IoEvent->DokanOpenInfo->EventId : -1, (void*)IoEvent);
  ...
  {
    DWORD error = SendWriteRequest(IoEvent, IoEvent->EventContext->Operation.Write.RequestLength, &writeIoBatch);
  ...
  {
    status = IoEvent->DokanInstance->DokanOperations->WriteFile(writeIoBatch->EventContext->Operation.Write.FileName, (PCHAR)writeIoBatch->EventContext + writeIoBatch->EventContext->Operation.Write.BufferOffset,
                                                                writeIoBatch->EventContext->Operation.Write.BufferLength, &writtenLength, writeIoBatch->EventContext->Operation.Write.ByteOffset.QuadPart, &IoEvent->DokanFileInfo);
  }
  else
  {
  ...
  if(status == STATUS_SUCCESS)
  {
    ...
    IoEvent->EventResult->Operation.Write.CurrentByteOffset.QuadPart = writeIoBatch->EventContext->Operation.Write.ByteOffset.QuadPart + writtenLength;
  }
