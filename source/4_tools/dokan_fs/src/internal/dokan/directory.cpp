...
VOID DokanFillDirInfo(PFILE_DIRECTORY_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
...
VOID DokanFillFullDirInfo(PFILE_FULL_DIR_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
VOID DokanFillIdFullDirInfo(PFILE_ID_FULL_DIR_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
VOID DokanFillIdBothDirInfo(PFILE_ID_BOTH_DIR_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
VOID DokanFillIdExtdDirInfo(PFILE_ID_EXTD_DIR_INFO Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
VOID DokanFillIdExtdBothDirInfo(PFILE_ID_EXTD_BOTH_DIR_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
VOID DokanFillBothDirInfo(PFILE_BOTH_DIR_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
VOID DokanFillNamesInfo(PFILE_NAMES_INFORMATION Buffer, PWIN32_FIND_DATAW FindData, ULONG Index)
{
  ULONG nameBytes = (ULONG)wcslen(FindData->cFileName) * sizeof(WCHAR);
...
ULONG
DokanFillDirectoryInformation(FILE_INFORMATION_CLASS DirectoryInfo, PVOID Buffer, PULONG LengthRemaining, PWIN32_FIND_DATAW FindData, ULONG Index, PDOKAN_INSTANCE DokanInstance)
{
  ULONG nameBytes;
  ULONG thisEntrySize;
  ...
  {
    case FileDirectoryInformation: thisEntrySize += sizeof(FILE_DIRECTORY_INFORMATION); break;
    case FileFullDirectoryInformation: thisEntrySize += sizeof(FILE_FULL_DIR_INFORMATION); break;
    case FileIdFullDirectoryInformation: thisEntrySize += sizeof(FILE_ID_FULL_DIR_INFORMATION); break;
    case FileNamesInformation: thisEntrySize += sizeof(FILE_NAMES_INFORMATION); break;
    case FileBothDirectoryInformation: thisEntrySize += sizeof(FILE_BOTH_DIR_INFORMATION); break;
    case FileIdBothDirectoryInformation: thisEntrySize += sizeof(FILE_ID_BOTH_DIR_INFORMATION); break;
    case FileIdExtdDirectoryInformation: thisEntrySize += sizeof(FILE_ID_EXTD_DIR_INFO); break;
    case FileIdExtdBothDirectoryInformation: thisEntrySize += sizeof(FILE_ID_EXTD_BOTH_DIR_INFORMATION); break;
    default: break;
  }
  ...
  switch(DirectoryInfo)
  {
    case FileDirectoryInformation: DokanFillDirInfo((PFILE_DIRECTORY_INFORMATION)Buffer, FindData, Index, DokanInstance); break;
    case FileFullDirectoryInformation: DokanFillFullDirInfo((PFILE_FULL_DIR_INFORMATION)Buffer, FindData, Index, DokanInstance); break;
    case FileIdFullDirectoryInformation: DokanFillIdFullDirInfo((PFILE_ID_FULL_DIR_INFORMATION)Buffer, FindData, Index, DokanInstance); break;
    case FileNamesInformation: DokanFillNamesInfo((PFILE_NAMES_INFORMATION)Buffer, FindData, Index); break;
    case FileBothDirectoryInformation: DokanFillBothDirInfo((PFILE_BOTH_DIR_INFORMATION)Buffer, FindData, Index, DokanInstance); break;
    case FileIdBothDirectoryInformation: DokanFillIdBothDirInfo((PFILE_ID_BOTH_DIR_INFORMATION)Buffer, FindData, Index, DokanInstance); break;
    case FileIdExtdDirectoryInformation: DokanFillIdExtdDirInfo((PFILE_ID_EXTD_DIR_INFO)Buffer, FindData, Index, DokanInstance); break;
    case FileIdExtdBothDirectoryInformation: DokanFillIdExtdBothDirInfo((PFILE_ID_EXTD_BOTH_DIR_INFORMATION)Buffer, FindData, Index, DokanInstance); break;
    default: break;
  }
...
int WINAPI DokanFillFileData(PWIN32_FIND_DATAW FindData, PDOKAN_FILE_INFO FileInfo)
{
...
{
  ULONG lengthRemaining = IoEvent->EventContext->Operation.Directory.BufferLength;
  ...
  BOOL caseSensitive    = IoEvent->DokanInstance->DokanOptions->Options & DOKAN_OPTION_CASE_SENSITIVE;

  if(IoEvent->EventContext->Operation.Directory.SearchPatternLength > 0)
  {
    pattern = (PWCHAR)((SIZE_T)&IoEvent->EventContext->Operation.Directory.SearchPatternBase[0] + (SIZE_T)IoEvent->EventContext->Operation.Directory.SearchPatternOffset);
  }

  if(pattern && wcscmp(pattern, L"*") != 0 && (!IoEvent->DokanInstance->DokanOperations->FindFilesWithPattern || IoEvent->DokanOpenInfo->UnimplementedFindFilesWithPattern))
  {
    patternCheck = TRUE;
  }
  ...
  for(size_t i = 0; i < DokanVector_GetCount(DirList); ++i)
  {
    ...
    DbgPrint("FileMatch? : {} ({},{},{})", rsl::to_string(rsl::wstring_view(find->FindData.cFileName)), (pattern ? rsl::to_string(rsl::wstring_view(pattern)) : rsl::string("null")), IoEvent->EventContext->Operation.Directory.FileIndex, index);

    // pattern is not specified or pattern match is ignore cases
    if(!patternCheck || DokanIsNameInExpression(pattern, find->FindData.cFileName, !caseSensitive))
    {
      if(IoEvent->EventContext->Operation.Directory.FileIndex <= index)
      {
        // index+1 is very important, should use next entry index
        ULONG entrySize = DokanFillDirectoryInformation((FILE_INFORMATION_CLASS)IoEvent->EventContext->Operation.Directory.FileInformationClass, currentBuffer, &lengthRemaining, &find->FindData, index + 1, IoEvent->DokanInstance);
        // buffer is full
        if(entrySize == 0)
        {
          bufferOverFlow = TRUE;
  ...
  IoEvent->EventResult->BufferLength = IoEvent->EventContext->Operation.Directory.BufferLength - lengthRemaining;
...
VOID AddMissingCurrentAndParentFolder(PDOKAN_IO_EVENT IoEvent)
{
  ...
  if(wcscmp(IoEvent->EventContext->Operation.Directory.DirectoryName, L"\\") == 0 || (pattern != NULL && wcscmp(pattern, L"*") != 0))
  {
    return;
  }
...
NTSTATUS WriteDirectoryResults(PDOKAN_IO_EVENT EventInfo, PDOKAN_VECTOR dirList)
{
  ...
  DbgPrint("WriteDirectoryResults() New directory index is {}.", index);
  ...
  if(index < 0)
  {
    ...
    EventInfo->EventResult->Operation.Directory.Index = EventInfo->EventContext->Operation.Directory.FileIndex;
  }
  else
  {
    DbgPrint("index to {}", index);
...
VOID EndFindFilesCommon(PDOKAN_IO_EVENT IoEvent, NTSTATUS Status)
{
  ...
  if(Status == STATUS_SUCCESS)
  {
    ...
    EnterCriticalSection(&IoEvent->DokanOpenInfo->CriticalSection);
    {
      ...
      if(IoEvent->EventContext->Operation.Directory.SearchPatternLength > 0)
      {
        IoEvent->DokanOpenInfo->DirListSearchPattern = _wcsdup((PWCHAR)((SIZE_T)&IoEvent->EventContext->Operation.Directory.SearchPatternBase[0] + (SIZE_T)IoEvent->EventContext->Operation.Directory.SearchPatternOffset));
      }
    }
...
VOID DispatchDirectoryInformation(PDOKAN_IO_EVENT IoEvent)
{
  ...
  ULONG fileInfoClass  = IoEvent->EventContext->Operation.Directory.FileInformationClass;
  BOOL forceScan       = FALSE;

  DbgPrint("###FindFiles file handle = {}, eventID = {}, event Info = {}", (void*)IoEvent->DokanOpenInfo, IoEvent->DokanOpenInfo != NULL ? IoEvent->DokanOpenInfo->EventId : -1, (void*)IoEvent);

  CheckFileName(IoEvent->EventContext->Operation.Directory.DirectoryName);

  // check whether this is handled FileInfoClass
  if(fileInfoClass != FileDirectoryInformation && fileInfoClass != FileFullDirectoryInformation && fileInfoClass != FileBothDirectoryInformation && fileInfoClass != FileNamesInformation && fileInfoClass != FileIdBothDirectoryInformation &&
     fileInfoClass != FileIdFullDirectoryInformation && fileInfoClass != FileIdExtdDirectoryInformation && fileInfoClass != FileIdExtdBothDirectoryInformation)
  {
    DbgPrint("Dokan Information: Unsupported file information class {}", fileInfoClass);
  ...
  CreateDispatchCommon(IoEvent, IoEvent->EventContext->Operation.Directory.BufferLength,
                       /*UseExtraMemoryPool=*/FALSE,
                       /*ClearNonPoolBuffer=*/TRUE);

  IoEvent->EventResult->Operation.Directory.Index = IoEvent->EventContext->Operation.Directory.FileIndex;

  if(IoEvent->EventContext->Operation.Directory.SearchPatternLength > 0)
  {
    searchPattern = (PWCHAR)((SIZE_T)&IoEvent->EventContext->Operation.Directory.SearchPatternBase[0] + (SIZE_T)IoEvent->EventContext->Operation.Directory.SearchPatternOffset);
  }
  ...
  EnterCriticalSection(&IoEvent->DokanOpenInfo->CriticalSection);
  {
    ...
    {
      forceScan = wcscmp(searchPattern, IoEvent->DokanOpenInfo->DirListSearchPattern) != 0 ? TRUE : FALSE;
    }
    ...
    {
      forceScan = wcscmp(IoEvent->DokanOpenInfo->DirListSearchPattern, L"*") != 0 ? TRUE : FALSE;
    }
    // In FastFat SL_INDEX_SPECIFIED overrides SL_RESTART_SCAN
    forceScan = (forceScan || (!(IoEvent->EventContext->Flags & SL_INDEX_SPECIFIED) && (IoEvent->EventContext->Flags & SL_RESTART_SCAN))) ? TRUE : FALSE;
    if(!forceScan)
    {
  ...
  {
    status = IoEvent->DokanInstance->DokanOperations->FindFilesWithPattern(IoEvent->EventContext->Operation.Directory.DirectoryName, searchPattern ? searchPattern : L"*", DokanFillFileData, &IoEvent->DokanFileInfo);
  ...
  if(status == STATUS_NOT_IMPLEMENTED && IoEvent->DokanInstance->DokanOperations->FindFiles)
  {
    status = IoEvent->DokanInstance->DokanOperations->FindFiles(IoEvent->EventContext->Operation.Directory.DirectoryName, DokanFillFileData, &IoEvent->DokanFileInfo);
  }
