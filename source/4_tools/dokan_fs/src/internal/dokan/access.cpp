...
HANDLE DOKANAPI DokanOpenRequestorToken(PDOKAN_FILE_INFO FileInfo)
{
  ...
  status = SendToDevice(rawDeviceName, FSCTL_GET_ACCESS_TOKEN, eventInfo, eventInfoSize, eventInfo, eventInfoSize, &returnedLength);
  if(status)
  {
  ...
  {
    DbgPrint("Failed to OpenRequestorToken for {}", ioEvent->EventContext->SerialNumber);
  }
  free(eventInfo);
  return handle;
}