...
#include "dokan_fs/internal/dokan/list.h"
...
VOID DispatchDriverLogs(PDOKAN_IO_EVENT IoEvent)
{
  ...
  PDOKAN_LOG_MESSAGE log_message = (PDOKAN_LOG_MESSAGE)((PCHAR)IoEvent->EventContext + sizeof(EVENT_CONTEXT));
  if(log_message->MessageLength)
  {
    ULONG paquet_size = FIELD_OFFSET(DOKAN_LOG_MESSAGE, Message[0]) + log_message->MessageLength;
    if(((PCHAR)log_message + paquet_size) <= ((PCHAR)IoEvent->EventContext + IoEvent->EventContext->Length))
    {
      DbgPrint("DriverLog: %.*s", log_message->MessageLength, log_message->Message);
    }
    else
    {
...
NewDokanInstance()
{
  PDOKAN_INSTANCE dokanInstance = (PDOKAN_INSTANCE)malloc(sizeof(DOKAN_INSTANCE));
  ...
  (void)InitializeCriticalSectionAndSpinCount(&dokanInstance->CriticalSection, 0x80000400);
  ...
  {
    DbgPrint("Dokan Error: Cannot create Dokan instance because the "
  ...
  EnterCriticalSection(&g_InstanceCriticalSection);
  {
    ...
    {
      DbgPrint("Dokan Error: Failed to create thread pool cleanup group.");
      LeaveCriticalSection(&g_InstanceCriticalSection);
      DeleteCriticalSection(&dokanInstance->CriticalSection);
      CloseHandle(dokanInstance->DeviceClosedWaitHandle);
      free(dokanInstance);
    ...
    InitializeThreadpoolEnvironment(&dokanInstance->ThreadInfo.CallbackEnvironment);
    SetThreadpoolCallbackPool(&dokanInstance->ThreadInfo.CallbackEnvironment, threadPool);
    SetThreadpoolCallbackCleanupGroup(&dokanInstance->ThreadInfo.CallbackEnvironment, dokanInstance->ThreadInfo.CleanupGroup, NULL);
...
VOID DeleteDokanInstance(PDOKAN_INSTANCE DokanInstance)
{
  ...
  if(DokanInstance->ThreadInfo.CleanupGroup)
  {
    CloseThreadpoolCleanupGroupMembers(DokanInstance->ThreadInfo.CleanupGroup, FALSE, DokanInstance);
    CloseThreadpoolCleanupGroup(DokanInstance->ThreadInfo.CleanupGroup);
    DokanInstance->ThreadInfo.CleanupGroup = NULL;
    DestroyThreadpoolEnvironment(&DokanInstance->ThreadInfo.CallbackEnvironment);
  }
  if(DokanInstance->NotifyHandle && DokanInstance->NotifyHandle != INVALID_HANDLE_VALUE)
  {
    CloseHandle(DokanInstance->NotifyHandle);
  }
  if(DokanInstance->KeepaliveHandle && DokanInstance->KeepaliveHandle != INVALID_HANDLE_VALUE)
  {
  ...
  if(DokanInstance->GlobalDevice && DokanInstance->GlobalDevice != INVALID_HANDLE_VALUE)
  {
...
BOOL IsMountPointDriveLetter(LPCWSTR mountPoint)
{
  ...
  if(mountPointLength == 1 || (mountPointLength == 2 && mountPoint[1] == L':') || (mountPointLength == 3 && mountPoint[1] == L':' && mountPoint[2] == L'\\'))
  {
    return TRUE;
  }
  return FALSE;
...
{
  return (L'a' <= DriveLetter && DriveLetter <= L'z') || (L'A' <= DriveLetter && DriveLetter <= L'Z');
}
...
BOOL CheckDriveLetterAvailability(WCHAR DriveLetter)
{
  ...
  {
    DbgPrint("CheckDriveLetterAvailability failed, bad drive letter {}", rsl::to_string(rsl::wstring_view(&DriveLetter)));
    return FALSE;
  }

  device = CreateFileW(dosDevice, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_NO_BUFFERING, NULL);

  if(device != INVALID_HANDLE_VALUE)
  {
    DbgPrint("CheckDriveLetterAvailability failed, {}: is already used", rsl::to_string(rsl::wstring_view(&DriveLetter)));
    CloseHandle(device);
    return FALSE;
  }

  ZeroMemory(buffer, MAX_PATH * sizeof(WCHAR));
  result = QueryDosDeviceW(driveName, buffer, MAX_PATH);
  if(result > 0)
  {
    DbgPrint("CheckDriveLetterAvailability failed, QueryDosDevice - Drive letter \"{}\" is already used.", rsl::to_string(rsl::wstring_view(&DriveLetter)));
    return FALSE;
  }
  ...
  if(result > 0)
  {
    DbgPrint("CheckDriveLetterAvailability failed, GetLogicalDrives - Drive letter \"{}\" is already used.", rsl::to_string(rsl::wstring_view(&DriveLetter)));
    return FALSE;
  }
...
VOID CheckAllocationUnitSectorSize(PDOKAN_OPTIONS DokanOptions)
{
  ...
  if((allocationUnitSize < 512 || allocationUnitSize > 65536 || (allocationUnitSize & (allocationUnitSize - 1)) != 0) // Is power of two
     || (sectorSize < 512 || sectorSize > 65536 || (sectorSize & (sectorSize - 1))))
  { // Is power of two
  ...
  DbgPrint("AllocationUnitSize: {} SectorSize: {}", DokanOptions->AllocationUnitSize, DokanOptions->SectorSize);
}
...
VOID SetupIOEventForProcessing(PDOKAN_IO_EVENT IoEvent)
{
  ...
  IoEvent->DokanOpenInfo              = (PDOKAN_OPEN_INFO)(UINT_PTR)IoEvent->EventContext->Context;
  ...
  IoEvent->DokanFileInfo.IsDirectory = (UCHAR)IoEvent->DokanOpenInfo->IsDirectory;
...
VOID DispatchEvent(PDOKAN_IO_EVENT ioEvent)
{
  ...
  {
    case IRP_MJ_CREATE: DispatchCreate(ioEvent); break;
    case IRP_MJ_CLEANUP: DispatchCleanup(ioEvent); break;
    case IRP_MJ_CLOSE: DispatchClose(ioEvent); break;
    case IRP_MJ_DIRECTORY_CONTROL: DispatchDirectoryInformation(ioEvent); break;
    case IRP_MJ_READ: DispatchRead(ioEvent); break;
    case IRP_MJ_WRITE: DispatchWrite(ioEvent); break;
    case IRP_MJ_QUERY_INFORMATION: DispatchQueryInformation(ioEvent); break;
    case IRP_MJ_QUERY_VOLUME_INFORMATION: DispatchQueryVolumeInformation(ioEvent); break;
    case IRP_MJ_LOCK_CONTROL: DispatchLock(ioEvent); break;
    case IRP_MJ_SET_INFORMATION: DispatchSetInformation(ioEvent); break;
    case IRP_MJ_FLUSH_BUFFERS: DispatchFlush(ioEvent); break;
    case IRP_MJ_QUERY_SECURITY: DispatchQuerySecurity(ioEvent); break;
    case IRP_MJ_SET_SECURITY: DispatchSetSecurity(ioEvent); break;
    case DOKAN_IRP_LOG_MESSAGE: DispatchDriverLogs(ioEvent); break;
    default:
      DbgPrint("Dokan Warning: Unsupported IRP {:#x}, event Info = {:#x}.", ioEvent->EventContext->MajorFunction, (void*)ioEvent->EventContext);
...
// End it all
VOID HandleProcessIoFatalError(PDOKAN_INSTANCE DokanInstance, PDOKAN_IO_BATCH IoBatch, DWORD Result)
{
  PushIoBatchBuffer(IoBatch);
  OnDeviceIoCtlFailed(DokanInstance, Result);
}

VOID FreeIoEventResult(PEVENT_INFORMATION EventResult, ULONG EventResultSize, BOOL PoolAllocated)
{
  if(!EventResult)
  {
    return;
...
{
  PTP_WORK work = CreateThreadpoolWork(Callback, IoEvent, &IoEvent->DokanInstance->ThreadInfo.CallbackEnvironment);
  if(!work)
  {
    DWORD lastError = GetLastError();
    DbgPrint("Dokan Error: CreateThreadpoolWork() has returned error code {:#x}.", lastError);
...
GetEventInfoSize(__in ULONG MajorFunction, __in PEVENT_INFORMATION EventInfo)
{
  ...
  return (DWORD)max((ULONG)sizeof(EVENT_INFORMATION), FIELD_OFFSET(EVENT_INFORMATION, Buffer[0]) + EventInfo->BufferLength);
}

DWORD SendAndPullEventInformation(PDOKAN_IO_EVENT IoEvent, PDOKAN_IO_BATCH IoBatch, BOOL ReleaseBatchBuffers)
{
  DWORD lastError              = 0;
  ...
  if(IoEvent && IoEvent->EventResult)
  {
    ...
    eventInfoSize                 = GetEventInfoSize(IoEvent->EventContext->MajorFunction, eventInfo);
    eventInfo->PullEventTimeoutMs = IoBatch->MainPullThread ? /*infinite*/ 0 : DOKAN_PULL_EVENT_TIMEOUT_MS;
    if(ReleaseBatchBuffers)
    {
      PushIoBatchBuffer(IoEvent->IoBatch);
      PushIoEventBuffer(IoEvent);
    }
    DbgPrint("Dokan Information: SendAndPullEventInformation() with NTSTATUS {:#x}, "
             "context {:#x}, and result object {} with size {}",
             eventInfo->Status, eventInfo->Context, (void*)eventInfo, eventInfoSize);
  }
  else
  {
  ...
  if(!DeviceIoControl(IoBatch->DokanInstance->Device,     // Handle to device
                      ...
                      ))
  {
    ...
    {
      DbgPrint("Dokan Error: Dokan device result ioctl failed for wait with code {}.", lastError);
    }
    return lastError;
  }
  if(eventInfo)
...
VOID CALLBACK DispatchBatchIoCallback(PTP_CALLBACK_INSTANCE Instance, PVOID Parameter, PTP_WORK Work)
{
  ...
  while(TRUE)
  {
    ...
    ULONG_PTR currentNumberOfBytesTransferred = ioBatch->NumberOfBytesTransferred;
    ...
    while(eventContextBatchCount)
    {
      ...
      if(!ioEvent)
      {
        DbgPrint("Dokan Error: IoEvent allocation failed.");
...
VOID CALLBACK DispatchDedicatedIoCallback(PTP_CALLBACK_INSTANCE Instance, PVOID Parameter, PTP_WORK Work)
{
  UNREFERENCED_PARAMETER(Instance);
  UNREFERENCED_PARAMETER(Work);
  ...
  while(TRUE)
  {
    ...
    DWORD error = SendAndPullEventInformation(ioEvent, ioBatch, /*ReleaseBatchBuffers=*/FALSE);
    if(error)
    {
      PushIoEventBuffer(ioEvent);
...
BOOL DOKANAPI DokanIsFileSystemRunning(_In_ DOKAN_HANDLE DokanInstance)
{
  ...
  return WaitForSingleObject(instance->DeviceClosedWaitHandle, 0) == WAIT_TIMEOUT;
}

DWORD DOKANAPI DokanWaitForFileSystemClosed(_In_ DOKAN_HANDLE DokanInstance, _In_ DWORD dwMilliseconds)
{
  DOKAN_INSTANCE* instance = (DOKAN_INSTANCE*)DokanInstance;
  if(!instance)
  {
...
BOOL DOKANAPI DokanRegisterWaitForFileSystemClosed(_In_ DOKAN_HANDLE DokanInstance, _Out_ PHANDLE WaitHandle, _In_ WAITORTIMERCALLBACKFUNC Callback, _In_ PVOID Context, ULONG dwMilliseconds)
{
  DOKAN_INSTANCE* instance = (DOKAN_INSTANCE*)DokanInstance;
  if(!instance)
  {
  ...
  return RegisterWaitForSingleObject(WaitHandle, instance->DeviceClosedWaitHandle, Callback, Context, dwMilliseconds, WT_EXECUTEONLYONCE);
}

BOOL DOKANAPI DokanUnregisterWaitForFileSystemClosed(_In_ HANDLE WaitHandle, BOOL WaitForCallbacks)
{
  return UnregisterWaitEx(WaitHandle, WaitForCallbacks ? INVALID_HANDLE_VALUE : NULL);
}
...
int DOKANAPI DokanCreateFileSystem(_In_ PDOKAN_OPTIONS DokanOptions, _In_ PDOKAN_OPERATIONS DokanOperations, _Out_ DOKAN_HANDLE* DokanInstance)
{
  ...
  if(g_DebugMode)
  {
    DbgPrint("Dokan: debug mode on");
  }

  if(g_UseStdErr)
  {
    DbgPrint("Dokan: use stderr");
    g_DebugMode = TRUE;
  }

  if((DokanOptions->Options & DOKAN_OPTION_NETWORK) && !IsMountPointDriveLetter(DokanOptions->MountPoint.data()))
  {
    DokanOptions->Options &= ~DOKAN_OPTION_NETWORK;
    DbgPrint("Dokan: Mount point folder is specified with network device option. Disable network device.");
  }

  if((DokanOptions->Options & DOKAN_OPTION_NETWORK) && DokanOptions->UNCName.empty())
  {
    DbgPrint("Dokan: Network filesystem is enabled without UNC name.");
    return DOKAN_MOUNT_POINT_ERROR;
  }

  if(DokanOptions->Version < DOKAN_MINIMUM_COMPATIBLE_VERSION)
  {
    DbgPrint("Dokan Error: Incompatible version ({}), minimum is ({}) ", DokanOptions->Version, DOKAN_MINIMUM_COMPATIBLE_VERSION);
    return DOKAN_VERSION_ERROR;
  }

  if(DokanOptions->SingleThread)
  {
    DbgPrint("Dokan Info: Single thread mode enabled.");
  }
  ...
  dokanInstance->GlobalDevice    = CreateFileW(DOKAN_GLOBAL_DEVICE_NAME,           // lpFileName
                                               0,                                  // dwDesiredAccess
  ...
  if(DokanOptions->MountPoint.length())
  {
    wcscpy_s(dokanInstance->MountPoint, sizeof(dokanInstance->MountPoint) / sizeof(WCHAR), DokanOptions->MountPoint.data());
    ...
    if(!(DokanOptions->Options & DOKAN_OPTION_MOUNT_MANAGER) && IsMountPointDriveLetter(dokanInstance->MountPoint) && !CheckDriveLetterAvailability(dokanInstance->MountPoint[0]))
    {
      DbgPrint("Dokan Error: CheckDriveLetterAvailability Failed");
      DeleteDokanInstance(dokanInstance);
      return DOKAN_MOUNT_ERROR;
    }
  }

  if(DokanOptions->UNCName.length())
  {
    wcscpy_s(dokanInstance->UNCName, sizeof(dokanInstance->UNCName) / sizeof(WCHAR), DokanOptions->UNCName.data());
  }
  ...
  dokanInstance->Device = CreateFileW(rawDeviceName,                      // lpFileName
                                      0,                                  // dwDesiredAccess
                                      FILE_SHARE_READ | FILE_SHARE_WRITE, // dwShareMode
                                      NULL,                               // lpSecurityAttributes
                                      OPEN_EXISTING,                      // dwCreationDistribution
  ...
  {
    DbgPrint("Dokan Error: GetProcessAffinityMask failed with Error {}", GetLastError());
  }
  if(DokanOptions->SingleThread)
  {
  ...
  DbgPrint("Dokan: Using {} main pull threads", mainPullThreadCount);
  for(DWORD x = 0; x < mainPullThreadCount; ++x)
  {
    ...
    QueueIoEvent(ioEvent, DokanOptions->Options & DOKAN_OPTION_ALLOW_IPC_BATCHING ? DispatchBatchIoCallback : DispatchDedicatedIoCallback);
  }

  if(!DokanMount(dokanInstance->MountPoint, dokanInstance->DeviceName, DokanOptions))
  {
    SendReleaseIRP(dokanInstance->DeviceName);
    DbgPrint("Dokan Error: DokanMount Failed");
    DeleteDokanInstance(dokanInstance);
    return DOKAN_MOUNT_ERROR;
  }

  wchar_t keepalive_path[128];
  StringCbPrintfW(keepalive_path, sizeof(keepalive_path), L"\\\\?{}", dokanInstance->DeviceName, DOKAN_KEEPALIVE_FILE_NAME);
  dokanInstance->KeepaliveHandle = CreateFileW(keepalive_path, 0, 0, NULL, OPEN_EXISTING, 0, NULL);
  if(dokanInstance->KeepaliveHandle == INVALID_HANDLE_VALUE)
  {
    ...
    DbgPrint("Failed to open keepalive file: {} error {}", rsl::to_string(rsl::wstring_view(keepalive_path)), GetLastError());
  }
  else
  {
    DWORD keepalive_bytes_returned = 0;
    if(!DeviceIoControl(dokanInstance->KeepaliveHandle, FSCTL_ACTIVATE_KEEPALIVE, NULL, 0, NULL, 0, &keepalive_bytes_returned, NULL))
      DbgPrint("Failed to activate keepalive handle.");
  }

  wchar_t notify_path[128];
  StringCbPrintfW(notify_path, sizeof(notify_path), L"\\\\?{}", dokanInstance->DeviceName, DOKAN_NOTIFICATION_FILE_NAME);
  ...
  DbgPrint("Dokan Information: mounted: {} -> {}", rsl::to_string(rsl::wstring_view(dokanInstance->MountPoint)), rsl::to_string(rsl::wstring_view(dokanInstance->DeviceName)));
...
VOID ALIGN_ALLOCATION_SIZE(PLARGE_INTEGER size, PDOKAN_OPTIONS DokanOptions)
{
  ...
  size->QuadPart = (size->QuadPart + (r > 0 ? DokanOptions->AllocationUnitSize - r : 0));
}
...
ULONG DispatchGetEventInformationLength(ULONG bufferSize)
{
  ...
  return max((ULONG)sizeof(EVENT_INFORMATION), FIELD_OFFSET(EVENT_INFORMATION, Buffer[0]) + bufferSize);
}
...
VOID CreateDispatchCommon(PDOKAN_IO_EVENT IoEvent, ULONG SizeOfEventInfo, BOOL UseExtraMemoryPool, BOOL ClearNonPoolBuffer)
{
  ...
  else
  {
    ...
    {
      IoEvent->EventResultSize = DispatchGetEventInformationLength(SizeOfEventInfo);
      IoEvent->EventResult     = (PEVENT_INFORMATION)malloc(IoEvent->EventResultSize);
      if(!IoEvent->EventResult)
      {
        return;
      }
      ZeroMemory(IoEvent->EventResult, ClearNonPoolBuffer ? IoEvent->EventResultSize : FIELD_OFFSET(EVENT_INFORMATION, Buffer[0]));
    }
  }
  assert(IoEvent->EventResult && IoEvent->EventResultSize >= DispatchGetEventInformationLength(SizeOfEventInfo));
...
VOID ReleaseDokanOpenInfo(PDOKAN_IO_EVENT IoEvent)
{
  ...
  {
    IoEvent->DokanOpenInfo->CloseFileName    = _wcsdup(IoEvent->EventContext->Operation.Close.FileName);
  ...
  if(fileNameForClose)
  {
    ...
    {
      IoEvent->DokanInstance->DokanOperations->CloseFile(fileNameForClose, &IoEvent->DokanFileInfo);
    }
...
BOOL SendReleaseIRP(LPCWSTR DeviceName)
{
  ...
  DbgPrint("send release to {}", rsl::to_string(rsl::wstring_view(DeviceName)));

  GetRawDeviceName(DeviceName, rawDeviceName, MAX_PATH);
  if(!SendToDevice(rawDeviceName, FSCTL_EVENT_RELEASE, NULL, 0, NULL, 0, &returnedLength))
  {
    DbgPrint("Failed to unmount device: {}", rsl::to_string(rsl::wstring_view(DeviceName)));
    return FALSE;
  }
...
BOOL SendGlobalReleaseIRP(LPCWSTR MountPoint)
{
  if(MountPoint != NULL)
  {
    ...
    {
      ULONG returnedLength;
      ULONG inputLength                               = sizeof(DOKAN_UNICODE_STRING_INTERMEDIATE) + (MAX_PATH * sizeof(WCHAR));
      ...
      if(szMountPoint != NULL)
      {
        ...
        DbgPrint("Send global Release for {}", rsl::to_string(rsl::wstring_view(MountPoint)));

        if(!SendToDevice(DOKAN_GLOBAL_DEVICE_NAME, FSCTL_EVENT_RELEASE, szMountPoint, inputLength, NULL, 0, &returnedLength))
        {
          DbgPrint("Failed to unmount: {}", rsl::to_string(rsl::wstring_view(MountPoint)));
          free(szMountPoint);
          return FALSE;
        }
...
int DokanStart(_In_ PDOKAN_INSTANCE DokanInstance)
{
  ...
  if(DokanInstance->DokanOptions->Options & DOKAN_OPTION_ENABLE_UNMOUNT_NETWORK_DRIVE)
  {
  ...
  if(driverLetter && mountManager && !CheckDriveLetterAvailability(DokanInstance->MountPoint[0]))
  {
  ...
  {
    if(DokanInstance->DokanOptions->VolumeSecurityDescriptorLength > VOLUME_SECURITY_DESCRIPTOR_MAX_SIZE)
    {
      DbgPrint("Dokan Error: Invalid volume security descriptor length "
                    "provided %ld",
                    DokanInstance->DokanOptions->VolumeSecurityDescriptorLength);
      return DOKAN_START_ERROR;
    }
    eventStart.VolumeSecurityDescriptorLength = DokanInstance->DokanOptions->VolumeSecurityDescriptorLength;
    memcpy_s(eventStart.VolumeSecurityDescriptor, sizeof(eventStart.VolumeSecurityDescriptor), DokanInstance->DokanOptions->VolumeSecurityDescriptor, sizeof(DokanInstance->DokanOptions->VolumeSecurityDescriptor));
  }

  memcpy_s(eventStart.MountPoint, sizeof(eventStart.MountPoint), DokanInstance->MountPoint, sizeof(DokanInstance->MountPoint));
  memcpy_s(eventStart.UNCName, sizeof(eventStart.UNCName), DokanInstance->UNCName, sizeof(DokanInstance->UNCName));

  eventStart.IrpTimeout                     = DokanInstance->DokanOptions->Timeout;
  eventStart.FcbGarbageCollectionIntervalMs = 2000;

  SendToDevice(DOKAN_GLOBAL_DEVICE_NAME, FSCTL_EVENT_START, &eventStart, sizeof(EVENT_START), &driverInfo, sizeof(EVENT_DRIVER_INFO), &returnedLength);

  if(driverInfo.Status == DOKAN_START_FAILED)
  {
    if(driverInfo.DriverVersion != eventStart.UserVersion)
    {
      REX_ERROR(LogDokan, "Dokan Error: driver version mismatch, driver %X, dll %X", driverInfo.DriverVersion, eventStart.UserVersion);
      return DOKAN_VERSION_ERROR;
    }
  ...
  else if(driverInfo.Status == DOKAN_MOUNTED)
  {
    ...
    wcscpy_s(DokanInstance->DeviceName, sizeof(DokanInstance->DeviceName) / sizeof(WCHAR), driverInfo.DeviceName);
...
{
  ULONG returnedLength;
  return SendToDevice(DOKAN_GLOBAL_DEVICE_NAME, FSCTL_SET_DEBUG_MODE, &Mode, sizeof(ULONG), NULL, 0, &returnedLength);
}

BOOL DOKANAPI DokanMountPointsCleanUp()
{
  ULONG returnedLength;
  return SendToDevice(DOKAN_GLOBAL_DEVICE_NAME, FSCTL_MOUNTPOINT_CLEANUP, NULL, 0, NULL, 0, &returnedLength);
}

BOOL SendToDevice(LPCWSTR DeviceName, DWORD IoControlCode, PVOID InputBuffer, ULONG InputLength, PVOID OutputBuffer, ULONG OutputLength, PULONG ReturnedLength)
{
  HANDLE device;
  BOOL status;

  device = CreateFileW(DeviceName,                         // lpFileName
                       0,                                  // dwDesiredAccess
                       FILE_SHARE_READ | FILE_SHARE_WRITE, // dwShareMode
                       NULL,                               // lpSecurityAttributes
                       OPEN_EXISTING,                      // dwCreationDistribution
  ...
  if(!status)
  {
    DbgPrint("DokanError: Ioctl {:#x} failed with code {} on Device {}", IoControlCode, GetLastError(), rsl::to_string(rsl::wstring_view(DeviceName)));
    return FALSE;
  }
...
PDOKAN_MOUNT_POINT_INFO DOKANAPI DokanGetMountPointList(BOOL uncOnly, PULONG nbRead)
{
  ...
  *nbRead = 0;
  ...
  do
  {
    ...
    dokanMountPointInfo = (PDOKAN_MOUNT_POINT_INFO)malloc(bufferLength);
    if(dokanMountPointInfo == NULL)
      return NULL;
    ZeroMemory(dokanMountPointInfo, bufferLength);

    success = SendToDevice(DOKAN_GLOBAL_DEVICE_NAME, FSCTL_EVENT_MOUNTPOINT_LIST, NULL, 0, dokanMountPointInfo, bufferLength, &returnedLength);
  ...
  *nbRead = returnedLength / sizeof(DOKAN_MOUNT_POINT_INFO);
  results = (PDOKAN_MOUNT_POINT_INFO)malloc(returnedLength);
  if(results != NULL)
  {
    ...
    for(ULONG i = 0; i < *nbRead; ++i)
    {
      if(!uncOnly || wcscmp(dokanMountPointInfo[i].UNCName, L"") != 0)
        CopyMemory(&results[i], &dokanMountPointInfo[i], sizeof(DOKAN_MOUNT_POINT_INFO));
    }
  }
  free(dokanMountPointInfo);
  return results;
...
VOID DOKANAPI DokanMapKernelToUserCreateFileFlags(ACCESS_MASK DesiredAccess, ULONG FileAttributes, ULONG CreateOptions, ULONG CreateDisposition, ACCESS_MASK* outDesiredAccess, DWORD* outFileAttributesAndFlags, DWORD* outCreationDisposition)
{
  ...
  if(outCreationDisposition)
  {
    ...
    {
      case FILE_CREATE: *outCreationDisposition = CREATE_NEW; break;
      case FILE_OPEN: *outCreationDisposition = OPEN_EXISTING; break;
      case FILE_OPEN_IF: *outCreationDisposition = OPEN_ALWAYS; break;
      case FILE_OVERWRITE: *outCreationDisposition = TRUNCATE_EXISTING; break;
      case FILE_SUPERSEDE:
      // The documentation isn't clear on the difference between replacing a file
      // and truncating it.
      // For now we just map it to create/truncate
      case FILE_OVERWRITE_IF: *outCreationDisposition = CREATE_ALWAYS; break;
      default: *outCreationDisposition = 0; break;
    }
  }
...
VOID DOKANAPI DokanInit()
{
  ...
  (void)InitializeCriticalSectionAndSpinCount(&g_InstanceCriticalSection, 0x80000400);
...
VOID DOKANAPI DokanShutdown()
{
  ...
  EnterCriticalSection(&g_InstanceCriticalSection);
  {
    while(!IsListEmpty(&g_InstanceList))
    {
      ...
      PDOKAN_INSTANCE dokanInstance = CONTAINING_RECORD(entry, DOKAN_INSTANCE, ListEntry);
...
BOOL DOKANAPI DokanNotifyPath(_In_ DOKAN_HANDLE DokanInstance, _In_ LPCWSTR FilePath, _In_ ULONG CompletionFilter, _In_ ULONG Action)
{
  DOKAN_INSTANCE* instance = (DOKAN_INSTANCE*)DokanInstance;
  ...
  length -= prefixSize;
  ULONG returnedLength;
  ULONG inputLength                           = (ULONG)(sizeof(DOKAN_NOTIFY_PATH_INTERMEDIATE) + (length * sizeof(WCHAR)));
  ...
  if(!DeviceIoControl(instance->NotifyHandle, FSCTL_NOTIFY_PATH, pNotifyPath, inputLength, NULL, 0, &returnedLength, NULL))
  {
    DbgPrint("Failed to send notify path command:{}", rsl::to_string(rsl::wstring_view(FilePath)));
    free(pNotifyPath);
    return FALSE;
  }
  free(pNotifyPath);
...
BOOL DOKANAPI DokanNotifyCreate(_In_ DOKAN_HANDLE DokanInstance, _In_ LPCWSTR FilePath, _In_ BOOL IsDirectory)
{
  return DokanNotifyPath(DokanInstance, FilePath, IsDirectory ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_ADDED);
}

BOOL DOKANAPI DokanNotifyDelete(_In_ DOKAN_HANDLE DokanInstance, _In_ LPCWSTR FilePath, _In_ BOOL IsDirectory)
{
  return DokanNotifyPath(DokanInstance, FilePath, IsDirectory ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, FILE_ACTION_REMOVED);
}

BOOL DOKANAPI DokanNotifyUpdate(_In_ DOKAN_HANDLE DokanInstance, _In_ LPCWSTR FilePath)
{
  return DokanNotifyPath(DokanInstance, FilePath, FILE_NOTIFY_CHANGE_ATTRIBUTES, FILE_ACTION_MODIFIED);
}

BOOL DOKANAPI DokanNotifyXAttrUpdate(_In_ DOKAN_HANDLE DokanInstance, _In_ LPCWSTR FilePath)
{
  return DokanNotifyPath(DokanInstance, FilePath, FILE_NOTIFY_CHANGE_ATTRIBUTES, FILE_ACTION_MODIFIED);
}
...
{
  BOOL success = DokanNotifyPath(DokanInstance, OldPath, IsDirectory ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, IsInSameDirectory ? FILE_ACTION_RENAMED_OLD_NAME : FILE_ACTION_REMOVED);
  success &= DokanNotifyPath(DokanInstance, NewPath, IsDirectory ? FILE_NOTIFY_CHANGE_DIR_NAME : FILE_NOTIFY_CHANGE_FILE_NAME, IsInSameDirectory ? FILE_ACTION_RENAMED_NEW_NAME : FILE_ACTION_ADDED);
  return success;
}
