...
NTSTATUS
DokanFillFileBasicInfo(PFILE_BASIC_INFORMATION BasicInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength)
{
...
NTSTATUS
DokanFillFileStandardInfo(PFILE_STANDARD_INFORMATION StandardInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength, PDOKAN_FILE_INFO DokanFileInfo, PDOKAN_INSTANCE DokanInstance)
{
  ...
  ALIGN_ALLOCATION_SIZE(&StandardInfo->AllocationSize, DokanInstance->DokanOptions);
...
NTSTATUS
DokanFillFilePositionInfo(PFILE_POSITION_INFORMATION PosInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength)
{
...
NTSTATUS
DokanFillInternalInfo(PFILE_INTERNAL_INFORMATION InternalInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength)
{
...
NTSTATUS
DokanFillFileAllInfo(PFILE_ALL_INFORMATION AllInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength, PDOKAN_FILE_INFO DokanFileInfo, PDOKAN_INSTANCE DokanInstance)
{
  ...
  DokanFillFileStandardInfo(&AllInfo->StandardInformation, FileInfo, RemainingLength, DokanFileInfo, DokanInstance);
  ...
  allRemainingLength -= FIELD_OFFSET(FILE_ALL_INFORMATION, NameInformation.FileName);
...
NTSTATUS
DokanFillFileNameInfo(PFILE_NAME_INFORMATION NameInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength, PEVENT_CONTEXT EventContext)
{
  UNREFERENCED_PARAMETER(FileInfo);

  if(*RemainingLength < sizeof(FILE_NAME_INFORMATION) + EventContext->Operation.File.FileNameLength)
  {
    return STATUS_BUFFER_OVERFLOW;
  }

  NameInfo->FileNameLength = EventContext->Operation.File.FileNameLength;
  RtlCopyMemory(&(NameInfo->FileName[0]), EventContext->Operation.File.FileName, EventContext->Operation.File.FileNameLength);
...
NTSTATUS
DokanFillFileAttributeTagInfo(PFILE_ATTRIBUTE_TAG_INFORMATION AttrTagInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength)
{
...
NTSTATUS
DokanFillNetworkOpenInfo(PFILE_NETWORK_OPEN_INFORMATION NetInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength, PDOKAN_INSTANCE DokanInstance)
{
...
NTSTATUS
DokanFillIdInfo(PFILE_ID_INFORMATION IdInfo, PBY_HANDLE_FILE_INFORMATION FileInfo, PULONG RemainingLength)
{
...
BOOL WINAPI DokanFillFindStreamData(PWIN32_FIND_STREAM_DATA FindStreamData, PVOID FindStreamContext)
{
  PDOKAN_IO_EVENT ioEvent = (PDOKAN_IO_EVENT)FindStreamContext;
  ULONG offset            = (ULONG)ioEvent->EventResult->BufferLength;
  ULONG resultBufferSize  = ioEvent->EventContext->Operation.File.BufferLength;

  ULONG streamNameLength = (ULONG)wcslen(FindStreamData->cStreamName) * sizeof(WCHAR);
...
VOID DOKANAPI DokanEndDispatchGetFileInformation(PDOKAN_IO_EVENT IoEvent, PBY_HANDLE_FILE_INFORMATION ByHandleFileInfo, NTSTATUS Status)
{
  ULONG remainingLength = IoEvent->EventContext->Operation.File.BufferLength;

  DbgPrint("\tresult =  {}", Status);
  ...
  else
  {
    switch(IoEvent->EventContext->Operation.File.FileInformationClass)
    {
      case FileBasicInformation:
        ...
        Status = DokanFillFileBasicInfo((PFILE_BASIC_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength);
        break;

      case FileIdInformation:
        DbgPrint("\tFileIdInformation");
        Status = DokanFillIdInfo((PFILE_ID_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength);
        break;

      case FileInternalInformation:
        DbgPrint("\tFileInternalInformation");
        Status = DokanFillInternalInfo((PFILE_INTERNAL_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength);
        break;
      ...
      case FileStandardInformation:
        ...
        Status = DokanFillFileStandardInfo((PFILE_STANDARD_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength, &IoEvent->DokanFileInfo, IoEvent->DokanInstance);
        break;

      case FileAllInformation:
        DbgPrint("\tFileAllInformation");
        Status = DokanFillFileAllInfo((PFILE_ALL_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength, &IoEvent->DokanFileInfo, IoEvent->DokanInstance);
        break;
      ...
      case FileAttributeTagInformation:
        ...
        Status = DokanFillFileAttributeTagInfo((PFILE_ATTRIBUTE_TAG_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength);
        break;
      ...
      case FileNormalizedNameInformation: DbgPrint("\tFileNormalizedNameInformation");
      case FileNameInformation:
        // this case is not used because driver deal with
        DbgPrint("\tFileNameInformation");
        Status = DokanFillFileNameInfo((PFILE_NAME_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength, IoEvent->EventContext);
        break;

      case FileNetworkOpenInformation:
        DbgPrint("\tFileNetworkOpenInformation");
        Status = DokanFillNetworkOpenInfo((PFILE_NETWORK_OPEN_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength, IoEvent->DokanInstance);
        break;

      case FilePositionInformation:
        // this case is not used because driver deal with
        DbgPrint("\tFilePositionInformation");
        Status = DokanFillFilePositionInfo((PFILE_POSITION_INFORMATION)IoEvent->EventResult->Buffer, ByHandleFileInfo, &remainingLength);
        break;
      ...
      default:
      {
        ...
        DbgPrint("  unknown type:", IoEvent->EventContext->Operation.File.FileInformationClass);
      }
      break;
    }

    IoEvent->EventResult->Status       = Status;
    IoEvent->EventResult->BufferLength = IoEvent->EventContext->Operation.File.BufferLength - remainingLength;
  }

  DbgPrint("\tDispatchQueryInformation result =  {}", Status);
  EventCompletion(IoEvent);
}

VOID DOKANAPI DokanEndDispatchFindStreams(PDOKAN_IO_EVENT IoEvent, NTSTATUS Status)
{
  ULONG resultBufferSize              = IOEVENT_RESULT_BUFFER_SIZE(IoEvent);
  PFILE_STREAM_INFORMATION streamInfo = (PFILE_STREAM_INFORMATION)&IoEvent->EventResult->Buffer[IoEvent->EventResult->BufferLength];
  ...
  IoEvent->EventResult->Status = Status;
  DbgPrint("\tDokanEndDispatchFindStreams result =  {}", Status);
  EventCompletion(IoEvent);
}
...
VOID DispatchQueryInformation(PDOKAN_IO_EVENT IoEvent)
{
  ...
  DbgPrint("###GetFileInfo file handle = {}, eventID = {}, event Info = {}, filename = {}", (void*)IoEvent->DokanOpenInfo, IoEvent->DokanOpenInfo != NULL ? IoEvent->DokanOpenInfo->EventId : -1, (void*)IoEvent, rsl::to_string(rsl::wstring_view(IoEvent->EventContext->Operation.File.FileName)));

  CheckFileName(IoEvent->EventContext->Operation.File.FileName);

  CreateDispatchCommon(IoEvent, IoEvent->EventContext->Operation.File.BufferLength,
                       /*UseExtraMemoryPool=*/FALSE,
                       /*ClearNonPoolBuffer=*/TRUE);

  if(IoEvent->EventContext->Operation.File.FileInformationClass == FileStreamInformation)
  {
    DbgPrint("FileStreamInformation");
    // https://msdn.microsoft.com/en-us/library/windows/hardware/ff540364(v=vs.85).aspx
    if(IoEvent->EventContext->Operation.File.BufferLength < sizeof(FILE_STREAM_INFORMATION))
    {
      status = STATUS_BUFFER_TOO_SMALL;
    }
    else if(IoEvent->DokanInstance->DokanOperations->FindStreams)
    {
      status = IoEvent->DokanInstance->DokanOperations->FindStreams(IoEvent->EventContext->Operation.File.FileName, DokanFillFindStreamData, IoEvent, &IoEvent->DokanFileInfo);
  ...
  else if(IoEvent->DokanInstance->DokanOperations->GetFileInformation)
  {
    ...
    status = IoEvent->DokanInstance->DokanOperations->GetFileInformation(IoEvent->EventContext->Operation.File.FileName, &byHandleFileInfo, &IoEvent->DokanFileInfo);
