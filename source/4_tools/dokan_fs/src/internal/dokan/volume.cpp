...
NTSTATUS DOKAN_CALLBACK DokanGetDiskFreeSpace(PULONGLONG FreeBytesAvailable, PULONGLONG TotalNumberOfBytes, PULONGLONG TotalNumberOfFreeBytes, PDOKAN_FILE_INFO DokanFileInfo)
{
...
NTSTATUS DOKAN_CALLBACK DokanGetDefaultVolumeInformation(LPWSTR VolumeNameBuffer, DWORD VolumeNameSize, LPDWORD VolumeSerialNumber, LPDWORD MaximumComponentLength, LPDWORD FileSystemFlags, LPWSTR FileSystemNameBuffer, DWORD FileSystemNameSize,
...
NTSTATUS DokanGetVolumeInformation(PDOKAN_OPERATIONS DokanOperations, LPWSTR VolumeNameBuffer, DWORD VolumeNameSize, LPDWORD VolumeSerialNumber, LPDWORD MaximumComponentLength, LPDWORD FileSystemFlags, LPWSTR FileSystemNameBuffer,
                                   DWORD FileSystemNameSize, PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  {
    status = DokanOperations->GetVolumeInformation(VolumeNameBuffer, VolumeNameSize, VolumeSerialNumber, MaximumComponentLength, FileSystemFlags, FileSystemNameBuffer, FileSystemNameSize, DokanFileInfo);
  }

  if(status == STATUS_NOT_IMPLEMENTED)
  {
    status = DokanGetDefaultVolumeInformation(VolumeNameBuffer, VolumeNameSize, VolumeSerialNumber, MaximumComponentLength, FileSystemFlags, FileSystemNameBuffer, FileSystemNameSize, DokanFileInfo);
  }
...
NTSTATUS
DokanFsVolumeInformation(PEVENT_INFORMATION EventInfo, PEVENT_CONTEXT EventContext, PDOKAN_FILE_INFO FileInfo, PDOKAN_OPERATIONS DokanOperations)
{
  ...
  status = DokanGetVolumeInformation(DokanOperations,
...
NTSTATUS
DokanFsSizeInformation(PEVENT_INFORMATION EventInfo, PEVENT_CONTEXT EventContext, PDOKAN_FILE_INFO FileInfo, PDOKAN_OPERATIONS DokanOperations)
{
  ...
  {
    status = DokanOperations->GetDiskFreeSpace(&freeBytesAvailable, // FreeBytesAvailable
...
NTSTATUS
DokanFsAttributeInformation(PEVENT_INFORMATION EventInfo, PEVENT_CONTEXT EventContext, PDOKAN_FILE_INFO FileInfo, PDOKAN_OPERATIONS DokanOperations)
{
  WCHAR volumeName[MAX_PATH];
  DWORD volumeSerial;
  DWORD maxComLength = 0;
...
NTSTATUS
DokanFsFullSizeInformation(PEVENT_INFORMATION EventInfo, PEVENT_CONTEXT EventContext, PDOKAN_FILE_INFO FileInfo, PDOKAN_OPERATIONS DokanOperations)
{
  ULONGLONG freeBytesAvailable = 0;
  ULONGLONG totalBytes         = 0;
  ULONGLONG freeBytes          = 0;
...
{
  CreateDispatchCommon(IoEvent, IoEvent->EventContext->Operation.Volume.BufferLength,
                       /*UseExtraMemoryPool=*/FALSE,
                       /*ClearNonPoolBuffer=*/TRUE);

  DbgPrint("###QueryVolumeInfo file handle = {}, eventID = {}, event Info = {}",
           (void*)IoEvent->DokanOpenInfo, IoEvent->DokanOpenInfo != NULL ? IoEvent->DokanOpenInfo->EventId : -1, (void*)IoEvent);

  IoEvent->EventResult->Status = STATUS_INVALID_PARAMETER;

  switch(IoEvent->EventContext->Operation.Volume.FsInformationClass)
  {
    case FileFsVolumeInformation: IoEvent->EventResult->Status = DokanFsVolumeInformation(IoEvent->EventResult, IoEvent->EventContext, &IoEvent->DokanFileInfo, IoEvent->DokanInstance->DokanOperations); break;
    case FileFsSizeInformation: IoEvent->EventResult->Status = DokanFsSizeInformation(IoEvent->EventResult, IoEvent->EventContext, &IoEvent->DokanFileInfo, IoEvent->DokanInstance->DokanOperations); break;
    case FileFsAttributeInformation: IoEvent->EventResult->Status = DokanFsAttributeInformation(IoEvent->EventResult, IoEvent->EventContext, &IoEvent->DokanFileInfo, IoEvent->DokanInstance->DokanOperations); break;
    case FileFsFullSizeInformation: IoEvent->EventResult->Status = DokanFsFullSizeInformation(IoEvent->EventResult, IoEvent->EventContext, &IoEvent->DokanFileInfo, IoEvent->DokanInstance->DokanOperations); break;
    default: DbgPrint("error unknown volume info {}", IoEvent->EventContext->Operation.Volume.FsInformationClass);
  }
