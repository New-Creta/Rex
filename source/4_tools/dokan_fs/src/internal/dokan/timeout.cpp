...
BOOL DOKANAPI DokanResetTimeout(ULONG Timeout, PDOKAN_FILE_INFO FileInfo)
{
  ...
  status = SendToDevice(rawDeviceName, FSCTL_RESET_TIMEOUT, eventInfo, eventInfoSize, NULL, 0, &returnedLength);
  if(!status)
  {
    DbgPrint("Failed to Reset Timeout for {} with timeout: {}", ioEvent->EventContext->SerialNumber, Timeout);
  }
  free(eventInfo);
  return status;
}