...
VOID SetIOSecurityContext(PEVENT_CONTEXT EventContext, PDOKAN_IO_SECURITY_CONTEXT ioSecurityContext)
{
...
BOOL CreateSuccesStatusCheck(NTSTATUS status, ULONG disposition)
{
  ...
  if(status == STATUS_OBJECT_NAME_COLLISION && (disposition == FILE_OPEN_IF || disposition == FILE_SUPERSEDE || disposition == FILE_OVERWRITE_IF))
    return TRUE;
...
VOID DispatchCreate(PDOKAN_IO_EVENT IoEvent)
{
  ...
  fileName = (WCHAR*)((PCHAR)&IoEvent->EventContext->Operation.Create + IoEvent->EventContext->Operation.Create.FileNameOffset);
  ...
  disposition = (IoEvent->EventContext->Operation.Create.CreateOptions >> 24) & 0x000000ff;
  // The low 24 bits of this member correspond to the CreateOptions parameter
  options = IoEvent->EventContext->Operation.Create.CreateOptions & FILE_VALID_OPTION_FLAGS;
  ...
  DbgPrint("###Create file handle = {}, eventID = {}, event Info = {}, Filepath = {}", (void*)IoEvent->DokanOpenInfo, currentEventId, (void*)IoEvent, rsl::to_string(rsl::wstring_view(fileName)));

  if(IoEvent->DokanInstance->DokanOperations->ZwCreateFile)
  {
    SetIOSecurityContext(IoEvent->EventContext, &ioSecurityContext);

    if((IoEvent->EventContext->Flags & SL_OPEN_TARGET_DIRECTORY) && IoEvent->DokanInstance->DokanOperations->Cleanup && IoEvent->DokanInstance->DokanOperations->CloseFile)
    {
      if(options & FILE_NON_DIRECTORY_FILE && options & FILE_DIRECTORY_FILE)
        status = STATUS_INVALID_PARAMETER;
      else
        status = IoEvent->DokanInstance->DokanOperations->ZwCreateFile(origFileName, &ioSecurityContext, ioSecurityContext.DesiredAccess, IoEvent->EventContext->Operation.Create.FileAttributes, IoEvent->EventContext->Operation.Create.ShareAccess,
                                                                       disposition, origOptions, &IoEvent->DokanFileInfo);
      ...
      if(CreateSuccesStatusCheck(status, disposition))
      {
        ...
        IoEvent->DokanInstance->DokanOperations->CloseFile(origFileName, &IoEvent->DokanFileInfo);
      }
    ...
    if(options & FILE_NON_DIRECTORY_FILE && options & FILE_DIRECTORY_FILE)
      ...
      status = IoEvent->DokanInstance->DokanOperations->ZwCreateFile(fileName, &ioSecurityContext, ioSecurityContext.DesiredAccess, IoEvent->EventContext->Operation.Create.FileAttributes, IoEvent->EventContext->Operation.Create.ShareAccess,
                                                                     disposition, options, &IoEvent->DokanFileInfo);
  ...
  if(!CreateSuccesStatusCheck(status, disposition))
  {
    ...
    if(STATUS_ACCESS_DENIED == status && IoEvent->DokanInstance->DokanOperations->ZwCreateFile && (IoEvent->EventContext->Operation.Create.SecurityContext.DesiredAccess & DELETE))
    {
      ...
      SetIOSecurityContext(IoEvent->EventContext, &ioSecurityContext);
      ACCESS_MASK newDesiredAccess = (MAXIMUM_ALLOWED & ioSecurityContext.DesiredAccess) ? (FILE_DELETE_CHILD | FILE_LIST_DIRECTORY)
                                                                                         : (((DELETE & ioSecurityContext.DesiredAccess) ? FILE_DELETE_CHILD : 0) | ((FILE_READ_ATTRIBUTES & ioSecurityContext.DesiredAccess) ? FILE_LIST_DIRECTORY : 0));

      options |= FILE_OPEN_FOR_BACKUP_INTENT; // Enable open directory
      options &= ~FILE_NON_DIRECTORY_FILE;    // Remove non dir flag

      status = IoEvent->DokanInstance->DokanOperations->ZwCreateFile(fileName, &ioSecurityContext, newDesiredAccess, IoEvent->EventContext->Operation.Create.FileAttributes, IoEvent->EventContext->Operation.Create.ShareAccess, disposition, options,
      ...
      {
        DbgPrint("Parent CreateFile failed status = %lx", status);
  ...
  DbgPrint("Dokan Information: DokanEndDispatchCreate() status = {}, file "
           "handle = {}, eventID = {}, result = {}",
           IoEvent->EventResult->Status, (void*)IoEvent->DokanOpenInfo, IoEvent->DokanOpenInfo ? IoEvent->DokanOpenInfo->EventId : -1, IoEvent->EventResult->Operation.Create.Information);
}
