...
namespace rex
{
  ...
  public:
    //-------------------------------------------------------------------------
    FrameData(const renderer::ResourceSlot& frameSlot)
    ...
    void set_object_committed_resource_slot(s32 frameIndex, s32 numItems, s32 bufferByteSize)
    {
      ...
      m_object_committed_resource_slot = renderer::attach_committed_resource_to_frame(rsl::move(attach_object_constants));
    }
    ...
    void set_pass_committed_resource_slot(s32 frameIndex, s32 numItems, s32 bufferByteSize)
    {
      ...
      m_pass_committed_resource_slot = renderer::attach_committed_resource_to_frame(rsl::move(attach_object_constants));
    }
    ...
    //-------------------------------------------------------------------------
    const renderer::ResourceSlot& frame_slot() const
    {
      return m_frame_slot;
    }

    //-------------------------------------------------------------------------
    const renderer::ResourceSlot& object_committed_resource_slot() const
    {
      return m_object_committed_resource_slot;
    }

    //-------------------------------------------------------------------------
    const renderer::ResourceSlot& pass_committed_resource_slot() const
    {
      return m_pass_committed_resource_slot;
    }

    //-------------------------------------------------------------------------
    const rsl::vector<renderer::ResourceSlot>& object_constant_buffer_slots() const
    {
      return m_object_constant_buffer_slots;
    }

    //-------------------------------------------------------------------------
    const renderer::ResourceSlot& pass_constant_buffer_slot() const
    {
      return m_pass_constant_buffer_slot;
    }

  private:
    renderer::ResourceSlot m_frame_slot;
    renderer::ResourceSlot m_object_committed_resource_slot;
    renderer::ResourceSlot m_pass_committed_resource_slot;

    rsl::vector<renderer::ResourceSlot> m_object_constant_buffer_slots;
    renderer::ResourceSlot m_pass_constant_buffer_slot;
  };
  ...
  renderer::ResourceSlot get_object_committed_resource_of_frame(const renderer::ResourceSlot* frame)
  {
    ...
    return it != rsl::cend(g_regina_ctx.frame_resource_data) ? it->object_committed_resource_slot() : renderer::ResourceSlot::make_invalid();
  }
  //-------------------------------------------------------------------------
  renderer::ResourceSlot get_pass_committed_resource_of_frame(const renderer::ResourceSlot* frame)
  {
    ...
    return it != rsl::cend(g_regina_ctx.frame_resource_data) ? it->pass_committed_resource_slot() : renderer::ResourceSlot::make_invalid();
  }
  //-------------------------------------------------------------------------
  renderer::ResourceSlot get_active_object_constant_buffer_for_frame(s32 frame, s32 idx)
  {
    ...
    return it != rsl::cend(g_regina_ctx.frame_resource_data) ? it->object_constant_buffer_slots()[idx] : renderer::ResourceSlot::make_invalid();
  }
  //-------------------------------------------------------------------------
  renderer::ResourceSlot get_active_pass_constant_buffer_for_frame(s32 frame)
  {
    ...
    return it != rsl::cend(g_regina_ctx.frame_resource_data) ? it->pass_constant_buffer_slot() : renderer::ResourceSlot::make_invalid();
  }
  ...
  bool build_shader_and_input_layout()
  {
    ...
    link_shader_command_desc.constants     = {renderer::commands::ConstantLayoutDescription {renderer::commands::ConstantType::CBUFFER, "ObjectConstants", 0},
    ...
    input_layout_command_desc.input_layout = {renderer::commands::InputLayoutDescription {"POSITION", 0, renderer::VertexBufferFormat::FLOAT3, 0, 0, renderer::InputLayoutClassification::PER_VERTEX_DATA, 0},
  ...
  bool build_frame_resources()
  {
    ...
    {
      renderer::ResourceSlot frame_slot = renderer::create_frame_resource();
