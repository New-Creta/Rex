...
//    rsl::vector<rex::VertexPosNormCol> vertices;
//    rsl::vector<u16> indices;
//
//    const rex::Submesh box      = build_submesh(rex::mesh_factory::create_box(1.5f, 1.5f, 1.5f), vertices, indices);
//    const rex::Submesh grid     = build_submesh(rex::mesh_factory::create_grid(20.0f, 30.0f, 60, 40), vertices, indices);
//    const rex::Submesh sphere   = build_submesh(rex::mesh_factory::create_sphere(0.5f, 20, 20), vertices, indices);
//    const rex::Submesh cylinder = build_submesh(rex::mesh_factory::create_cylinder(0.5f, 0.3f, 3.0f, 20, 20), vertices, indices);
//
//    // Create the descs for the vertex buffer and index buffer
//    m_vb = rex::memory::Blob(rsl::make_unique<rex::VertexPosNormCol[]>(vertices.size())); // NOLINT(modernize-avoid-c-arrays)
//    m_vb.write(vertices.data(), m_vb.size());
//    rex::VertexBufferDesc vb_desc {rex::memory::BlobView(m_vb), sizeof(rex::VertexPosNormCol)/*, rex::VertexPosCol::layout()*/};
//
//    m_ib = rex::memory::Blob(rsl::make_unique<u16[]>(indices.size())); // NOLINT(modernize-avoid-c-arrays)
//    m_ib.write(indices.data(), m_ib.size());
//    rex::IndexBufferDesc ib_desc {rex::memory::BlobView(m_ib), rex::IndexBufferFormat::Uint16, indices.size()};
//
//    // Create the mesh geometry
//    m_geometry = rsl::make_unique<rex::Mesh>("scene_geometry"_med, vb_desc, ib_desc);
//
...
//    rex::add_mesh(m_geometry.get(), box, rex::memory::BlobView(box_cb));
//
//    // Grid
...
//    rex::add_mesh(m_geometry.get(), grid, rex::memory::BlobView(grid_cb));
//
...
//
//      rex::add_mesh(m_geometry.get(), cylinder, rex::memory::BlobView(left_cyl_cb));
//      rex::add_mesh(m_geometry.get(), cylinder, rex::memory::BlobView(right_cyl_cb));
//      rex::add_mesh(m_geometry.get(), sphere, rex::memory::BlobView(left_sphere_cb));
//      rex::add_mesh(m_geometry.get(), sphere, rex::memory::BlobView(right_sphere_cb));
//    }
//  }
//
//  rex::Submesh SampleScene::build_submesh(const rex::mesh_factory::MeshData16& meshData, rsl::vector<rex::VertexPosNormCol>& vertices, rsl::vector<u16>& indices) // NOLINT(readability-convert-member-functions-to-static)
//  {
//    rex::Submesh submesh;
